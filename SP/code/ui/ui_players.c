...
static void UI_TorsoSequencing(playerInfo_t *pi) {
	...
	if (currentAnim == TORSO_ATTACK	|| currentAnim == TORSO_ATTACK2 || currentAnim == TORSO_ATTACK3 || currentAnim == TORSO_ATTACK4 || currentAnim == TORSO_ATTACK5 || currentAnim == TORSO_ATTACK5B) {
		UI_SetTorsoAnim(pi, TORSO_STAND);
		return;
	}
...
// 		 &torso.oldframe, &torso.frame, &torso.backlerp);
	...
	if (dp_realtime <= pi->muzzleFlashTime) {
		...
		if (pi->flashDlightColor[0] || pi->flashDlightColor[1] || pi->flashDlightColor[2]) {
			trap_R_AddLightToScene(flash.origin, 200 + (rand() & 31), pi->flashDlightColor[0], pi->flashDlightColor[1], pi->flashDlightColor[2], 0);
		}
	}
	// add the backpack
	if (pi->backpackModel) {
