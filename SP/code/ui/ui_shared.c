		...
		case K_BACKSPACE:
			...
			if (id != -1) {
				key = -1;      // null out the key, but let it pass down so it can get 'unbound'
								...
								// so if user has both 'r' and 'g' bound to ' + attack' and they only want the 'r', // they click to bind the key, < backsp > to kill the bindings(which appears to the user to work)
