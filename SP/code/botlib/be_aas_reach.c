...
int AAS_AreaReachability(int areanum) {
	...
	// if this area is disabled, then fail
...
=======================================================================================================================================
*/
int AAS_FallDamageDistance(int facenum, vec3_t center) {
	int i;
	float scale;
	aas_face_t *face;
	aas_edge_t *edge;
...
=======================================================================================================================================
*/
float AAS_MaxJumpHeight(float distance) {
...
// 					Log_Write("lines no overlap: from area %d to %d\r\n", area1num, area2num);
					...
					if (groundface1->faceflags & FACE_GROUND) {
						...
						if (dist < ground_bestdist || // or the vertical distance is pretty much the same
						...
						// if the vertical distance is smaller
						if (dist < water_bestdist || // or the vertical distance is pretty much the same
							// but the overlapping part of the edges is longer
...
// 			if (!(AAS_PointContents(trace.endpos) & (CONTENTS_LAVA|CONTENTS_SLIME)))
		...
		AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 3, 30, 0.1, SE_HITGROUND|SE_ENTERWATER|SE_ENTERSLIME |
		...
		if (move.stopevent & SE_ENTERLAVA) {
			return qfalse;                                 // modified since slime is no longer deadly
		}
...
int AAS_Reachability_Ladder(int area1num, int area2num) {
	...
	if (ladderface1 && ladderface2) {
		...
		if (ladderface1vertical) {
			...
			if (i >= area2->numfaces && area2num != area1num && // the reachabilities shouldn't exist already
...
// 	VectorScale(velocity, speed, velocity);
		...
		*/
		...
		if (velocity[0] || velocity[1]) {
			...
			for (i = 0; i < 20; i++) {
				AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse, velocity, cmdmove, 0, 30, 0.1, SE_HITGROUND|SE_ENTERWATER|SE_ENTERSLIME |
		...
		for (area2num = 1; area2num < (*aasworld).numareas; area2num++) {
			...
			for (i = 0; i < area2->numfaces; i++) {
				...
				zvel = velocity[2];
				// get the horizontal speed for the jump, if it isn't possible to calculate this speed (the jump is not possible) then there's no jump reachability created(the jump is not possible) then there's no jump reachability created
				...
				if (ret && speed < 150) {
					...
					// if (hordist < 1.6 * facecenter[2] - areastart[2]) {
						...
						VectorScale(dir, speed, cmdmove);
											AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse,   velocity, cmdmove, 30, 30, 0.1,   SE_ENTERWATER|SE_ENTERSLIME |
...
int AAS_Reachability_Grapple(int area1num, int area2num) {
	...
	for (i = 0; i < area2->numfaces; i++) {
		...
		if ((*aasworld).areasettings[areanum].contents & AREACONTENTS_LAVA) { // modified since slime is no longer deadly
...
=======================================================================================================================================
*/
void AAS_Reachability_WalkOffLedge(int area1num, int area2num) {
		...
		for (n = 0; n < 1 /*2*/; n++) {
			...
			if (ret && speed < 270) {
				...
				// if (hordist < 1.6 * (facecenter[2] - areastart[2])) {
					...
					VectorSet(cmdmove, 0, 0, 0);
					*/
									AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 30, 30, 0.1, SE_ENTERWATER|SE_ENTERSLIME |
											   SE_ENTERLAVA|SE_HITGROUNDDAMAGE |
					...
					if (abs(edge1num) == abs(edge2num)) {
						...
						if (!AAS_AreaGrounded(reachareanum) && !AAS_AreaSwim(reachareanum)) {
							...
							break;
											...
											if ((*aasworld).areasettings[reachareanum].contents & AREACONTENTS_LAVA) { // modified since slime is no longer deadly
