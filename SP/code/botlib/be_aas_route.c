...
// routing memory calls go here, so we can change between Hunk / Zone easily
...
void AAS_InitRouting(void) {
	...
	// load or create the routing cache
...
// memset((*aasworld).areaupdate, 0, (*aasworld).numareas * sizeof(aas_routingupdate_t));
	...
	while (updateliststart) {
		...
		for (i = 0, revlink = revreach->first; revlink; revlink = revlink->next, i++) {
			...
			t = curupdate->tmptraveltime + //AAS_AreaTravelTime(curupdate->areanum, curupdate->start, reach->end) + curupdate->areatraveltimes[i] + reach->traveltime;
...
int AAS_AreaRouteToGoalArea(int areanum, vec3_t origin, int goalareanum, int travelflags, int *traveltime, int *reachnum) {
	...
	for (i = 0; i < cluster->numportals; i++, pPortalnum++) {
		...
		// this may be NULL if we were unable to calculate the cache this frame
		if (!areacache) {
			return qfalse;
		}
...
// 	maxs[2] = 0;
	...
	for (i = 1; i < numAreas; i++) {
		...
		for (j = 1; j < numAreas; j++) {
			...
			// check PVS first, since it's much faster
			...
			if (!besttraveltime || besttraveltime > t) {
				...
				if ((*aasworld).areavisibility[nextareanum]) {
					...
					if (!AAS_AreaVisible(enemyareanum, nextareanum)) { // now last of all, check that this area is a safe hiding spot
						if (((*aasworld).visCache[nextareanum] == 2) || (!(*aasworld).visCache[nextareanum] && !botimport.AICast_VisibleFromPos(enemyorigin, enemynum, (*aasworld).areawaypoints[nextareanum], srcnum, qfalse))) {
...
int AAS_FindAttackSpotWithinRange(int srcnum, int rangenum, int enemynum, float rangedist, int travelflags, float *outpos) {
	...
	static float lastTime;
	// currently doesn't work with multiple AAS worlds, so only enable for the default world
	// if (aasworld != aasworlds) {
		return 0;
	}
	// don't run this more than once per frame
