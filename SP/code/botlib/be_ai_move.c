...
// disabled this to prevent wierd navigational behaviour(mostly by Zombie, since it's so slow)
//#define AVOIDREACH
...
// provide a means of resetting the avoidreach, so if a bot stops moving, they don't avoid the area they were heading for
// ========================================================================
...
int BotFuzzyPointReachabilityArea(vec3_t origin) {
	...
	for (z = 1; z >= -1; z -= 1) {
		for (x = 1; x >= -1; x -= 2) {
			for (y = 1; y >= -1; y -= 2) {
				VectorCopy(origin, end);
				// increased this for Wolf larger bounding boxes
				end[0] += x * 256; // 8;
				end[1] += y * 256; // 8;
				end[2] += z * 200; // 12;
				numareas = AAS_TraceAreas(origin, end, areas, points, 10);
...
int BotGetReachabilityToGoal(vec3_t origin, int areanum, int entnum, int lastgoalareanum, int lastareanum, int *avoidreach, float *avoidreachtimes, int *avoidreachtries, bot_goal_t *goal, int travelflags) {
	...
	for (reachnum = AAS_NextAreaReachability(areanum, 0); reachnum;
	  ...
	  // get the reachability from the number
		...
		// if this sends us to a looped route, ignore it
		...
		// not sure why this was disabled, but it causes looped links in the route - cache
		// update.. seems to work better like this....
		t += reach.traveltime; // + AAS_AreaTravelTime(areanum, origin, reach.start);
		// t += reach.traveltime + AAS_AreaTravelTime(areanum, origin, reach.start);
		// if there exists other entities in this area, avoid it
...
int BotMovementViewTarget(int movestate, bot_goal_t *goal, int travelflags, float lookahead, vec3_t target) {
	...
	while (reachnum && dist < lookahead) {
		...
		reachnum = BotGetReachabilityToGoal(reach.end, reach.areanum, -1, ms->lastgoalareanum, lastareanum, ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal, travelflags);
...
int BotPredictVisiblePosition(vec3_t origin, int areanum, bot_goal_t *goal, int travelflags, vec3_t target) {
	...
	for (i = 0; i < 20 && (areanum != goal->areanum); i++) {
		// 
		reachnum = BotGetReachabilityToGoal(end, areanum, -1, lastgoalareanum, lastareanum, avoidreach, avoidreachtimes, avoidreachtries, goal, travelflags);
...
bot_moveresult_t BotTravel_BarrierJump(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if (dist < 9) {
		EA_Jump(ms->client);
		// do the movement also, so we have momentum to get onto the barrier
...
bot_moveresult_t BotFinishTravel_BarrierJump(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if (ms->velocity[2] < 250) {
		// extend the end point a bit, so we strive to get over the ledge more
		vec3_t end;
...
bot_moveresult_t BotTravel_WalkOffLedge(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if ((dist < 72) && (DotProduct(dir, hordir) < 0)) { // walk in the direction of start ->end
		...
		if (reachhordist < 20) {
			// speed = 100;
			speed = 200;   // increased this to speed up travel speed down steps
		...
		if (ent != -1) {
			...
			if (modelnum >= 0 && modelnum < MAX_MODELS) {
				...
				if (modeltype == MODELTYPE_FUNC_PLAT) {
					...
					if (reach.traveltype != TRAVEL_ELEVATOR || // NOTE: the face number is the plat model number
					...
					if (reach.traveltype != TRAVEL_FUNCBOB || // NOTE: the face number is the func_bobbing model number
						(reach.facenum & 0x0000FFFF) != modelnum) {
						reachnum = AAS_NextModelReachability(0, modelnum);
...
// best calc it each frame, especially for Zombie, which moves so slow that we can't afford to take a long route
// 	reachnum = 0;
// done.
		if (reachnum) {
			AAS_ReachabilityFromNum(reachnum, &reach);
			...
			// special elevator case
				...
				// or the area changed
				if (ms->lastgoalareanum != goal->areanum || ms->reachability_time < AAS_Time() || ms->lastareanum != ms->areanum || ((ms->lasttime > (AAS_Time() - 0.5)) && (VectorDistance(ms->origin, ms->lastorigin) < 20 * (AAS_Time() - ms->lasttime)))) {
					reachnum = 0;
		...
		if (!reachnum) {
			...
			reachnum = BotGetReachabilityToGoal(ms->origin, ms->areanum, ms->entitynum, ms->lastgoalareanum, ms->lastareanum, ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal, travelflags);
		...
		for (i = numareas - 1; i >= 0; i--) {
			if (AAS_AreaJumpPad(areas[i])) {
				...
				foundjumppad = qtrue;
				lastreachnum = BotGetReachabilityToGoal(end, areas[i], ms->entitynum,  ms->lastgoalareanum, ms->lastareanum,  ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries,  goal, travelflags);
