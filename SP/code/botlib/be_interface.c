...
//AAS_ClearShownDebugLines();
	...
	if (bot_testhidepos) {
		...
		if (bot_testhidepos > 1) {
			if (hideposarea) {
				botimport.Print(PRT_MESSAGE, "hidepos(%i) %2.1f %2.1f %2.1f\n", hideposarea, (*aasworld).areawaypoints[hideposarea][0], (*aasworld).areawaypoints[hideposarea][1], (*aasworld).areawaypoints[hideposarea][2]);
			} else {
...
/*	face = AAS_AreaGroundFace(newarea, parm2);
	...
	*/
	...
	if (parm0 & 2) {
		...
		reachnum = BotGetReachabilityToGoal(origin, newarea, -1, lastgoalareanum, lastareanum, avoidreach, avoidreachtimes, avoidreachtries, &goal, TFL_DEFAULT|TFL_FUNCBOB, TFL_DEFAULT|TFL_FUNCBOB);
		AAS_ReachabilityFromNum(reachnum, &reach);
...
*/
	...
	if (bsptrace.fraction < 1.0) {
		AAS_DrawPlaneCross(bsptrace.endpos, bsptrace.plane.normal, bsptrace.plane.dist, // + bsptrace.exp_dist, bsptrace.plane.type, LINECOLOR_RED);
