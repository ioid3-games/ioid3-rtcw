...
static void CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask, int capsule, trace_t *tr) {
	...
	for (i = 0; i < cg_numSolidEntities; i++) {
		...
		// special case, ignore chairs if we are carrying them
...
void CG_PredictPlayerState(void) {
	...
	if (cg_nopredict.integer || cg_synchronousClients.integer
		 || (cg.snap->ps.eFlags & EF_MG42_ACTIVE)) { // somewhat of a hack, but just disable prediction if on MG42, since it's just not very prediction friendly
...
// 	return;
	...
	// cg.predictedPlayerState.weaponstate = oldPlayerState.weaponstate;	// added this, since they can become unsynched on loadgame, leaving incorrect anims
	...
	for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
		...
		if (cg.predictedPlayerState.commandTime == oldPlayerState.commandTime) {
			...
			if (cg.thisFrameTeleport) {
				...
				CG_AdjustPositionForMover(cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.oldTime, adjusted, cg.predictedPlayerState.viewangles, new_angles, deltaAngles);
		...
		// if waiting for mission stats to go, ignore all input
	...
	CG_AdjustPositionForMover(cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.time, cg.predictedPlayerState.origin, cg.predictedPlayerState.viewangles, cg.predictedPlayerState.viewangles, deltaAngles);
