...
#if 0  // this entire function not used anymore, since we now grab all this stuff from the server
...
static qboolean CG_RegisterClientSkin(clientInfo_t *ci, const char *modelName, const char *skinName) {
	char filename[MAX_QPATH];
	// try and register the new "body_ * .skin" file for skeletal animation
...
*/
			...
			// these are also used by supersoldier "spirits" in end map
...
// ---- (SA) added checks for same head. FIXME: soon this will be more efficient than just looking for a complete character(body / head) match.
		if (
			!Q_stricmp(ci->modelName, match->modelName) && !Q_stricmp(ci->skinName, match->skinName) && !Q_stricmp(ci->hSkinName, match->hSkinName)
		) {
...
// if (cent->currentState.number == 3)
	...
	if (cg.time >= lf->frameTime) {
		...
		} else if (lf->oldAnimationNumber != lf->animationNumber && (!anim->moveSpeed || lf->oldFrame < anim->firstFrame || lf->oldFrame >= anim->firstFrame + anim->numFrames)) { // added this so walking frames don't always get reset to 0, which can happen in the middle of a walking anim, which looks wierd
			lf->frameTime = lf->animationTime;     // initial lerp
...
static void CG_IdleHeadMovement(centity_t *cent, const vec3_t torsoAngles, vec3_t headAngles) {
	...
	if ((cent->currentState.eFlags & EF_DEAD) || !(cent->currentState.eFlags & EF_HEADLOOK)) {
		...
		VectorSet(cent->pe.headLookIdeal, diffRandMax * (-0.25 + crandom()) * 0.25, // pitch
...
static void CG_PlayerPowerups(centity_t *cent) {
	...
	// AI don't use these effects, they are generally added manually by the game
	if (cent->currentState.aiChar) {
		return;
	}
...
// 		return;
// 	}
// Not using friendly sprites in GT_WOLF
	...
	if (dist < SHADOW_MAX_DIST) { // show more detail
		...
		for (tagIndex = 0; shadowParts[tagIndex].tagname; tagIndex++) {
			...
			for (subIndex = 0; (subIndex = CG_GetOriginForTag(cent, &cent->pe.legsRefEnt, shadowParts[tagIndex].tagname, subIndex, origin, axis)) >= 0; subIndex++) {
				...
				CG_ImpactMark(shadowParts[tagIndex].shader, origin, trace.plane.normal, angles[YAW] /*cent->pe.legs.yawAngle*/, alpha, alpha, alpha, 1, qfalse, shadowParts[tagIndex].size * scale, qtrue, -1);
			}
		}
	}
...
void CG_AddZombieSpiritEffect(centity_t *cent) {
	...
	for (t = cent->pe.lastZombieSpirit + step; t <= cg.time; t += step) {
		...
		for (i = 0; i < MAX_ZOMBIE_SPIRITS; i++) {
			...
			cent->pe.zombieSpiritTrailHead[i] = CG_AddTrailJunc(cent->pe.zombieSpiritTrailHead[i],  cgs.media.zombieSpiritTrailShader,  t,  STYPE_STRETCH,  p[i],  trailLife * 2,  alpha,  0.0, (0.5 + 0.5 * fadeRatio) * idealWidth,  0,  TJFL_NOCULL,  colorWhite,  colorWhite,  1.0, 1);
...
#define LOPER_MAX_POINT_TESTS_PERFRAME  20
	...
	for (i = 0; i < numPoints; i++) {
		...
		if ((cent->currentState.eFlags & EF_MONSTER_EFFECT) && ((!cent->pe.lightningTimes[i]) || (cent->pe.lightningTimes[i] > cg.time) || (cent->pe.lightningTimes[i] < cg.time - 50) || (VectorDistance(cent->lerpOrigin, cent->pe.lightningPoints[i]) > maxDist))) {
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, int powerups, int team, entityState_t *es, const vec3_t fireRiseDir) {
	...
	// if in camera mode, force a full lod, since we are in a controlled environment
	if (cg.cameraMode) {
...
// 	}
	...
	if (powerups & (1 << PW_INVIS)) {
		...
		if (es->time2) {
			...
			if (es->eFlags & EF_MONSTER_EFFECT2 && cent->currentState.effect2Time < cg.time) { // Zombie death effect
...
#endif
/*
	} else if (es->eFlags & EF_MONSTER_EFFECT2 && es->aiChar == AICHAR_ZOMBIE && cent->currentState.effect2Time < cg.time) {	// Zombie death effect
		...
		if (cent->pe.zombieDeathFadeEnd + fadeRiseTime > cg.time) {
			...
			if (cent->pe.zombieDeathFadeEnd > cg.time) {
				...
				if (ent->hModel == cent->pe.legsRefEnt.hModel || ent->hModel == cent->pe.torsoRefEnt.hModel) {
...
void CG_AnimPlayerConditions(centity_t *cent) {
	...
	// changed this since we dont need to be careful about bandwidth anymore, and this method
...
// ---- (SA)	modified
		...
		char *ssTags[] = {     "tag_chest", "tag_calfleft", "tag_armleft", "tag_back", "tag_legleft", "tag_calfright", "tag_armright", "tag_back", "tag_legright",

								"tag_footleft", "tag_footright", "tag_sholeft", "tag_shoright", "tag_torso", "tag_calfleft", "tag_calfright"};

		char *heinrichTags[] = {"tag_chest", "tag_calfleft", "tag_armleft", "tag_back", "tag_legleft", "tag_calfright", "tag_armright", "tag_back", "tag_legright",

								"tag_footleft", "tag_footright", "tag_sholeft", "tag_shoright", "tag_torso", "tag_legleft", "tag_legright",

								"tag_sholeft", "tag_shoright", "tag_legleft", "tag_legright", "tag_legleft", "tag_legright"};
		// TTimo: init
		...
		// TTimo: unused
		/*
			char *tags[] = {		"tag_armleft", "tag_armright", "tag_back", "tag_back", "tag_calfleft", "tag_calfleft", "tag_calfright", "tag_calfright", "tag_chest", "tag_chest", "tag_footleft", "tag_footright", "tag_legleft", "tag_sholeft", "tag_shoright", "tag_torso"
									};
					*/

// TTimo: unused
		/*
int parts[] = {34, 38, 0, 19, 0, 14, 21, 15, 16, 0, 0, 32, 0, 33, 0, 45
				};
	*/
...
// 		VectorCopy(cg.snap->entities[fleshEntityNum - 1].pos.trBase, legs.origin);
	...
	// restore the cent so we don't interfere with animation timings
	*cent = backupCent;
