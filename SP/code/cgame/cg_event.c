...
void CG_PainEvent(centity_t *cent, int health, qboolean crouching) {
	...
	{"tag_legleft", PEFOFS(legsRefEnt), BOTH_PAIN7}, {NULL, 0, 0},};
	vec3_t tagOrg;
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
	...
	case EV_FOOTSTEP_ROOF: // tile sound
		...
		if (cg_footsteps.integer) {
			// if (cent->currentState.aiChar == AICHAR_ELITEGUARD)
			// 	trap_S_StartSound(NULL, es->number, CHAN_BODY, // 		cgs.media.footsteps[FOOTSTEP_ELITE_ROOF][footstepcnt]);
			// else
...
// 					VectorScale(dir, 16, sprVel);
			...
			CG_AddDebris(cent->lerpOrigin, dir, 80, // speed
							1000,   // duration
							3 + rand() % 2);      // count
		}
