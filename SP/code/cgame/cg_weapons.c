...
static void CG_RunWeapLerpFrame(clientInfo_t *ci, weaponInfo_t *wi, lerpFrame_t *lf, int newAnimation, float speedScale) {
	...
	// if the animation number is the same, but we are using a different weapon, we need to reset the lf->animation
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent) {
	...
	// Special case for WP_CLASS_SPECIAL
...
*/
	...
	if (ps->weapon > WP_NONE) {
		// handle WP_CLASS_SPECIAL for different classes
	...
	case WP_VENOM:
		...
		if (!clientNum) {
			...
			if (surfFlags & SURF_SNOW) {
				CG_AddDirtBulletParticles(origin, dir,   120, // speed
										   900, // duration
										   3, // count
										   0.6f,   20,   4,   0.3f,   "water_splash"); // rand scale
			}
			// grass / gravel
			if (surfFlags & (SURF_GRASS|SURF_GRAVEL)) {
				CG_AddDirtBulletParticles(origin, dir,   190, // speed
										   800, // duration
										   3, // count
										   0.1,   60,   10,   0.5,   "dirt_splash"); // rand scale
			} else {
		...
		if (cg_gameType.integer == GT_SINGLE_PLAYER) { // JPW NERVE
			for (i = 0; i < 5; i++) {
				for (j = 0; j < 3; j++)
					sprOrg[j] = origin[j] + 64 * dir[j] + 24 * crandom();
				sprVel[2] += rand() % 50;
				CG_ParticleExplosion("blacksmokeanimb", sprOrg, sprVel,  3500 + rand() % 250, // duration
									  10,     // startsize
									  250 + rand() % 60);         // endsize
			}
			...
			CG_ParticleExplosion("explode1", sprOrg, sprVel, 1200, // duration
								  9, // startsize
								  300);       // endsize
...
// JPW NERVE
			...
			CG_AddDebris(origin, dir, 400 + random() * 300, // speed
						  1400, // duration
						  7 + rand() % 12);   // count
		}
		break;
