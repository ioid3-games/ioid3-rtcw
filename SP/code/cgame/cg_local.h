...
typedef struct {
	...
	animation_t *cgAnim;   // pointer to the root of the animation array to use(*animation above points at the current sequence)	
} lerpFrame_t;
...
typedef enum {
	LEMT_NONE, LEMT_BLOOD
} leMarkType_t;        // fragment local entities can leave marks on walls

typedef enum {
	LEBS_NONE, LEBS_BLOOD, LEBS_ROCK, LEBS_WOOD, LEBS_BRASS, LEBS_BONE
...
typedef enum {
	W_PART_1, W_PART_2, W_PART_3, W_PART_4, W_PART_5, W_PART_6, W_PART_7, W_MAX_PARTS
} barrelType_t;
...
// end
	...
	int lastFiredWeapon;
	int lastWeapSelInBank[MAX_WEAP_BANKS];         // remember which weapon was last selected in a bank for 'weaponbank' commands 
...
// modified
	...
	qhandle_t spotLightBaseModel;
	qhandle_t spotLightLightModel;
	qhandle_t spotLightLightModelBroke;
...
// sfxHandle_t flightSound;
	...
	sfxHandle_t bookSound;

	sfxHandle_t staminaSound;  
...
typedef struct {
	...
	char itemPrintNames[MAX_ITEMS][32];            
	int cursorX;
	int cursorY;
...
extern vmCvar_t cg_hintFadeTime;           
...
extern vmCvar_t cg_useSuggestedWeapons;        
...
void CG_ImpactMark(qhandle_t markShader, const vec3_t origin, const vec3_t dir, float orientation, float r, float g, float b, float a, qboolean alphaFade, float radius, qboolean temporary, int duration);
...
void trap_S_FadeBackgroundTrack(float targetvol, int time, int sound); 
...
typedef enum {
	SYSTEM_PRINT, CHAT_PRINT, TEAMCHAT_PRINT
} q3print_t;
