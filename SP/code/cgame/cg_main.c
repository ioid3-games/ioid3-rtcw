...
cvarTable_t cvarTable[] = {
	...
	{&cg_emptyswitch, "cg_emptyswitch", "0", CVAR_ARCHIVE}, // END JOSEPH
	...
	{&cg_skybox, "cg_skybox", "1", CVAR_CHEAT}, // done.
...
// 	{&cg_smoothClients, "cg_smoothClients", "0", CVAR_USERINFO|CVAR_ARCHIVE},
	...
	{&cg_teamChatsOnly, "cg_teamChatsOnly", "0", CVAR_ARCHIVE}, // the following variables are created in other parts of the system, but we also reference them here
	...
	// Rafael - particle switch
	{&cg_wolfparticles, "cg_wolfparticles", "1", CVAR_ARCHIVE}, // done
...
// 	{&cg_youGotMail, "cg_youGotMail", "0", CVAR_ROM}, // used to display notebook new - info icon
	...
	{&mp_itemDesc, "mp_itemDesc", "", 0}, // - NERVE - SMF
...
// 	cgs.media.hintShaders[HINT_BUILD] = trap_R_RegisterShader("gfx/2d/buildHint");		// DHM - Nerve
	...
	cgs.media.youGotMailShader = trap_R_RegisterShader("gfx/2d/yougotmail");   
	cgs.media.youGotObjectiveShader = trap_R_RegisterShader("gfx/2d/yougotobjective");  

	for (i = 0; i < NUM_CROSSHAIRS; i++) {
		cgs.media.crosshairShader[i] = trap_R_RegisterShaderNoMip(va("gfx/2d/crosshair%c", 'a' + i));
	}

	cgs.media.crosshairFriendly = trap_R_RegisterShader("gfx/2d/friendlycross"); 
...
// 	cgs.media.hastePuffShader = trap_R_RegisterShader("hasteSmokePuff");
	// Allow flags again, will change later to more appropriate models
...
// 	cgs.media.fireTrailShader = trap_R_RegisterShader("fireTrail");
	...
	cgs.media.thirdPersonBinocModel = trap_R_RegisterModel("models/powerups/holdable / binocs_thirdperson.md3"); 
	cgs.media.cigModel = trap_R_RegisterModel("models/players/ infantryss / acc / cig.md3");   
...
// 	cg.weaponSelect = WP_MP40;
	...
	// clear all sounds, so we dont hear anything after level load
...
// 	trap_S_StartBackgroundTrack("sound / music / fla_mp03.wav", "sound / music / fla_mp03.wav", 1);
	// NERVE - SMF
// commented out 'cause this moved
...
void CG_S_AddRangedLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx, int range) {
	trap_S_AddLoopingSound(entityNum, origin, velocity, range, sfx, 255); // assume full volume, since thats how it worked before
}
