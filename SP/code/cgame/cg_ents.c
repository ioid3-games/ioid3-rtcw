...
void CG_PositionEntityOnTag(refEntity_t *entity, const refEntity_t *parent, char *tagName, int startIndex, vec3_t *offset) {
	...
	// not sure why this was here.. causes jittery torso animation, since the torso might have
...
// 		time = le->lastTrailTime + step;
		...
		le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.zombieSpiritTrailShader, 
// 											time, le->lastTrailTime + 50, STYPE_STRETCH, le->refEntity.origin, 
// 											(int)le->effectWidth, // trail life
											 1000, 0.3, 0.0, 
// 											le->radius, 50, 0, 0, // TJFL_FIXDISTORT, colorWhite, colorWhite, 1.0, 1);
		// if the player is on fire, then make the part on fire
...
// 	eorigin[2] += 8;						// and subtract the item's offset(that is used to place it on the ground)
	...
	// 		just pick them up. (if anybody is storing this somewhere, // 		for the current frame please let me know so I don't
...
static void CG_CalcEntityLerpPositions(centity_t *cent) {
	...
	if (cent != &cg.predictedPlayerEntity) {
		CG_AdjustPositionForMover(cent->lerpOrigin, cent->currentState.groundEntityNum, cg.snap->serverTime, cg.time, cent->lerpOrigin, cent->lerpAngles, cent->lerpAngles, NULL);
	}
}
...
void CG_AddPacketEntities(void) {
	...
	cg.rumbleScale = 0.0;  // default to 0 each frame
	...
	// count the number of players in the scene, so we can force low LOD's for dead bodies
