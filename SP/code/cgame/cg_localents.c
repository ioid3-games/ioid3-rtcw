...
void CG_BloodTrail(localEntity_t *le) {
	...
	for (; t <= t2; t += step) {
		...
		if (cent && cent->currentState.aiChar == AICHAR_ZOMBIE) {
			...
			le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.bloodTrailShader, t, STYPE_STRETCH, newOrigin, 180, 1.0, // start alpha
												 0.0, // end alpha
...
void CG_FragmentBounceSound(localEntity_t *le, trace_t *trace) {
	...
	if (le->leBounceSoundType == LEBS_BLOOD) {
		...
		// half the hits will make thunk sounds (this is just to start since we don't even have the sound yet...)
		if (rand() & 1) {
			int r = rand() & 3;
			sfxHandle_t s;
...
void CG_AddFragment(localEntity_t *le) {
	...
	// disabled since it caused debris to get stuck in walls(flags mostly)
	// if it has come to a halt, pause velocity
...
// 	}
	...
	if (le->pos.trType == TR_STATIONARY && le->leMarkType == LEMT_BLOOD) {
		// disabled for performance reasons in boss1
...
*/
		...
		// add a trail
		le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, le->refEntity.customShader, le->refEntity.origin, 200, 1.0 - lifeFrac, // start alpha
											 0.0, // 1.0 - lifeFrac, // end alpha
											 lifeFrac * 2.0 * (((le->endTime - le->startTime) > 400) + 1) * 1.5, lifeFrac * 2.0 * (((le->endTime - le->startTime) > 400) + 1) * 1.5);
		// if it is in a nodrop zone, remove it
...
void CG_AddFuseSparkElements(localEntity_t *le) {
	...
	while (time < cg.time) {
		...
		// if (lifeFrac > 0.2) {
		// add a trail
		le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.sparkParticleShader, time, STYPE_STRETCH, le->refEntity.origin, (int)(lifeFrac * (float)(le->endTime - le->startTime) / 2.0), 1.0 /* (1.0 - lifeFrac)*/, 0.0, FUSE_SPARK_WIDTH * (1.0 - lifeFrac), FUSE_SPARK_WIDTH * (1.0 - lifeFrac), TJFL_SPARKHEADFLARE, whiteColor, whiteColor, 0, 0);
		//}
...
void CG_AddBloodElements(localEntity_t *le) {
	...
	while (1) {
		...
		lifeFrac = (float)(cg.time - le->startTime) / (float)(le->endTime - le->startTime);
		// add a trail
		le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, cgs.media.bloodTrailShader, le->refEntity.origin, 200, 1.0 - lifeFrac, // start alpha
...
void CG_AddClientCritter(localEntity_t *le) {
	...
	while (time <= cg.time) {
		...
		if (le->leType == LE_ZOMBIE_SPIRIT) {
			le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.zombieSpiritTrailShader, time, STYPE_STRETCH, le->refEntity.origin, (int)le->effectWidth, // trail life
												 0.3 * alpha, 0.0, le->radius, 0, 0, // TJFL_FIXDISTORT, colorWhite, colorWhite, 1.0, 1);
		}

		if (le->leType == LE_HELGA_SPIRIT && le->refEntity.hModel != cgs.media.ssSpiritSkullModel) {
			le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.helgaSpiritTrailShader, time, STYPE_STRETCH, le->refEntity.origin, (int)le->effectWidth, // trail life
												 0.3 * alpha, 0.0, le->radius, 0, 0, // TJFL_FIXDISTORT, colorWhite, colorWhite, 1.0, 1);
		}
		// tracking factor
...
void CG_AddDebrisElements(localEntity_t *le) {
	...
	for (t = le->lastTrailTime + step; t < cg.time; t += step) {
...
#if 1  // flame
		if (le->effectWidth > 0) {
			le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, cgs.media.fireTrailShader, le->refEntity.origin, (int)(500.0 * (0.5 + 0.5 * (1.0 - lifeFrac))), // trail life
												 1.0, // alpha
												 0.5, // end alpha
												 3, // start width
...
#else  // spark line
		if (le->effectWidth > 0) {
			le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, cgs.media.sparkParticleShader, le->refEntity.origin, (int)(600.0 * (0.5 + 0.5 * (0.5 - lifeFrac))), // trail life
		...
		// smoke
		if (le->effectFlags & 1) {
			le->headJuncIndex2 = CG_AddSmokeJunc(le->headJuncIndex2, cgs.media.smokeTrailShader, le->refEntity.origin, (int)(2000.0 * (0.5 + 0.5 * (1.0 - lifeFrac))), // trail life
...
static void CG_AddSpriteExplosion(localEntity_t *le) {
	...
	if (le->light) {
		float light;
		// modified this so the light fades out rather than shrinking
		/*
