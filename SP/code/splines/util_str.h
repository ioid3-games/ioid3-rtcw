...
inline idStr::idStr
(
	const idStr text, int start, int end
) : m_data(NULL) {
	int i;
	int len;
...
(
	...
	if (text >= m_data->data && text <= m_data->data + m_data->len) {
		...
		// This means that I don't have to ensure that anything is alloced, // though I'll assert just in case.
