...
char *castAttributeStrings[] = {
	...
	"STARTING_HEALTH", "HEARING_SCALE", "HEARING_SCALE_NOT_PVS", "INNER_DETECTION_RADIUS", "PAIN_THRESHOLD_SCALE",
...
void AIChar_AIScript_AlertEntity(gentity_t *ent) {
	...
	// has to disable this so I could test some maps which have erroneously placed alertentity calls
...
// make sure client registers the default weapons for this char
	...
	// had to move this down since some dev maps don't properly spawn the guys in, so we
...
// 			reloading = qtrue;	// this gets reset at the Map_Restart() since the server unloads the game dll
		...
		if (ready) {
			...
			// spawn a thinker that will enable rendering after the client has had time to process the entities and setup the display
			// trap_Cvar_Set("cg_norender", "0");
			ent = G_Spawn();
			...
			trap_SendServerCommand(-1, va("snd_fade 1 %d", 2000)); 
		...
		if (ready) {
			...
			pcs->lastLoadTime = 0;
			pcs->attempts = 0;
			// disabled, since the pregame menu turns this off after the button is pressed, this isn't
			// required here
			// spawn a thinker that will enable rendering after the client has had time to process the entities and setup the display
			// trap_Cvar_Set("cg_norender", "0");
			// ent = G_Spawn();
