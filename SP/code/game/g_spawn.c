...
typedef enum {
	F_INT, F_FLOAT, F_STRING, F_VECTOR, F_ANGLEHACK
} fieldtype_t;
...
{"model2", FOFS(model2), F_STRING},
	...
	// ---- (SA) {"shader", FOFS(dl_shader), F_STRING}, // shader to use for a target_effect or dlight

	// (SA) for target_unlock
	{"key", FOFS(key), F_INT}, // done

	// (SA) for item placement
	{"stand", FOFS(s.frame), F_INT}, // (SA)	end

	// Rafael - mg42
	{"harc", FOFS(harc), F_FLOAT},
	{"varc", FOFS(varc), F_FLOAT}, // done.
...
void SP_info_notnull_big(gentity_t *ent); 
...
spawn_t spawns[] = {
	...
	{"func_group", SP_info_null}, // JOSEPH 1 - 26 - 00
	{"func_train_rotating", SP_func_train_rotating},
	{"func_secret", SP_func_secret}, // END JOSEPH
	// Rafael
	...
	// ---- (SA) Wolf triggers
	{"trigger_once", SP_trigger_once}, // ---- done
...
// ---- (SA) {"shooter_zombiespit", SP_shooter_zombiespit},
...
// JPW NERVE
	...
	{"ai_boss_heinrich", SP_ai_boss_heinrich}, // ---- (SA) {"ai_eliteguard", SP_ai_eliteguard},
