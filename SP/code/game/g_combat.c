...
void AddScore(gentity_t *ent, int score) {
	...
	// no scoring during single player
	// fix typo
...
void TossClientItems(gentity_t *self) {
	...
	G_ThrowChair(self, forward, qtrue); // drop chair if you're holding one 
...
// JPW NERVE new drop behavior
	...
	self->s.powerups = 0;
// only corpse in SP; in MP, need CONTENTS_BODY so medic can operate
	if (g_gametype.integer == GT_SINGLE_PLAYER) {
...
qboolean IsHeadShot(gentity_t *targ, gentity_t *attacker, vec3_t dir, vec3_t point, int mod) {
	...
	if (head_shot_weapon) {
		...
		// if there is a valid tag_head for this entity, then use that
...
void G_ArmorDamage(gentity_t *targ) {
	...
	// remove flame protection after enough parts gone
...
// 	if (reloading || saveGamePending) {	// map transition is happening, don't do anything
	...
	// not in single player(skill levels?)
...
*/
	...
	// Cast AI's don't hurt other Cast AI's as much
...
// 		knockback = 200;
	...
	// don't play these in single player
	if (g_gametype.integer != GT_SINGLE_PLAYER) {
		// done.
	...
	if (g_gametype.integer == GT_SINGLE_PLAYER) {   // removed from multiplayer -- plays havoc with pfaust & demolition balancing
	...
	if (IsHeadShot(targ, attacker, dir, point, mod)) {
		// different headshot behavior in multiplayer
		if (g_gametype.integer != GT_SINGLE_PLAYER) {
			...
			take *= 2;
			// allow headshot damage multiplier(helmets, etc)
	...
	if (take) {
		...
		if (targ->health <= 0) {
			...
			if (targ->die) { // mg42 doesn't have die func(FIXME)
...
//  MP weapons should do 1 / 8 damage through walls over 1 / 8th distance
		else {
			if (g_gametype.integer != GT_SINGLE_PLAYER) {
