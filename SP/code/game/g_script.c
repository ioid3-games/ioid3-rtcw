...
g_script_stack_action_t gScriptActions[] = {
	...
	{"aiscriptname", G_ScriptAction_AIScriptName}, // multiplayer scripting commands start with "wm_"(Wolf Multiplayer) {"wm_mapdescription", G_ScriptAction_MapDescription},
	...
	{"wm_set_round_timelimit", G_ScriptAction_SetRoundTimelimit}, // dhm
	...
	{"mu_play", G_ScriptAction_MusicPlay}, // (char *music) {"mu_stop", G_ScriptAction_MusicStop}, // (int time)						// time to fadeout
...
g_script_event_define_t gScriptEvents[] = {
	...
	{"trigger", G_Script_EventMatch_StringEqual}, // something has triggered us(always followed by an identifier) {"pain", G_Script_EventMatch_IntInRange}, // we've been hurt
	{"death", 0}, // RIP
	{"activate", G_Script_EventMatch_StringEqual}, // something has triggered us(always followed by an identifier) {"stopcam", 0},
	{NULL, 0}
};
...
void script_mover_blocked(gentity_t *ent, gentity_t *other) {
	...
	// until they move out the way. then we can just call the GotoMarker() again, // telling it that we are just now calling it for the first time, so it should
