...
#define JITTER_MAX (check->r.maxs[0] / 2.0)
	...
	if (check->client) {
		...
		// AI's need their ideal angle adjusted instead
...
void Blocked_DoorRotate(gentity_t *ent, gentity_t *other) {
	...
	// set this timer, so AI know not to try and open the door immediately
...
void Use_Static(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	if (ent->r.linked) {
		...
		// AAS areas are now free
		// if (ent->model)
		// 	G_SetAASBlockingEntity(ent, qfalse);
	} else {
		trap_LinkEntity(ent);
		// DISABLED since func_static will carve up AAS anyway, so blocking makes no sense
		// AAS areas are now occupied
		// if (ent->model)
		// 	G_SetAASBlockingEntity(ent, qtrue);
	}
}
...
void InitExplosive(gentity_t *ent) {
	...
	if (ent->model2) {
		...
		// these are used later for the explosion, // so we can use them now for the animation info
