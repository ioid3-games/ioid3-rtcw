...
void AICast_SpecialFunc(cast_state_t *cs) {
	...
	case AICHAR_WARZOMBIE:
		...
		if ((enemy) && (cs->vislist[cs->enemyNum].real_visible_timestamp > level.time - 5000) && (Distance(cs->bs->origin, enemy->s.pos.trBase) > 200) && (Distance(cs->bs->origin, enemy->s.pos.trBase) < 600) && (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE)
				// &&	(infront(ent, enemy)) && (infront(enemy, ent))) {
			// crouch
	...
	case AICHAR_HELGA:
		...
		if ((cs->weaponFireTimes[WP_MONSTER_ATTACK2] && (cs->weaponFireTimes[WP_MONSTER_ATTACK2] > level.time - 12000)) || (cs->weaponFireTimes[WP_MONSTER_ATTACK1] && (cs->weaponFireTimes[WP_MONSTER_ATTACK1] > level.time - 6000))) {
...
char *AIFunc_BattleHunt(cast_state_t *cs) {
	...
	if (AICast_EntityVisible(cs, cs->enemyNum, qtrue) // take into account reaction time && AICast_CheckAttack(cs, cs->enemyNum, qfalse) && cs->obstructingTime < level.time) {
	...
	if (numEnemies == -1) { // query mode
		...
		if (AICast_GotEnoughAmmoForWeapon(cs, cs->weaponNum)) {
			...
			for (i = 0; i < numEnemies; i++) {
				...
				if ((AICast_EntityVisible(cs, enemies[i], qfalse) && AICast_CheckAttack(cs, enemies[i], qfalse)) || ((VectorLength(cs->takeCoverPos) < 1 || dist <= 8) && (AICast_EntityVisible(AICast_GetCastState(enemies[i]), cs->entityNum, qfalse) || AICast_CheckAttack(AICast_GetCastState(enemies[i]), cs->entityNum, qfalse) || AICast_EntityVisible(AICast_GetCastState(enemies[i]), cs->entityNum, qtrue)))) {
					cs->enemyNum = enemies[i];
					return AIFunc_BattleStart(cs);
				} else if (cs->enemyNum < 0) {
					cs->lastEnemy = enemies[i];
	...
	*/
	...
	if (!moved && cs->leaderNum < 0 && (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE || g_entities[cs->entityNum].waterlevel > 1) && AICast_EntityVisible(cs, cs->enemyNum, qtrue)) {
		...
		if (tr.entityNum == followent->s.number) {
			...
			if (move.stopevent == PREDICTSTOP_HITENT) { // success!
				trap_EA_Move(cs->entityNum, dir, 400);
				// if we are really close, we might be stuck on a corner, so randomly move sideways
	...
	// if we are going to reach them soon, predict the attack
	{
		...
		else if (((cs->attributes[TACTICAL] > 0.85) && (cs->aiFlags & AIFL_ROLL_ANIM) && !client->ps.torsoTimer && !client->ps.legsTimer && (cs->lastRollMove < level.time - 800) && (move.numtouch == 0) && (moveDist > simTime * cs->attributes[RUNNING_SPEED] * 0.98) && move.groundEntityNum == ENTITYNUM_WORLD) && (AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, move.endpos, cs->attackcrouch_time > level.time, qfalse))) {
			cs->takeCoverTime = 0;
		...
		else if (!(cs->aiFlags & AIFL_WALKFORWARD) && (VectorDistance(cs->bs->origin, g_entities[cs->enemyNum].s.pos.trBase) < AICast_WeaponRange(cs, cs->weaponNum)) && (cs->obstructingTime < level.time) && (cs->attributes[TACTICAL] > 0.1) && (AICast_VisibleFromPos(cs->vislist[cs->enemyNum].visible_pos, cs->enemyNum, move.endpos, cs->entityNum, qfalse))) {
			// start a crouch attack?
			// if (cs->attributes[ATTACK_CROUCH] > 0.1) {
			// 	cs->attackcrouch_time = level.time + 3000;
			// else
...
char *AIFunc_BattleTakeCover(cast_state_t *cs) {
	...
	if (numEnemies == -1) { // query mode
		...
		if (AICast_GotEnoughAmmoForWeapon(cs, cs->weaponNum)) {
			...
			for (i = 0; i < numEnemies; i++) {
				if (AICast_CheckAttack(cs, enemies[i], qfalse) || AICast_CheckAttack(AICast_GetCastState(enemies[i]), cs->entityNum, qfalse) || AICast_EntityVisible(AICast_GetCastState(enemies[i]), cs->entityNum, qtrue)) {
...
// DISABLED: doesn't work well with new AI system
	...
	if (VectorLength(cs->takeCoverPos) > 1 && dist > 8) {
		...
		else if ((cs->aiFlags & AIFL_DIVE_ANIM && !client->ps.torsoTimer && cs->castScriptStatus.castScriptEventIndex < 0 && cs->lastRollMove < level.time - 800 && move.numtouch == 0 && (moveDist > simTime * cs->attributes[RUNNING_SPEED] * 0.98) && move.groundEntityNum == ENTITYNUM_WORLD) && (shouldAttack && !AICast_VisibleFromPos(g_entities[cs->enemyNum].s.pos.trBase, cs->enemyNum, move.endpos, cs->entityNum, qfalse))) {
		...
		if (!(cs->aiFlags & AIFL_MISCFLAG1) && !AICast_VisibleFromPos(cs->vislist[cs->enemyNum].real_visible_pos, cs->enemyNum, move.endpos, cs->entityNum, qfalse) && !AICast_VisibleFromPos(cs->vislist[cs->enemyNum].real_visible_pos, cs->enemyNum, cs->bs->origin, cs->entityNum, qfalse) && trap_AAS_PointAreaNum(move.endpos)) { // make sure the endpos is in a valid area
...
// 		VectorCopy(angles, bestangles);
	...
	if (cs->enemyNum < 0 ||
			!AICast_CheckAttack(cs, cs->enemyNum, qfalse) || (fabs(AngleDifference(angles[YAW], mg42->s.angles[YAW])) > mg42->harc) || (angles[PITCH] < 0 && angles[PITCH] + 5 < -mg42->varc) || (angles[PITCH] > 0 && angles[PITCH] - 5 > 5.0)) {
		qboolean shouldAttack;
		...
		if (!shouldAttack) {
			// keep firing at anything behind solids, in case they find a position where they can shoot us, but our checkattack() doesn't find a clear shot
			if (cs->enemyNum < 0 || !AICast_CheckAttack(cs, cs->enemyNum, qtrue) || (!cs->vislist[cs->enemyNum].real_visible_timestamp || (cs->vislist[cs->enemyNum].real_visible_timestamp < level.time - 2000))) {
				// face straight forward
				cs->ideal_viewangles[PITCH] = 0;
				return NULL;
			}
...
char *AIFunc_Battle(cast_state_t *cs) {
	...
	if ((cs->bs->cur_ps.weaponTime < 100) // if reloading, don't chase until ready && (cs->castScriptStatus.scriptNoMoveTime < level.time) && (/*!AICast_EntityVisible(cs, cs->enemyNum, qtrue) ||*/ !AICast_CheckAttack(cs, cs->enemyNum, qfalse))) {
		...
		if (cs->attributes[TACTICAL] > 0.3
				...
				&& cs->attributes[AGGRESSION] < (random() + 0.5 * cs->attributes[TACTICAL]) && (cs->takeCoverTime < level.time) && AICast_GetTakeCoverPos(cs, cs->enemyNum, cs->vislist[cs->enemyNum].real_visible_pos, cs->takeCoverPos)) {
			// start taking cover
		...
		// Take Cover?
		if (AICast_WantsToTakeCover(cs, qfalse) && (cs->takeCoverTime < level.time) && AICast_GetTakeCoverPos(cs, cs->enemyNum, cs->vislist[cs->enemyNum].real_visible_pos, cs->takeCoverPos)) {
			// go to a position that cannot be seen from the last place we saw the enemy, and wait there for some time
