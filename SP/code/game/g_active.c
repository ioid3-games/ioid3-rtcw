...
// 		pm.noFootsteps = qtrue;
	...
	// prevent player from standing on a chair which is on a script_mover
	...
	// no events while waiting for a loadgame
	if (saveGamePending) {
	...
	// allow AI Cast's to evaluate results of their pmove's
	...
	// AI needs to be interpolated, so use the old method(which sets TR_INTERPOLATE)
...
void ClientEndFrame(gentity_t *ent) {
	...
	// if they are using a dangerous weapon, let AI do their avoidance
	if (g_gametype.integer == GT_SINGLE_PLAYER) {
