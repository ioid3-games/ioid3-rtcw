...
void AICast_ChooseWeapon(cast_state_t *cs, qboolean battleFunc) {
	...
	if (cs->bs->cur_ps.weaponstate == WEAPON_RAISING || cs->bs->cur_ps.weaponstate == WEAPON_RAISING_TORELOAD || 
		 cs->bs->cur_ps.weaponstate == WEAPON_DROPPING || cs->bs->cur_ps.weaponstate == WEAPON_DROPPING_TORELOAD) { 
		return;
	}
