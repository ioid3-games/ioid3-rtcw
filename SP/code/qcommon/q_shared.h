...
// this is just here so different elements of the engine can be aware of this setting as it changes
...
typedef enum {
	...
	ERR_ENDGAME                // not an error. just clean up properly, exit to the menu, and start up the "endgame" menu 
} errorParm_t;
...
typedef enum {
	PLACE_STRETCH, PLACE_CENTER,
	// horizontal only
	PLACE_LEFT, PLACE_RIGHT,
	// vertical only
	PLACE_TOP, PLACE_BOTTOM
} screenPlacement_e;
...
typedef enum {
	AISTATE_RELAXED, AISTATE_QUERY, AISTATE_ALERT, AISTATE_COMBAT,
...
typedef struct playerState_s {
	...
	// need this to fix friction problems with slow zombie's whereby
	// the friction prevents them from accelerating to their full potential
	float friction;
	// AI character id is used for weapon association
	int aiChar;
	int teamNum;
	// Rafael
	int gunfx;
