...
void MSG_WriteDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from, usercmd_t *to) {
	...
	MSG_WriteDeltaKey(msg, key, from->cld, to->cld, 16);     // for multiplayer clientDamage
}
...
void MSG_ReadDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from, usercmd_t *to) {
	...
	if (MSG_ReadBits(msg, 1)) {
		...
		to->cld = MSG_ReadDeltaKey(msg, key, from->cld, 16);          // for multiplayer clientDamage
	} else {
...
netField_t entityStateFields[] = {
	...
	{NETF(onFireStart), 32}, {NETF(onFireEnd), 32}, {NETF(aiChar), 8}, {NETF(teamNum), 8}, {NETF(effect1Time), 32}, {NETF(effect2Time), 32}, {NETF(effect3Time), 32}, {NETF(aiState), 2}, {NETF(animMovetype), 6},
};
