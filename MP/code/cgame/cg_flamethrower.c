...
#define FLAME_SOUND_RANGE   1024.0
	...
	while (f) {
	 ...
	 (isClientFlame && f->blueLife > (lived / 2.0)) {
			skip = qfalse;
	 ...
	 (DotProduct(v, f->velDir) < 0) {
			...
			if (!skip) {
				...
				if (f->blueLife > lived * (f->ignitionOnly ? 3.0 : 3.0)) {
					...
					blueTrailHead = CG_AddTrailJunc(blueTrailHead, shader, cg.time, STYPE_STRETCH, f->org, 1, alpha, alpha, f->size * (f->ignitionOnly /*&& (cg.snap->ps.clientNum != f->ownerCent || cg_thirdPerson.integer)*/ ? 2.0 : 1.0), FLAME_MAX_SIZE, TJFL_NOCULL|TJFL_FIXDISTORT, c, c, 1.0, 5.0);
				}
				// fire stream
				if (!f->ignitionOnly) {
					float bscale;
	 ...
	 (lived / 1.3 < bscale * FLAME_BLUE_FADEIN_TIME(f->blueLife)) {
	 ...
	 (lived / 1.3 < (f->blueLife - FLAME_BLUE_FADEOUT_TIME(f->blueLife))) {
					...
					if (!fskip) {
						...
						fuelTrailHead = CG_AddTrailJunc(fuelTrailHead, cgs.media.flamethrowerFireStream, cg.time, (f->ignitionOnly ? STYPE_STRETCH : STYPE_REPEAT), f->org, 1, alpha, alpha, (f->size / 2 < f->sizeMax / 4 ? f->size / 2 : f->sizeMax / 4), FLAME_MAX_SIZE, TJFL_NOCULL|TJFL_FIXDISTORT|TJFL_CROSSOVER, c, c, 0.5, 1.5);
					}
				}
			}
		}
...
void CG_AddFlameChunks(void) {
	...
	while (f) {
	 ...
	 (f->ignitionOnly && (f->blueLife < (cg.time - f->timeStart))) {
				f->dead = qtrue;
			} else {
				CG_MoveFlameChunk(f);
	...
	while (f) {
	 ...
	 (!f->ignitionOnly || (centFlameInfo[f->ownerCent].lastFlameChunk == f)) { // don't draw the ignition flame after we start firing
...
// 	}
	...
	// No more client side damage
	// send client damage updates if required
	/*
	for (cent=cg_entities, i = 0; i < cgs.maxclients; cent++, i++) {
		if (centFlameInfo[i].lastDmgCheck > centFlameInfo[i].lastDmgUpdate && centFlameInfo[i].lastDmgUpdate < cg.time - 50) // JPW NERVE(cgs.gametype == GT_SINGLE_PLAYER ? 50 : 50)) -- sean changed clientdamage so this isn't a saturation issue any longer
		{
