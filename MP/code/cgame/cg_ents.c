...
// 			dir[1] = crandom() * 10;
// 			dir[2] = 10 + rnd * 30;
			CG_SmokePuff(cent->lerpOrigin, dir, 15 + (random() * 10), 0.3 + rnd, 0.3 + rnd, 0.3 + rnd, 0.4, 1500 + (rand() % 500), cg.time, cg.time + 500, 0, cgs.media.smokePuffShader);
		}
	}
	// dhm - end
// JPW NERVE same thing but for smoking barrels instead of nasty server - side effect from single player
	if (cgs.gametype >= GT_WOLF && cent->currentState.eFlags & EF_SMOKINGBLACK) {
		...
		if (cent->lastTrailTime < cg.time) {
			...
			CG_SmokePuff(cent->lerpOrigin, dir, 40 + random() * 70, // 40 + (rnd * 40), rnd * 0.1, rnd * 0.1, rnd * 0.1, 1, 2800 + (rand() % 4000), // 2500 + (random() * 1500), cg.time, 0, 0, cgs.media.smokePuffShader);
		}
	}
}
...
// modified
	...
	if (es->modelindex2) { // modelindex2 was specified for the ent, meaning it probably has an alternate model(as opposed to the one in the itemlist)
	 ...
	 (item->giType == IT_HEALTH || item->giType == IT_AMMO || item->giType == IT_POWERUP) {
