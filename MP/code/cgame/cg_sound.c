...
// cg_sound.c - parsing and use of sound script files
...
static void CG_SoundParseSounds(char *filename, char *buffer) {
	...
	while (1) {
	 ...
	 (!Q_strcasecmp(token, "announcer")) {
			sound.channel = CHAN_ANNOUNCER;
			continue;

		} else {
	 (!Q_strcasecmp(token, "body")) {
			sound.channel = CHAN_BODY;
			continue;

		} else {
	 (!Q_strcasecmp(token, "voice")) {
			sound.channel = CHAN_VOICE;
			continue;

		} else {
	 (!Q_strcasecmp(token, "weapon")) {
			sound.channel = CHAN_WEAPON;
			continue;

		} else {
	 (!Q_strcasecmp(token, "item")) {
			sound.channel = CHAN_ITEM;
			continue;

		} else {
	 (!Q_strcasecmp(token, "auto")) {
			sound.channel = CHAN_AUTO;
			continue;
		}
