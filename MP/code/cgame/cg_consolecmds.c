...
static void CG_LimboMessage_f(void) {
	...
	CG_PriorityCenterPrint(va("%s %s %s %s %s.", CG_TranslateString("You will spawn as an"), teamStr, classStr, CG_TranslateString("with a"), weapStr), SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH, -1);
}
...
static void CG_DumpLocation_f(void) {
	...
	CG_Printf("Entity dumped to '%s'(%i %i %i).\n", locfilename, (int)cg.snap->ps.origin[0], (int)cg.snap->ps.origin[1], (int)cg.snap->ps.origin[2]);
}
...
// 	{"camera", CG_Camera_f}, // duffy
	...
	{"SetWeaponCrosshair", CG_SetWeaponCrosshair_f}, // - NERVE - SMF
