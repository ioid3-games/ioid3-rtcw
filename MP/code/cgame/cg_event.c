...
// JPW NERVE copied here for mg42 SFX event
	...
	case EV_FOOTSTEP:
	 ...
	 (cent->currentState.aiChar >= AICHAR_STIMSOLDIER1 && cent->currentState.aiChar <= AICHAR_STIMSOLDIER3) {
	...
	case EV_FOOTSTEP_METAL:
	 ...
	 (cent->currentState.aiChar >= AICHAR_STIMSOLDIER1 && cent->currentState.aiChar <= AICHAR_STIMSOLDIER3) {
				trap_S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.footsteps[FOOTSTEP_SUPERSOLDIER_METAL][footstepcnt]);
			} else {
...
// nasty kludge because there's no WP_MG42 struct to hold echosound, so we pull it from GM's predefined globals hweaponSnd & hEchoweaponsnd
