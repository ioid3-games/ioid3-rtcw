...
*/
	...
	if (cgs.timelimit > 0) {
		...
		if (cgs.timelimit > 2 && !(cg.timelimitWarnings & 1) && (msec > (cgs.timelimit - 2) * 60 * 1000) && (msec < (cgs.timelimit - 2) * 60 * 1000 + 1000)) {
	 ...
	 (ps->persistant[PERS_TEAM] == TEAM_BLUE && cg.twoMinuteSound_a[0] != '0') {
				trap_S_StartLocalSound(cgs.media.twoMinuteSound_a, CHAN_ANNOUNCER);
			}
		}

		if (!(cg.timelimitWarnings & 2) && (msec > (cgs.timelimit) * 60 * 1000 - 30000) && (msec < (cgs.timelimit) * 60 * 1000 - 29000)) {
	 ...
	 (ps->persistant[PERS_TEAM] == TEAM_BLUE && cg.thirtySecondSound_a[0] != '0') {
