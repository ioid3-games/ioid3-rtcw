	...
	for (; t <= ent->trailTime; t += step) {
		...
		if (!ent->currentState.otherEntityNum2) { // axis team, generate red smoke
			CG_SmokePuff(origin, dir, 25 + rnd * 110, // width
							   rnd * 0.5 + 0.5, rnd * 0.5 + 0.5, 1, 0.5, 4800 + (rand() % 2800), // duration was 2800 + 		   t, 0, 0, cgs.media.smokePuffShader);
		} else {
			CG_SmokePuff(origin, dir, 25 + rnd * 110, // width
							   1.0, rnd * 0.5 + 0.5, rnd * 0.5 + 0.5, 0.5, 4800 + (rand() % 2800), // duration was 2800 + 		   t, 0, 0, cgs.media.smokePuffShader);
		}
	}
}
...
// localEntity_t *le;
	...
	for (; t <= ent->trailTime; t += step) {
		...
		BG_EvaluateTrajectory(&es->pos, t, lastPos);
		/*
		le = CG_SmokePuff(lastPos, vec3_origin, 5, // width
					  1, 1, 1, 0.33, 150 + rand()%350, // duration
					  t, 0, cgs.media.flameThrowerhitShader);
...
// trying this back on for DM
	...
	for (; t <= ent->trailTime; t += step) {
		...
		ent->headJuncIndex = CG_AddSmokeJunc(ent->headJuncIndex,   cgs.media.smokeTrailShader,   origin, 
// 											1500, 0.3, 10, 50);
											  1000, 0.3, 2, 20);
		ent->lastTrailTime = cg.time;
	}
...
// weapon pickup 'stand'
	...
	case WP_SMOKE_GRENADE:
		...
		break;
	case WP_SMOKETRAIL:    // for smoke bits from artillery spotter round, plus other effects maybe
		weaponInfo->missileTrailFunc = CG_PyroSmokeTrail;
		weaponInfo->missileDlight = 200;
		weaponInfo->wiTrailTime = 4000;
		weaponInfo->trailRadius = 256;
	...
	if (!cent->pe.teslaDamageApplyTime || cent->pe.teslaDamageApplyTime < cg.time - 200) {
		...
		if (cgs.localServer && cgs.gametype == GT_SINGLE_PLAYER) {
			...
			for (ctrav=cg_entities, i = 0; i < cgs.maxclients && numEnemies < 16; ctrav++, i++) {
				// proto and supersoldier are invulnerable to tesla
				...
				if (ctrav->currentState.aiChar && (ctrav != cent) && (ctrav->currentState.teamNum != playerTeam) && !(ctrav->currentState.eFlags & EF_DEAD) && ctrav->currentValid &&	// is in the visible frame
...
#define BARREL_SMOKE_TIME 1000
	...
	if (weaponNum == WP_GRENADE_LAUNCHER || weaponNum == WP_GRENADE_PINEAPPLE || weaponNum == WP_SPEARGUN || weaponNum == WP_SPEARGUN_CO2 || weaponNum == WP_KNIFE || weaponNum == WP_KNIFE2 || weaponNum == WP_CROSS || weaponNum == WP_DYNAMITE || weaponNum == WP_DYNAMITE2) {
		return;
	}
...
// cg.holdableSelectTime = 0;
	...
	// Move it under the Team Overlay
...
// JPW NERVE
		...
		// Changed to use COM_BitCheck so it would support 64 weapons
...
getNextBankWeap
	...
	sameBankPosition: if there's a weapon in the next bank at the same cycle, return that	(colt returns thompson for example) rather than the lowest weapon
=======================================================================================================================================
*/
...
getPrevBankWeap
	...
	sameBankPosition: if there's a weapon in the prev bank at the same cycle, return that	(thompson returns colt for example) rather than the highest weapon
=======================================================================================================================================
*/
...
// early out if we just fired Panzerfaust, go to pistola, then grenades
...
// 	(cent->currentState.clientNum != cg.snap->ps.clientNum && (cent->currentState.eFlags & EF_MG42_ACTIVE)))
	if ((cent->currentState.clientNum == cg.snap->ps.clientNum && cg.snap->ps.persistant[PERS_HWEAPON_USE]) || (cent->currentState.eFlags & EF_MG42_ACTIVE)) {
		if (cg.snap->ps.gunfx) {
			return;
		}
	...
	} else if (ent->weapon == WP_GRENADE_LAUNCHER || ent->weapon == WP_GRENADE_PINEAPPLE || ent->weapon == WP_DYNAMITE || ent->weapon == WP_DYNAMITE2 || ent->weapon == WP_SMOKE_GRENADE) { // JPW NERVE
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir, int surfFlags) { // (SA) modified to send missilehitwall surface parameters
	...
	float sfx2range = 0;        // kludge to get around lack of *VOLUME CONTROL * in sound mixer
	...
	case WP_KNIFE:
		i = rand() % 4;
		// this was cause global sound on client 0
...
// JPW NERVE
		if (!surfFlags) {
			...
			CG_AddBulletParticles(origin, dir, 20, // speed
								   800, // duration
								   3 + rand() % 6, // count
								   1.0);  // rand scale
		} else {
	...
	case WP_VENOM_FULL:
		...
		if (!clientNum) {
			...
			// if (sfx)
			CG_AddSparks(origin, dir, 350, // speed
						  200, // duration
						  15 + rand() % 7, // count
						  0.2);   // rand scale
			...
			if (cgs.gametype >= GT_WOLF && (surfFlags & SURF_GRASS || surfFlags & SURF_GRAVEL || surfFlags & SURF_SNOW)) { // JPW NERVE added SURF_SNOW
				...
				if (surfFlags & SURF_SNOW) {
					CG_AddDirtBulletParticles(origin, dir, 190, // speed
											   900, // duration
											   5, // count
				...
				if (sfx && (rand() % 3 == 0)) {
					CG_AddSparks(origin, dir, 450, // speed
								  300, // duration
								  3 + rand() % 3, // count
								  0.5);   // rand scale
				}
...
// check for water / dirt spurt
			if (trap_CM_PointContents(origin, 0) & CONTENTS_WATER) {
				...
				CG_AddDirtBulletParticles(trace.endpos, dir,   900, // speed
										   1800, // duration
										   15, // count
				...
				if (trace.surfaceFlags & SURF_GRASS || trace.surfaceFlags & SURF_GRAVEL) {
					CG_AddDirtBulletParticles(origin, dir, 600, // speed
											   2000, // duration
											   10, // count
...
// check for water / dirt spurt
		if (trap_CM_PointContents(origin, 0) & CONTENTS_WATER) {
			...
			CG_AddDirtBulletParticles(trace.endpos, dir,   400 + random() * 200, // speed
									   900, // duration
									   15, // count
			...
			if (trace.surfaceFlags & SURF_GRASS || trace.surfaceFlags & SURF_GRAVEL) {
				CG_AddDirtBulletParticles(origin, dir,   400 + random() * 200, // speed
										   3000, // duration
										   10, // count
			...
			CG_AddDebris(origin, dir, 400 + random() * 200, // speed
...
// check for water / dirt spurt
			if (trap_CM_PointContents(origin, 0) & CONTENTS_WATER) {
				...
				CG_AddDirtBulletParticles(trace.endpos, dir,   400, // speed
										   900, // duration
										   15, // count
				...
				if (trace.surfaceFlags & SURF_GRASS || trace.surfaceFlags & SURF_GRAVEL) {
					CG_AddDirtBulletParticles(origin, dir, 400, // speed
											   2000, // duration
											   10, // count
...
// JPW NERVE
		if (cg_gameType.integer == GT_SINGLE_PLAYER) {
			...
			// throw some debris
			CG_AddDebris(origin, dir, 120, // speed
						  2000, // 350, // duration
						// 15 + rand()%5);	// count
						  7 + rand() % 2); // count
		}
// multiplayer explosions over the top due to large damage radiusesesizes
		else {
// check for water / dirt spurt
			if (trap_CM_PointContents(origin, 0) & CONTENTS_WATER) {
/*
...
/*
		// some sparks
		CG_AddSparks(origin, dir, 200, // speed
						800, // duration
						5 + rand()%10, // count
						0.8);	// rand scale
*/
	...
	if (flesh) {
		...
		if (headshot && cg_blood.integer) {
			for (i = 0; i < 5; i++) {
				...
				CG_SmokePuff(origin, tmpv, 5 + rnd * 10, // width
								   1, rnd * 0.8, rnd * 0.8, 0.5, 500 + (rand() % 800), // duration was 2800 + 			   cg.time, 0, 0, cgs.media.fleshSmokePuffShader);
			}
		} else {
			...
			for (i = 0; i < 10; i++) {
				...
				VectorAdd(tmpv, tmpv2, tmpv);
				CG_SmokePuff(origin, tmpv, 5 + rnd * 10, // width
								   rnd * 0.3f + 0.5f, rnd * 0.3f + 0.5f, rnd * 0.3f + 0.5f, 0.125f, 500 + (rand() % 300), // duration was 2800 + 			   cg.time, 0, 0, cgs.media.smokePuffShader);
			}
		}
