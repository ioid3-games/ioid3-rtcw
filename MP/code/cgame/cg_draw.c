...
// 				}
			...
			CG_DrawStringExt(xx, y, st, pcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 3);
...
static void CG_DrawWeapReticle(void) {
	...
	if (sniper) {
	 ...
	 (cgs.glconfig.vidWidth * 480.0 < cgs.glconfig.vidHeight * 640.0) {
	...
	if (cg.limboMenu) { // JPW NERVE
		...
		if (cg_fixedAspect.integer) {
			...
			trap_R_DrawStretchPic(x + 0.5 * (cgs.glconfig.vidWidth - w), // JPW NERVE for scaled - down main windows
								   y + 0.5 * (cgs.glconfig.vidHeight - h), w, h, 0, 0, 1, 1, hShader);
		}
	}
	// NERVE - SMF
	if (cg.crosshairShaderAlt[cg_drawCrosshair.integer % NUM_CROSSHAIRS]) {
		...
		if (cg.limboMenu) { // JPW NERVE
			if (cg_fixedAspect.integer) {
				...
				trap_R_DrawStretchPic(x + 0.5 * (cg.refdef.width - w), y + 0.5 * (cg.refdef.height - h), w, h, 0, 0, 1, 1, cg.crosshairShaderAlt[cg_drawCrosshair.integer % NUM_CROSSHAIRS]);
			}
		} else {
			if (cg_fixedAspect.integer) {
				CG_DrawPic(((SCREEN_WIDTH - w) * 0.5f) + x, ((SCREEN_HEIGHT - h) * 0.5f) + y, w, h, cg.crosshairShaderAlt[cg_drawCrosshair.integer % NUM_CROSSHAIRS]);
...
static void CG_DrawLimboMessage(void) {
	...
	if (cg.snap->ps.persistant[PERS_RESPAWNS_LEFT] == 0) {
		...
		str = va(CG_TranslateString("Reinforcements deploy in %d seconds."), (int)(1 + (float)(cg_redlimbotime.integer - (cg.time % cg_redlimbotime.integer)) * 0.001f));
	} else {
		str = va(CG_TranslateString("Reinforcements deploy in %d seconds."), (int)(1 + (float)(cg_bluelimbotime.integer - (cg.time % cg_bluelimbotime.integer)) * 0.001f));
	}
...
static qboolean CG_DrawFollow(void) {
	...
	if (cg.snap->ps.pm_flags & PMF_LIMBO) {
	 ...
	 (cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_RED) {
			Com_sprintf(deploytime, sizeof(deploytime), CG_TranslateString("Deploying in %d seconds"), (int)(1 + (float)(cg_redlimbotime.integer - (cg.time % cg_redlimbotime.integer)) * 0.001f));
		} else {
			Com_sprintf(deploytime, sizeof(deploytime), CG_TranslateString("Deploying in %d seconds"), (int)(1 + (float)(cg_bluelimbotime.integer - (cg.time % cg_bluelimbotime.integer)) * 0.001f));
		}
...
static void CG_DrawCompass(void) {
	...
	if (cg.snap->ps.stats[STAT_PLAYER_CLASS] == PC_ENGINEER) {
	 ...
	 (cent->currentState.teamNum == 2 && cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE) {
				continue;
			}
...
static void CG_Draw2D(stereoFrame_t stereoFrame) {
	...
	if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR) {
		...
		if (cg.snap->ps.stats[STAT_HEALTH] > 0 || (cg.snap->ps.pm_flags & PMF_FOLLOW)) {
			...
			CG_DrawDynamiteStatus();
			CG_DrawCrosshairNames();
			// Don't draw icon in upper - right when switching weapons
			// CG_DrawWeaponSelect();
	 ...
	 (cg_fixedAspect.integer == 2 && !cg.limboMenu) {
				CG_SetScreenPlacement(PLACE_LEFT, PLACE_BOTTOM);
			}
