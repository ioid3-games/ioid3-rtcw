...
cvarTable_t cvarTable[] = {
	...
	// JOSEPH 10 - 27 - 99
	{&cg_autoactivate, "cg_autoactivate", "1", CVAR_ARCHIVE}, // END JOSEPH
...
// 	{&cg_smoothClients, "cg_smoothClients", "0", CVAR_USERINFO|CVAR_ARCHIVE},
	...
	{&cg_descriptiveText, "cg_descriptiveText", "1", CVAR_ARCHIVE}, // - NERVE - SMF
...
static const char *CG_FeederItemText(float feederID, int index, int column, qhandle_t *handle) {
	...
	*handle = -1;
	...
	if (info && info->infoValid) {
	 ...
	 (info->powerups & (1 << PW_REDFLAG)) {
				item = BG_FindItemForPowerup(PW_REDFLAG);
				*handle = cg_items[ITEM_INDEX(item)].icon;
	
		} else {
	 (info->powerups & (1 << PW_BLUEFLAG)) {
	 ...
	 (info->infoValid && info->team == TEAM_SPECTATOR) {
					return "Spectator";
				} else {
					return "";
				}
...
// pick a direction for smoke drift on the client -- cheap trick because it can be different on different clients, but who cares?
