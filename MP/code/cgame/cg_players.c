...
qboolean CG_EntOnFire(centity_t *cent) {
	...
	if (cent->currentState.number == cg.snap->ps.clientNum) {
		return ((cg.snap->ps.onFireStart < cg.time) && ((cg.snap->ps.onFireStart + 2000) > cg.time));
	} else {
...
// ---- (SA) modified this for head separation
	...
	if (headfail || !CG_RegisterClientModelname(ci, ci->modelName, ci->skinName)) {
	 ...
	 (cgs.gametype == GT_SINGLE_PLAYER && !headfail) {
	...
	// Not using in multiplayer, was causing hitches on player respawns / joins
...
#define ANIM_SPEEDMAX_HIGH  20
	...
	if (cg.time >= lf->frameTime) {
	 ...
	 (anim->moveSpeed < ANIM_SPEEDMAX_HIGH) {
	 ...
	 (lf->oldAnimationNumber != lf->animationNumber && (!anim->moveSpeed || lf->oldFrame < anim->firstFrame || lf->oldFrame >= anim->firstFrame + anim->numFrames)) { // added this so walking frames don't always get reset to 0, which can happen in the middle of a walking anim, which looks wierd
			lf->frameTime = lf->animationTime;     // initial lerp
	 ...
	 ((lf == &cent->pe.legs) && otherAnim && !(anim->flags & ANIMFL_FIRINGANIM) && ((lf->animationNumber & ~ANIM_TOGGLEBIT) == (cent->pe.torso.animationNumber & ~ANIM_TOGGLEBIT)) && (!anim->moveSpeed)) {
	 ...
	 ((lf == &cent->pe.torso) && otherAnim && !(anim->flags & ANIMFL_FIRINGANIM) && ((lf->animationNumber & ~ANIM_TOGGLEBIT) == (cent->pe.legs.animationNumber & ~ANIM_TOGGLEBIT)) && (otherAnim->moveSpeed)) {
...
static qboolean CG_PlayerShadow(centity_t *cent, float *shadowPlane) {
	...
	*shadowPlane = trace.endpos[2] + 1;
	...
	if (!(cent->currentState.eFlags & EF_ZOOMING) && (dist > SHADOW_MIN_DIST)) {
		...
		alpha *= distfade;
		CG_ImpactMark(cgs.media.shadowTorsoShader, trace.endpos, trace.plane.normal, 0, alpha, alpha, alpha, 1, qfalse, 16, qtrue, -1);
	} else {
		distfade = 0.0;
	}
	...
	if (dist < SHADOW_MAX_DIST) { // show more detail
		...
		for (tagIndex = 0; shadowParts[tagIndex].tagname; tagIndex++) {
			...
			for (subIndex = 0; (subIndex = CG_GetOriginForTag(cent, &cent->pe.legsRefEnt, shadowParts[tagIndex].tagname, subIndex, origin, axis)) >= 0; subIndex++) {
				...
				CG_ImpactMark(shadowParts[tagIndex].shader, origin, trace.plane.normal, angles[YAW] /*cent->pe.legs.yawAngle*/, alpha, alpha, alpha, 1, qfalse, shadowParts[tagIndex].size, qtrue, -1);
			}
		}
	}
