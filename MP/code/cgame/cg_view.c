...
void CG_KickAngles(void) {
	...
	for (t = cg.frametime; t > 0; t -= STEP) {
	 ...
	 (fabs(cg.kickAngles[i]) > maxKickAngles[i]) {
...
void CG_Zoom(void) {
	...
	if (cgs.gametype >= GT_WOLF && ((cg.snap->ps.pm_flags & PMF_FOLLOW) || cg.demoPlayback)) {
	 ...
	 (cg.predictedPlayerState.weapon == WP_SNIPERRIFLE)                                                   {
			cg.zoomval = cg_zoomDefaultSniper.value;

		} else {
	 (cg.predictedPlayerState.weapon == WP_FG42SCOPE) {
...
static int CG_CalcFov(void) {
	...
	if (cg.predictedPlayerState.pm_type == PM_INTERMISSION) {
	 ...
	 (cg.zoomval) { // zoomed by sniper / snooper
			fov_x = cg.zoomval;
			lastfov = fov_x;
