...
static void CG_DrawCursorhint(rectDef_t *rect) {
	...
	case HINT_BREAKABLE_DYNAMITE:
		...
		// We use HINT_BREAKABLE_DYNAMITE in multiplayer
...
static void CG_DrawPlayerHealth(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	...
	if (shader) {
		...
		// temp display of number of lifes left
...
qboolean CG_OwnerDrawVisible(int flags) {
	...
	if (flags & (CG_SHOW_BLUE_TEAM_HAS_REDFLAG|CG_SHOW_RED_TEAM_HAS_BLUEFLAG)) {
	 ...
	 (flags & CG_SHOW_RED_TEAM_HAS_BLUEFLAG && (cgs.blueflag == FLAG_TAKEN || cgs.flagStatus == FLAG_TAKEN_BLUE)) {
			return qtrue;
		}
...
// added drawWeaponPercent in multiplayer
	if (cgs.gametype >= GT_WOLF) {
	 ...
	 (cg.snap->ps.stats[STAT_PLAYER_CLASS] == PC_LT) {
