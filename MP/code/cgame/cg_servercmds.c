...
static void CG_ParseScores(void) {
	...
	for (i = 0; i < cg.numScores; i++) {
		...
		// the following parameters are not sent by server
		/*
...
*/
	...
	if (CG_GetVoiceChat(voiceChatList, cmd, &snd, &sprite, &chat)) {
		if (mode == SAY_TEAM || !cg_teamChatsOnly.integer) {
			...
			if (mode == SAY_TELL) {
				Com_sprintf(vchat.message, sizeof(vchat.message), "[%s]%c%c[%s]: %c%c%s", ci->name, Q_COLOR_ESCAPE, COLOR_YELLOW, CG_TranslateString(loc), Q_COLOR_ESCAPE, color, CG_TranslateString(chat));
...
static void CG_ServerCommand(void) {
	...
	if (!strcmp(cmd, "print")) {
	 ...
	 (!Q_stricmpn(cmd, "vote passed", 11) || !Q_stricmpn(cmd, "team vote passed", 16)) {
