...
void ClientRespawn(gentity_t *ent) {
	...
	// if single player, reload the last saved game for this player
	if (g_gametype.integer == GT_SINGLE_PLAYER) {
	...
	// Already handled in 'limbo()'
	if (g_gametype.integer < GT_WOLF) {
		CopyToBodyQue(ent);
	}

	ClientSpawn(ent, qfalse);
	// Add back if we decide to have a spawn effect
...
// client->ps.powerups[PW_INVULNERABLE] = level.time + 3000; // JPW NERVE some time to find cover
	...
	if (g_knifeonly.integer != 1) {
		...
		if (pc == PC_SOLDIER || pc == PC_LT) {
			// if LT is selected but illegal weapon, set to team - specific SMG
	...
	// medics on each team make cumulative health bonus -- this gets overridden for "revived" players
...
static void ClientCleanName(const char *in, char *out, int outSize) {
	...
	spaces++;
	 ...
	 (outpos > 0 && out[outpos - 1] == Q_COLOR_ESCAPE) {
...
// ---- (SA) added this for head separation
	...
	// Forcibly set both model and skin for multiplayer.
	if (g_gametype.integer >= GT_WOLF) {
	...
	// team`
	// Already took care of models and skins above
	if (g_gametype.integer < GT_WOLF) {
		// ---- (SA) added this for head separation
		// set head
		if (g_forceModel.integer) {
...
// ---- (SA) modified these for head separation
	...
	if (ent->r.svFlags & SVF_BOT) {
		s = va("n\\%s\\t\\%i\\model\\%s\\head\\%s\\c1\\%s\\hc\\%i\\w\\%i\\l\\%i\\skill\\%s", client->pers.netname, client->sess.sessionTeam, model, head, c1, client->pers.maxHealth, client->sess.wins, client->sess.losses, Info_ValueForKey(userinfo, "skill"));
	} else {
		s = va("n\\%s\\t\\%i\\model\\%s\\head\\%s\\c1\\%s\\hc\\%i\\w\\%i\\l\\%i", client->pers.netname, client->sess.sessionTeam, model, head, c1, client->pers.maxHealth, client->sess.wins, client->sess.losses);
	}
...
void ClientSpawn(gentity_t *ent, qboolean revived) {
	...
	if (revived) {
	 ...
	 (g_gametype.integer >= GT_TEAM) {
			spawnPoint = SelectCTFSpawnPoint(
				client->sess.sessionTeam, client->pers.teamState.state, spawn_origin, spawn_angles, client->sess.spawnObjectiveIndex);
		} else {
			do {
	...
	// AI should be clipped by monsterclip brushes
	if (ent->r.svFlags & SVF_CASTAI) {
	...
	if (g_gametype.integer >= GT_WOLF) {
		...
		// End Xian
		SetWolfSpawnWeapons(client); // increases stats[STAT_MAX_HEALTH] based on #of medics in game
	}
	// dhm - end
