...
char *AIFunc_LoperAttack2(cast_state_t *cs) {
	...
	if ((cs->weaponFireTimes[WP_MONSTER_ATTACK2] < level.time - 100) && (cs->bs->cur_ps.groundEntityNum == ENTITYNUM_NONE)) {
		// ready to inflict damage?
		if (cs->thinkFuncChangeTime < level.time - LOPER_LEAP_DELAY) {
			// check for damage
			if (// (tr = CheckMeleeAttack(&g_entities[cs->entityNum], LOPER_LEAP_RANGE, qtrue)) && (G_RadiusDamage(cs->bs->origin, ent, LOPER_LEAP_DAMAGE, LOPER_LEAP_RANGE, ent, MOD_LOPER_LEAP))) {
				// draw the client - side lightning effect
				ent->client->ps.eFlags |= EF_MONSTER_EFFECT;
				// do the damage
				// G_Damage(&g_entities[tr->entityNum], ent, ent, vec3_origin, tr->endpos, // LOPER_LEAP_DAMAGE, 0, MOD_LOPER_LEAP);
...
char *AIFunc_LoperAttack2Start(cast_state_t *cs) {
	...
	ent->client->ps.legsAnim = ((ent->client->ps.legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT)|LOPER_LEAP_ANIM;
...
char *AIFunc_LoperAttack3Start(cast_state_t *cs) {
	...
	AICast_AimAtEnemy(cs);
	// play the animation
	ent->client->ps.legsAnim = ((ent->client->ps.legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT)|LOPER_GROUND_ANIM;
...
int warriorHitTimes[NUM_WARRIOR_ANIMS][3] = { // up to three hits per attack
	{ANIMLENGTH(10, 20), -1}, {ANIMLENGTH(15, 20), -1}, {ANIMLENGTH(18, 20), -1}, {ANIMLENGTH(15, 20), -1}, {ANIMLENGTH(14, 20), -1},
};
