...
// and move them to a separate header file for easy modification
...
int AICast_ScanForEnemies(cast_state_t *cs, int *enemies) {
	...
	for (i = 0; i < g_maxclients.integer; i++) {
	 ...
	 (!enemyCount && (g_entities[i].health > 0) && AICast_QueryEnemy(cs, i) && (cs->vislist[i].flags & AIVIS_PROCESS_SIGHTING)) {
						enemies[queryCount] = i;
						queryCount++;
						friendlyAlertCount = 0;
			
		} else {
	 (!queryCount && !enemyCount && (cs->vislist[i].flags & AIVIS_INSPECT)) {
	...
	if (friendlyAlertCount) {
		...
		if (cs->aiState < AISTATE_COMBAT) {
			// go into alert mode, and return this entity so we can inspect it or something, // but let dynamic AI sort out what it wants to do
...
AICast_CheckAttack
  ...
  optimization, uses the cache to avoid possible duplicate calls with same world parameters
=======================================================================================================================================
*/
qboolean AICast_CheckAttack(cast_state_t *cs, int enemy, qboolean allowHitWorld) {

	if (cs->bs) {
		if ((cs->checkAttackCache.time == level.time) && (cs->checkAttackCache.enemy == enemy) && (cs->checkAttackCache.weapon == cs->bs->weaponnum) && (cs->checkAttackCache.allowHitWorld == allowHitWorld)) {
...
void AICast_ChooseWeapon(cast_state_t *cs, qboolean battleFunc) {
	...
	if (cs->bs->cur_ps.weaponstate == WEAPON_RAISING || cs->bs->cur_ps.weaponstate == WEAPON_DROPPING) {
		return;
	}
...
bot_moveresult_t AICast_CombatMove(cast_state_t *cs, int tfl) {
	...
	if (cs->combatGoalTime > level.time) {
	 ...
	 (!AICast_CheckAttack(cs, cs->bs->enemy, qfalse) && AICast_VisibleFromPos(cs->vislist[cs->bs->enemy].visible_pos, cs->bs->enemy, cs->bs->origin, cs->entityNum, qfalse)) {
			cs->combatGoalTime = 0;
			cs->combatSpotAttackCount = cs->startAttackCount;
		}
		...
		if (((cs->attributes[CAMPER] < random()) && (cs->takeCoverTime < level.time) && (cs->combatSpotAttackCount < cs->startAttackCount) && (cs->combatSpotDelayTime < level.time))) {
			if ((cs->attributes[TACTICAL] > 0.3 + random() * 0.5) && trap_AAS_RT_GetHidePos(cs->bs->origin, cs->bs->entitynum, cs->bs->areanum, cs->vislist[cs->bs->enemy].visible_pos, bs->enemy, BotPointAreaNum(cs->vislist[cs->bs->enemy].visible_pos), cs->combatGoalOrigin)) {
...
qboolean AICast_AimAtEnemy(cast_state_t *cs) {
	...
	if (vis->visible_timestamp < vis->lastcheck_timestamp) {
		...
		// we can see them, if this weapon isn't a direct - hit weapon(bullets), // then predict where they are going to be
	...
	if (cs->bs->weaponnum == WP_GRENADE_LAUNCHER || cs->bs->weaponnum == WP_GRENADE_PINEAPPLE) {
	 ...
	 (Distance(start, bestorigin) > 400) { // aim up a bit for distance
...
void AICast_ProcessAttack(cast_state_t *cs) {
	...
	if (!AICast_CanMoveWhileFiringWeapon(cs->bs->weaponnum)) {
		...
		if ((cs->castScriptStatus.scriptNoMoveTime < level.time) && ((bi.actionflags & ACTION_MOVEFORWARD) || (bi.actionflags & ACTION_MOVEBACK) || (bi.actionflags & ACTION_MOVELEFT) || (bi.actionflags & ACTION_MOVERIGHT) || (bi.speed))) {
			return;
		}
	}
...
qboolean AICast_GetTakeCoverPos(cast_state_t *cs, int enemyNum, vec3_t enemyPos, vec3_t returnPos) {
	...
	// can we just crouch?
	if ((cs->bs->attackcrouch_time < trap_AAS_Time()) && (enemyNum < aicast_maxclients) && AICast_CheckAttackAtPos(cs->entityNum, enemyNum, cs->bs->origin, qfalse, qfalse) && !AICast_CheckAttackAtPos(cs->entityNum, enemyNum, cs->bs->origin, qtrue, qfalse)) {
	...
	if (cs->dangerEntity == enemyNum && cs->dangerEntityValidTime > level.time) {
	 ...
	 (AICast_GetAvoid (cs, NULL, cs->takeCoverPos, qtrue, cs->dangerEntity)) {
...
qboolean AICast_WantToRetreat(cast_state_t *cs) {
	...
	if (cs->leaderNum < 0) {
		if ((cs->attributes[TACTICAL] > 0.11 + random() * 0.5) && ((cs->bs->cur_ps.weaponTime > 500) || ((cs->takeCoverTime < level.time - 100) && (AICast_WantsToTakeCover(cs, qtrue))))) {
			return qtrue;
		}
	}
...
void AICast_ProcessBullet(gentity_t *attacker, vec3_t start, vec3_t end) {
	...
	// AI should hear this pass by them really closely, or hitting a wall close by
