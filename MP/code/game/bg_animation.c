...
typedef enum {
	ANIM_CONDTYPE_BITFLAGS, ANIM_CONDTYPE_VALUE,
	...
	if (token && token[0]) {
		if (animModelInfo->version < 2 || !Q_stricmp(token, "HEADFRAMES")) {
			...
			for (i = 0; i < MAX_HEAD_ANIMS; i++) {
				...
				// modify according to last frame of the main animations, since the head is totally separate
...
typedef enum {
	PARSEMODE_DEFINES, PARSEMODE_ANIMATION, PARSEMODE_CANNED_ANIMATIONS, PARSEMODE_STATECHANGES, PARSEMODE_EVENTS
...
void BG_AnimParseAnimScript(animModelInfo_t *modelInfo, animScriptData_t *scriptData, int client, char *filename, char *input) {
	...
	while (1) {
	 ...
	 (indentLevel == 0 && (indexes[indentLevel] < 0)) {
...
// ---- (SA) // RF mod
	 ...
	 ((indentLevel == 1) && (indexes[indentLevel] < 0)) {
	 ...
	 (parseMode == PARSEMODE_CANNED_ANIMATIONS) {
	 ...
	 ((indentLevel == 2) && (indexes[indentLevel] < 0)) {
...
int BG_PlayAnim(playerState_t *ps, int animNum, animBodyPart_t bodyPart, int forceDuration, qboolean setTimer, qboolean isContinue, qboolean force) {
	...
	case ANIM_BP_LEGS:
	 ...
	 (setTimer && modelInfo->animations[animNum].loopFrames) {
				ps->legsTimer = duration;
			}
		}
