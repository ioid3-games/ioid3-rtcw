...
#define MG42_DAMAGE_MP      20         // testing with slowed firing rate // JPW NERVE was 30
...
void mg42_spawn(gentity_t *ent) {
	...
	if (g_knifeonly.integer != 1) {
		...
		// Use a different model in multiplayer
...
void miscGunnerThink(gentity_t *ent) {
	...
	if (ent->enemy) {
	 ...
	 (AICast_VisibleFromPos(ent->r.currentOrigin, ent->s.number, ent->enemy->r.currentOrigin, ent->enemy->s.number, qfalse)) {
			fire = qtrue;
		}
	}
