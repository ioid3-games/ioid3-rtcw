...
typedef enum {
	MOVER_POS1, MOVER_POS2, MOVER_POS3, MOVER_1TO2, MOVER_2TO1, // JOSEPH 1 - 26 - 00
	MOVER_2TO3, MOVER_3TO2, // END JOSEPH

	// Rafael
	MOVER_POS1ROTATE, MOVER_POS2ROTATE, MOVER_1TO2ROTATE, MOVER_2TO1ROTATE
} moverState_t;
...
struct gentity_s {
	...
	qboolean physicsObject;        // if true, it can be pushed by movers and fall off edges
									// all game items are physicsObjects, float physicsBounce;           // 1.0 = continuous bounce, 0.0 = no bounce
	...
	// the above warning does not really apply to MP, but I'll follow it for good measure
...
typedef enum {
	SPECTATOR_NOT, SPECTATOR_FREE, SPECTATOR_FOLLOW, SPECTATOR_SCOREBOARD
} spectatorState_t;
...
typedef struct {
	...
	int playerItem;                // for GT_WOLF
	int playerSkin;                // for GT_WOLF
	...
	int latchPlayerItem;           // for GT_WOLF not archived
	int latchPlayerSkin;           // for GT_WOLF not archived
} clientSession_t;
...
// The number of complaints allowed before kick / ban
...
typedef enum {
	shard_glass = 0, shard_wood, shard_metal, shard_ceramic, shard_rubble
} shards_t;
