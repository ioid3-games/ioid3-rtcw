...
a separate thinking function, which may or may not pass control
...
// 	contents_mask = CONTENTS_SOLID|CONTENTS_PLAYERCLIP|CONTENTS_WATER;
	...
	while (cnt++ < 4) {
	 ...
	 (!bestdist || bestdist < trace.fraction) {
...
char *AIFunc_Idle(cast_state_t *cs) {
	...
	if (numEnemies == -1) {   // query mode
	 ...
	 (AICast_EntityVisible(AICast_GetCastState(cs->bs->enemy), cs->entityNum, qtrue) || AICast_CheckAttack(AICast_GetCastState(cs->bs->enemy), cs->entityNum, qfalse)) {
	 ...
	 (cs->leaderNum < 0) {   // we should pursue if no leader, and not wanting to hide
				return AIFunc_BattleStart(cs);
			} else {
...
*/                                                                                                                                                                                                                                                                                                                                   // if we only just recently saw them, face that direction
		if ((g_entities[cs->lastEnemy].health > 0) && cs->vislist[cs->lastEnemy].visible_timestamp > (level.time - 20000) && AICast_VisibleFromPos(cs->bs->origin, cs->entityNum, cs->vislist[cs->lastEnemy].visible_pos, cs->lastEnemy, qfalse)) {
			vec3_t dir;
...
*/
...

	{
	 ...
	 (numEnemies == -2) { // inspection
		...
		// disabled this, if we are interrupted, scripts might not work right, and anyway, this is only a bullet, not as if it's a dead guy or anything
		// else if (numEnemies == -3)	// bullet impact
		// {
		// 	if (cs->aiState < AISTATE_COMBAT) {
		// 		return AIFunc_InspectBulletImpactStart(cs);
...
char *AIFunc_ChaseGoal(cast_state_t *cs) {
	...
	// go to them
	bs = cs->bs;
	// disabled this, MIKE sees dead people
	...
	if (cs->bs->enemy >= 0) { // attack the enemy if possible
	 ...
	 (numEnemies == -2) { // inspection may be required
			char *retval;
			// TTimo gcc: suggest parentheses around assignment used as truth value
			if ((retval = AIFunc_InspectFriendlyStart(cs, enemies[0]))) {
				return retval;
	 ...
	 (numEnemies == -3) { // bullet impact
			if (cs->aiState < AISTATE_COMBAT) {
				return AIFunc_InspectBulletImpactStart(cs);
			}

		} else {
	 (numEnemies == -4) { // audible event
			if (cs->aiState < AISTATE_COMBAT) {
				return AIFunc_InspectAudibleEventStart(cs, cs->audibleEventEnt);
			}
...
char *AIFunc_BattleHunt(cast_state_t *cs) {
	...
	if (AICast_EntityVisible(cs, bs->enemy, qtrue) // take into account reaction time && AICast_CheckAttack(cs, bs->enemy, qfalse) && cs->obstructingTime < level.time) {
	 ...
	 (AICast_GotEnoughAmmoForWeapon(cs, cs->bs->weaponnum)) {
			if (numEnemies > 0) {
...
char *AIFunc_BattleChase(cast_state_t *cs) {
	...
	// Do the movement..
	// move straight to them if we can
	if (!moved && cs->leaderNum < 0 && (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE || g_entities[cs->entityNum].waterlevel > 1) && AICast_EntityVisible(cs, cs->bs->enemy, qtrue)) {
		aicast_predictmove_t move;
		vec3_t dir;
		bot_input_t bi;
		usercmd_t ucmd;
	...
	// if we are going to reach them soon, predict the attack
	{
		...
		else if ((cs->attributes[TACTICAL] > 0.85 && cs->aiFlags & AIFL_ROLL_ANIM && !client->ps.torsoTimer && !client->ps.legsTimer && cs->lastRollMove < level.time - 800 && move.numtouch == 0 && moveDist > simTime * cs->attributes[RUNNING_SPEED] * 0.8 && move.groundEntityNum == ENTITYNUM_WORLD) && (AICast_CheckAttackAtPos(cs->entityNum, cs->bs->enemy, move.endpos, cs->bs->attackcrouch_time > trap_AAS_Time(), qfalse))) {
		...
		else if ((cs->obstructingTime < level.time) && (cs->attributes[TACTICAL] > 0.1) && (AICast_VisibleFromPos(cs->vislist[cs->bs->enemy].visible_pos, cs->bs->enemy, move.endpos, cs->entityNum, qfalse))) {
			// start a crouch attack?
			// if (cs->attributes[ATTACK_CROUCH] > 0.1) {
			// 	cs->bs->attackcrouch_time = trap_AAS_Time() + 3.0;
			// else
...
char *AIFunc_AvoidDanger(cast_state_t *cs) {
	...
	if (ent->s.onFireEnd < level.time) {
		if ((danger->s.weapon == WP_GRENADE_LAUNCHER || danger->s.weapon == WP_GRENADE_PINEAPPLE) && (danger->nextthink - level.time > 1500) && (level.lastGrenadeKick < level.time - 3000) && (cs->aiFlags & AIFL_CATCH_GRENADE) && !(danger->flags & FL_AI_GRENADE_KICK)) {
			...
			if (!AICast_SameTeam(cs, danger->r.ownerNum)) {
				if (G_PredictMissile(danger, danger->nextthink - level.time, cs->takeCoverPos, qfalse)) {
					...
					if (!tr.startsolid && (tr.fraction < 1.0) && VectorDistance(cs->bs->origin, cs->takeCoverPos) < cs->attributes[RUNNING_SPEED] * 0.0004 * (danger->nextthink - level.time - 2000)) {
			...
			if (VectorLength(danger->s.pos.trDelta) < 10 && VectorDistance(danger->r.currentOrigin, cs->bs->origin) < 128 && (level.lastGrenadeKick < level.time - 3000) && (cs->aiFlags & AIFL_CATCH_GRENADE)) {
				vec3_t vec;
...
// DISABLED: doesn't work well with new AI system
	...
	if (VectorLength(cs->takeCoverPos) > 1 && dist > 8) {
		...
		else if ((cs->aiFlags & AIFL_DIVE_ANIM && !client->ps.torsoTimer && cs->castScriptStatus.castScriptEventIndex < 0 && cs->lastRollMove < level.time - 800 && move.numtouch == 0 && moveDist > 64 && move.groundEntityNum == ENTITYNUM_WORLD) && (shouldAttack && !AICast_VisibleFromPos(g_entities[cs->bs->enemy].s.pos.trBase, cs->bs->enemy, move.endpos, cs->entityNum, qfalse))) {
		...
		if (!(cs->aiFlags & AIFL_MISCFLAG1) && !AICast_VisibleFromPos(cs->vislist[cs->bs->enemy].real_visible_pos, cs->bs->enemy, move.endpos, cs->entityNum, qfalse) && !AICast_VisibleFromPos(cs->vislist[cs->bs->enemy].real_visible_pos, cs->bs->enemy, cs->bs->origin, cs->entityNum, qfalse)) {
	...
	if (shouldAttack) {
		...
		if (!move.numtouch && (dist > 128) && cs->attributes[TACTICAL] > 0.4 && cs->attributes[ATTACK_CROUCH] > 0.1 && (cs->bs->attackcrouch_time >= trap_AAS_Time())) {
			cs->bs->attackcrouch_time = trap_AAS_Time() + 1;
		}
		// attack the enemy if possible
		AICast_ProcessAttack(cs);
	 ...
	 (!cs->crouchHideFlag) { // no enemy, and no need to crouch, so stop crouching
			// if (cs->bs->attackcrouch_time > trap_AAS_Time() + 1) {
			// 	cs->bs->attackcrouch_time = trap_AAS_Time() + 1;
			//}
		}
...
char *AIFunc_GrenadeFlush(cast_state_t *cs) {
	...
	if (AICast_CheckAttack(cs, bs->enemy, qfalse) && cs->obstructingTime < level.time) { // give us some time to throw the grenade, otherwise go back to attack state
		if ((cs->grenadeFlushEndTime > 0 && cs->grenadeFlushEndTime < level.time)) {
			// G_Printf("aborting, enemy is attackable\n");
			return AIFunc_BattleStart(cs);

		} else {
	 (cs->grenadeFlushEndTime < 0) {
	...
	if (cs->vislist[bs->enemy].lastcheck_timestamp > cs->vislist[bs->enemy].real_visible_timestamp || dist > 128) {
		// 
		// go to them
		// 
		if (followent->client && followent->health <= 0) {
...
char *AIFunc_BattleMG42(cast_state_t *cs) {
	...
	if (bs->enemy >= 0) {
	 ...
	 (cs->vislist[bs->enemy].visible_timestamp && cs->vislist[bs->enemy].visible_timestamp > (level.time - 5000)) {
	...
	if (bs->enemy < 0 ||
			!AICast_CheckAttack(cs, bs->enemy, qfalse) || (fabs(AngleDifference(angles[YAW], mg42->s.angles[YAW])) > mg42->harc) || (angles[PITCH] < 0 && angles[PITCH] + 5 < -mg42->varc) || (angles[PITCH] > 0 && angles[PITCH] - 5 > 5.0)) {
		qboolean shouldAttack;
		...
		if (numEnemies > 0) {
			...
			for (i = 0; i < numEnemies; i++) {
				if (!cs->vislist[enemies[i]].real_visible_timestamp || (cs->vislist[enemies[i]].real_visible_timestamp < level.time - 2000)) {
	 ...
	 (AICast_CheckAttack(cs, enemies[i], qtrue)) {
		...
		if (!shouldAttack) {
			// keep firing at anything behind solids, in case they find a position where they can shoot us, but our checkattack() doesn't find a clear shot
			if (bs->enemy < 0 || !AICast_CheckAttack(cs, bs->enemy, qtrue) || (!cs->vislist[bs->enemy].real_visible_timestamp || (cs->vislist[bs->enemy].real_visible_timestamp < level.time - 2000))) {
...
char *AIFunc_Battle(cast_state_t *cs) {
	...
	if ((cs->bs->cur_ps.weaponTime < 100) // if reloading, don't chase until ready && (cs->castScriptStatus.scriptNoMoveTime < level.time) && (!AICast_EntityVisible(cs, bs->enemy, qtrue) || !AICast_CheckAttack(cs, bs->enemy, qfalse))) {
		...
		if (cs->attributes[TACTICAL] > 0.3
				...
				&& cs->attributes[AGGRESSION] < (random() + 0.5 * cs->attributes[TACTICAL]) && (cs->takeCoverTime < level.time) && AICast_GetTakeCoverPos(cs, cs->bs->enemy, cs->vislist[cs->bs->enemy].real_visible_pos, cs->takeCoverPos)) {
			// start taking cover
		...
		// Take Cover?
		if (AICast_WantsToTakeCover(cs, qfalse) && (cs->takeCoverTime < level.time) && AICast_GetTakeCoverPos(cs, cs->bs->enemy, cs->vislist[cs->bs->enemy].real_visible_pos, cs->takeCoverPos)) {
			// go to a position that cannot be seen from the last place we saw the enemy, and wait there for some time
	...
	} else if ((cs->obstructingTime < (level.time - 500 + rand() % 300)) && (Distance(cs->bs->origin, cs->vislist[cs->bs->enemy].real_visible_pos) < 100)) {
	...
	*/
	...
	// Lob a Grenade?
	// if we haven't thrown a grenade in a bit, go into "grenade flush mode"
	if ((lastGrenadeFlush > level.time || lastGrenadeFlush < level.time - 7000) && (cs->castScriptStatus.castScriptEventIndex < 0) && (cs->startGrenadeFlushTime < level.time - 3000) && (COM_BitCheck(cs->bs->cur_ps.weapons, WP_GRENADE_LAUNCHER)) && (AICast_GotEnoughAmmoForWeapon(cs, WP_GRENADE_LAUNCHER)) && (cs->weaponFireTimes[WP_GRENADE_LAUNCHER] < level.time - (int)(aicast_skillscale * 3000)) && ((cs->bs->weaponnum == WP_GRENADE_LAUNCHER) || !(cs->castScriptStatus.scriptFlags & SFL_NOCHANGEWEAPON)) && (Distance(cs->bs->origin, cs->vislist[cs->bs->enemy].real_visible_pos) > 100) && (Distance(cs->bs->origin, cs->vislist[cs->bs->enemy].real_visible_pos) < 2000)) {
		// try and flush them out with a grenade
		// G_Printf("pineapple?\n");
		return AIFunc_GrenadeFlushStart(cs);
	}
	// Dodge enemy aim?
	if ((cs->attributes[AGGRESSION] < 1.0) && (ent->client->ps.weapon) && (ent->client->ps.groundEntityNum == ENTITYNUM_WORLD) && (!cs->lastRollMove || cs->lastRollMove < level.time - 4000) && (cs->attributes[TACTICAL] > 0.5) && (cs->aiFlags & AIFL_ROLL_ANIM) && (VectorLength(cs->bs->cur_ps.velocity) < 1)) {
...
recheck:
	...
	if (fabs(AngleDifference(cs->bs->ideal_viewangles[YAW], cs->bs->viewangles[YAW])) < 10) {
	 ...
	 ((cs->bs->weaponnum == WP_MONSTER_ATTACK2) && cs->aifuncAttack2) {
			if (AICast_CheckAttack(cs, cs->bs->enemy, qfalse)) {
	 ...
	 ((cs->bs->weaponnum == WP_MONSTER_ATTACK3) && cs->aifuncAttack3) {
			if (AICast_CheckAttack(cs, cs->bs->enemy, qfalse)) {
