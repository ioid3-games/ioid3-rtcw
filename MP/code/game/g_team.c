...
// 	return; // can't find attacker's flag
		...
		if ((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS ||
			   ...
			   CanDamage(flag, targ->client->ps.origin) || CanDamage(flag, attacker->client->ps.origin)) && attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
...
// JPW NERVE
		if (g_gametype.integer >= GT_WOLF) {
			...
			// Call trigger function in the 'game_manager' entity script
			gm = G_Find(NULL, FOFS(scriptName), "game_manager");
...
gentity_t *SelectRandomTeamSpawnPoint(int teamstate, team_t team, int spawnObjective) {
	...
	if (teamstate == TEAM_BEGIN) {
		// Don't check if spawn is active initially
		initialSpawn = qtrue;
...
// JPW NERVE
		if (g_gametype.integer >= GT_WOLF) {
			...
			if ((initialSpawn && (spot->spawnflags & 4)) || (!initialSpawn && !(spot->spawnflags & 2))) {
				continue;
			}
		}
		spots[count] = spot;
...
void TeamplayInfoMessage(gentity_t *ent) {
	...
	for (i = 0, cnt = 0; i < level.numConnectedClients && cnt < TEAM_MAXOVERLAY; i++) {
		player = g_entities + level.sortedClients[i];
			...
			Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i", level.sortedClients[i], player->client->pers.teamState.location, h, player->s.powerups, player->client->ps.stats[STAT_PLAYER_CLASS]);
			j = strlen(entry);
...
void SP_team_WOLF_objective(gentity_t *ent) {
	...
	// Give the script time to remove this ent if necessary
...
// JPW NERVE
	...
	if (self->count == TEAM_RED) {
	 ...
	 (self->s.frame == WCP_ANIM_AMERICAN_RAISED && !(self->spawnflags & ALLIED_ONLY)) {
			self->s.frame = WCP_ANIM_AMERICAN_TO_AXIS;
	 ...
	 (self->s.frame == WCP_ANIM_AXIS_RAISED && !(self->spawnflags & AXIS_ONLY)) {
			self->s.frame = WCP_ANIM_AXIS_TO_AMERICAN;
	...
	if (self->target) {
	 ...
	 (!strcmp(ent->classname, "team_CTF_bluespawn")) {
					ent->spawnflags &= ~2;
	 ...
	 (!strcmp(ent->classname, "team_CTF_redspawn")) {
					ent->spawnflags &= ~2;
