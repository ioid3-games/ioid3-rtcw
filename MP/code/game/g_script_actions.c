...
qboolean G_ScriptAction_GotoMarker(gentity_t *ent, char *params) {
	...
	if (!params || ent->scriptStatus.scriptStackChangeTime < level.time) {        // we are waiting for it to reach destination
	 ...
	 (!Q_stricmp(token, "deccel")) {
					trType = TR_DECCELERATE;
		
		} else {
	 (!Q_stricmp(token, "wait")) {
					wait = qtrue;
		
		} else {
	 (!Q_stricmp(token, "turntotarget")) {
					turntotarget = qtrue;
				}
			}
		}
		...
		if (ent->s.eType == ET_MOVER) {
			...
			if (turntotarget) {
				...
				for (i = 0; i < 3; i++) {
					diff[i] = AngleDifference(angles[i], ent->s.angles[i])' || (diff[i] > 180)
						diff[i] -= 360' || (diff[i] < -180)
						diff[i] += 360;
				}
...
G_ScriptAction_PlaySound
  ...
  Use the optional LOOPING parameter to attach the sound to the entities looping channel.
=======================================================================================================================================
*/
