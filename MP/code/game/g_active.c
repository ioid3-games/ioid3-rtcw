...
void SpectatorThink(gentity_t *ent, usercmd_t *ucmd) {
	...
	if ((client->buttons & BUTTON_ATTACK) && !(client->oldbuttons & BUTTON_ATTACK)) {
		...
		(client->sess.spectatorState == SPECTATOR_FOLLOW) && (client->buttons & BUTTON_ACTIVATE) && !(client->oldbuttons & BUTTON_ACTIVATE)) {
...
qboolean ClientInactivityTimer(gclient_t *client) {
	...
	} else if (client->pers.cmd.forwardmove || client->pers.cmd.rightmove || client->pers.cmd.upmove || (client->pers.cmd.wbuttons & WBUTTON_ATTACK2) || (client->pers.cmd.buttons & BUTTON_ATTACK)) {
...
void ClientIntermissionThink(gclient_t *client) {
	...
	if ((client->buttons & (BUTTON_ATTACK|BUTTON_USE_HOLDABLE) & (client->oldbuttons ^ client->buttons)) || (client->wbuttons & WBUTTON_ATTACK2 & (client->oldwbuttons ^ client->wbuttons))) {
...
// update counter for capture & hold display
...
// 	client->latched_wbuttons |= client->wbuttons & ~client->oldwbuttons;	// FIXME:? (SA) MP method
	...
	if (client->ps.stats[STAT_HEALTH] <= 0) {
		...
		if (level.time > client->respawnTime) {
			...
			if ((g_gametype.integer != GT_SINGLE_PLAYER) && (g_forcerespawn.integer > 0) && ((level.time - client->respawnTime) > g_forcerespawn.integer * 1000) && (!(ent->client->ps.pm_flags & PMF_LIMBO))) { // JPW NERVE
				// JPW NERVE
				if (g_gametype.integer >= GT_WOLF) {
					limbo(ent, qtrue);
				} else {
			...
			// Single player game respawns immediately as before, // 				  but in multiplayer, require button press before respawn
			if (g_gametype.integer == GT_SINGLE_PLAYER) {
				ClientRespawn(ent);
			}
...
// After reviving a player, their contents stay CONTENTS_CORPSE until it is determined
...
void WolfReviveBbox(gentity_t *self) {
	...
	for (i = 0; i < num; i++) {
	 ...
	 (hit->r.contents & (CONTENTS_SOLID|CONTENTS_BODY|CONTENTS_PLAYERCLIP)) {
