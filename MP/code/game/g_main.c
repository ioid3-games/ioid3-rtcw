...
cvarTable_t gameCvarTable[] = {
	...
	// noset vars
	{NULL, "gamename", GAMEVERSION, CVAR_SERVERINFO|CVAR_ROM, 0, qfalse}, {NULL, "gamedate", PRODUCT_DATE, CVAR_ROM, 0, qfalse},
	{&g_restarted, "g_restarted", "0", CVAR_ROM, 0, qfalse},
	// latched vars
	// default to GT_WOLF
	{&g_gametype, "g_gametype", "5", CVAR_SERVERINFO|CVAR_LATCH, 0, qfalse},
	// Rafael gameskill
	{&g_gameskill, "g_gameskill", "3", CVAR_SERVERINFO|CVAR_LATCH, 0, qfalse}, // done
...
// JPW NERVE multiplayer stuffs
	...
	{&g_swapteams, "g_swapteams", "0", CVAR_ROM, 0, qfalse}, // - NERVE - SMF

	{&g_logfile, "g_log", "games.log", CVAR_ARCHIVE, 0, qfalse},
	{&g_logfileSync, "g_logsync", "0", CVAR_ARCHIVE, 0, qfalse},
	{&g_password, "g_password", "", CVAR_USERINFO, 0, qfalse},
	{&g_banIPs, "g_banIPs", "", CVAR_ARCHIVE, 0, qfalse}, // show_bug.cgi?id=500
	...
	{&mod_url, "mod_url", "", CVAR_SERVERINFO|CVAR_ROM, 0, qfalse}, // configured by the server admin, points to the web pages for the server
...
// 	SnapVector(offset);
	...
	// Ignore trigger_objective_info
	if ((ps->stats[STAT_PLAYER_CLASS] != PC_MEDIC && traceEnt->client) || !(strcmp(traceEnt->classname, "trigger_objective_info")) || !(strcmp(traceEnt->classname, "trigger_multiple"))) {
		...
		if (checkEnt) {
			if (checkEnt->s.eType == ET_GENERAL || checkEnt->s.eType == ET_MG42_BARREL) {
				...
				if ((// general mg42 hint conditions
						 !Q_stricmp(traceEnt->classname, "misc_mg42")) && (ps->weapon != WP_SNIPERRIFLE) && // JPW NERVE no hint if you're scoped in sniperwise
					// ATVI #470
					// mount hint conditions only, if busted MG42, no check
					(
						(traceEnt->health < 255) || (
							 !(ent->client->ps.pm_flags & PMF_DUCKED) && (traceEnt->r.currentOrigin[2] - ent->r.currentOrigin[2] < 40) && (traceEnt->r.currentOrigin[2] - ent->r.currentOrigin[2] > 0) && !infront(traceEnt, ent)
					)
				)
				) {
					hintDist = CH_ACTIVATE_DIST;
...
// busted MG42 shouldn't show a use hint by default
...
// 				} else {
				if ((checkEnt->health > 0) || (checkEnt->spawnflags & 64)) { // JPW NERVE they are now if it's dynamite			// 0 health explosives are not breakable
					...
					// Show different hint if it can only be destroyed with dynamite
...
// we're in a trigger_objective_info field with at least one owner, so use this one and bail
						...
						if ((hit) && (((ent->client->sess.sessionTeam == TEAM_RED) && (hit->spawnflags & ALLIED_OBJECTIVE)) || ((ent->client->sess.sessionTeam == TEAM_BLUE) && (hit->spawnflags & AXIS_OBJECTIVE)))
						) {
...
// JPW NERVE changed this to be more specific
	 ...
	 (!Q_stricmp(traceEnt->classname, "team_CTF_blueflag") && ent->client->sess.sessionTeam == TEAM_RED) {
						hintType = HINT_POWERUP;
					}
	 ...
	 (!Q_stricmp(checkEnt->classname, "func_door"))       {
	 ...
	 (!Q_stricmp(checkEnt->classname, "func_button"))       {
					hintDist = CH_ACTIVATE_DIST;
...
void G_ShutdownGame(int restart) {
	...
	// shutdown the Botlib, so weapons and things get reset upon doing a "map xxx" command
	if (trap_Cvar_VariableIntegerValue("bot_enable")) {
		int i;
		// kill AI cast's
...
void CheckIntermissionExit(void) {
	...
	// Flat 10 second timer until exit
	if (g_gametype.integer >= GT_WOLF) {
...
// in CPH, check final capture / hold times and adjust winner
				// 0 == axis, 1 == allied
	...
	if (g_gametype.integer >= GT_WOLF && (g_maxlives.integer > 0 || g_axismaxlives.integer > 0 || g_alliedmaxlives.integer > 0)) {
	 ...
	 (level.numFinalDead[1] >= level.numteamVotingClients[1] && level.numteamVotingClients[1] > 0) {
			trap_GetConfigstring(CS_MULTI_MAPWINNER, cs, sizeof(cs));
...
// JPW NERVE
	 ...
	 (level.voteNo >= level.numVotingClients / 2) {
...
void G_RunFrame(int levelTime) {
	...
	if (!level.latchGametype && g_gamestate.integer == GS_PLAYING && ((g_gametype.integer == GT_WOLF && (worldspawnflags & NO_GT_WOLF)) || (g_gametype.integer == GT_WOLF_STOPWATCH && (worldspawnflags & NO_STOPWATCH)) || ((g_gametype.integer == GT_WOLF_CP || g_gametype.integer == GT_WOLF_CPH) && (worldspawnflags & NO_CHECKPOINT))) // JPW NERVE added CPH
	) {
	...
	for (i = 0; i < level.num_entities; i++, ent++) {
		...
		// if this entity is attached to a parent, move it around with it, so the server thinks it's at least close to where the client will view it
		if (ent->tagParent) {
			vec3_t org;
