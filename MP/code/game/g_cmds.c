...
void DeathmatchScoreboardMessage(gentity_t *ent) {
	...
	for (i = 0; i < numSorted; i++) {
		...
		Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i %i %i", level.sortedClients[i], cl->ps.persistant[PERS_SCORE], ping, (level.time - cl->pers.enterTime) / 60000, scoreFlags, g_entities[level.sortedClients[i]].s.powerups, playerClass, respawnsLeft);
		j = strlen(entry);
...
void SetTeam(gentity_t *ent, char *s) {
	 ...
	 (!Q_stricmp(s, "blue") || !Q_stricmp(s, "b")) {
		...
		if (g_teamForceBalance.integer) {
			...
			if (team == TEAM_RED && counts[TEAM_RED] - counts[TEAM_BLUE] >= 1) {
				trap_SendServerCommand(clientNum, "cp \"The Axis has too many players.\n\"");
				return; // ignore the request
			}

			if (team == TEAM_BLUE && counts[TEAM_BLUE] - counts[TEAM_RED] >= 1) {
				trap_SendServerCommand(clientNum, "cp \"The Allies have too many players.\n\"");
				return; // ignore the request
			}
	...
	// Force players to wait 30 seconds before they can join a new team.
	if (g_gametype.integer >= GT_WOLF && team != oldTeam && level.warmupTime == 0 && !client->pers.initialSpawn
		...
		trap_SendServerCommand(ent - g_entities, va("cp \"^3You must wait %i seconds before joining ^ 3a new team.\n\" 3", (int)(30 - ((level.time - client->pers.enterTime) / 1000))));
		return;
	}
	// dhm
...
void G_SayTo(gentity_t *ent, gentity_t *other, int mode, int color, const char *name, const char *message, qboolean localize) { // removed static so it would link
	...
	// NERVE - SMF
	if (mode == SAY_LIMBO) {
		trap_SendServerCommand(other - g_entities, va("%s \"%s%c%c%s\"", "lchat", name, Q_COLOR_ESCAPE, color, message));
	}
	// - NERVE - SMF
	else {
		trap_SendServerCommand(other - g_entities, va("%s \"%s%c%c%s\" %i", mode == SAY_TEAM ? "tchat" : "chat", name, Q_COLOR_ESCAPE, color, message, localize));
	}
}
...
static void G_VoiceTo(gentity_t *ent, gentity_t *other, int mode, const char *id, qboolean voiceonly) {
	...
	trap_SendServerCommand(other - g_entities, va("%s %d %d %d %s %i %i %i", cmd, voiceonly, ent->s.number, color, id, (int)ent->s.pos.trBase[0], (int)ent->s.pos.trBase[1], (int)ent->s.pos.trBase[2]));
}
...
static char *gc_orders[] = {
	"hold your position", "hold this position", "come here", "cover me", "guard location", "search and destroy", "report"
};
...
static const char *gameNames[] = {
	"Free For All", "Tournament", "Single Player", "Team Deathmatch", "Capture the Flag", "Wolf Multiplayer", "Wolf Stopwatch", "Wolf Checkpoint"
};
...
void Cmd_Activate_f(gentity_t *ent) {
	...
	if (traceEnt->classname) {
	 ...
	 (((Q_stricmp(traceEnt->classname, "func_door") == 0) || (Q_stricmp(traceEnt->classname, "func_door_rotating") == 0)))         {
			if (walking) {
	 ...
	 ((Q_stricmp(traceEnt->classname, "team_WOLF_checkpoint") == 0)) {
	 ...
	 (!Q_stricmp(traceEnt->classname, "script_mover")) {
			G_Script_ScriptEvent(traceEnt, "activate", ent->aiName);

		} else {
	 (!Q_stricmp(traceEnt->classname, "props_footlocker")) {
			traceEnt->use(traceEnt, ent, ent);
		}
	}
...
int Cmd_WolfKick_f(gentity_t *ent) {
	...
	if (!ent->melee) { // because we dont want you to open a door with a prop
	 ...
	 (!Q_stricmp(traceEnt->classname, "props_flippy_table") && traceEnt->use) {
			traceEnt->use(traceEnt, ent, ent);
		}
	}
...
// 	}
	...
	case CLDMG_TESLA:
		...
		if ((ent->aiCharacter == AICHAR_PROTOSOLDIER) || (ent->aiCharacter == AICHAR_SUPERSOLDIER) || (ent->aiCharacter == AICHAR_LOPER) || (ent->aiCharacter >= AICHAR_STIMSOLDIER1 && ent->aiCharacter <= AICHAR_STIMSOLDIER3)) {
			break;
		}
...
// 				break;
		...
		if (ent->takedamage && !AICast_NoFlameDamage(ent->s.number)) {
			...
			int damage = 5;
			// only do damage once they start burning
