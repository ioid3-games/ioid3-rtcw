...
// note to everyone: static functions can cause problems with the savegame code, so avoid // using them unless necessary. if that function is then assigned to an entity, client or cast field, 
...
void target_script_trigger_use(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	// In multiplayer, we use the brush scripting only
...
void target_rumble_think(gentity_t *ent) {
	...
	if (ent->start_size) {
	 ...
	 (level.time < (ent->timestamp + ent->end_size + ent->start_size)) {
			time = level.time - ent->timestamp;
