...
// changed this for more realistic angles(at the cost of more network traffic?)
	...
	if (pm->cmd.forwardmove || pm->cmd.rightmove) {
	 ...
	 (pm->cmd.rightmove < 0 && pm->cmd.forwardmove > 0) {
			pm->ps->movementDir = 1;
...
static void PM_Footsteps(void) {
	...
	if (pm->ps->eFlags & EF_DEAD) {
		// before going to limbo, play a wounded / fallen animation
...
// 			Com_Printf("Grenade Timer: %d\n", pm->ps->grenadeTimeLeft);
	...
	if (pm->waterlevel == 3) {
		if (pm->ps->weapon != WP_KNIFE && pm->ps->weapon != WP_KNIFE2 && pm->ps->weapon != WP_GRENADE_LAUNCHER && pm->ps->weapon != WP_GRENADE_PINEAPPLE && pm->ps->weapon != WP_DYNAMITE && pm->ps->weapon != WP_DYNAMITE2) {
