...
// JPW NERVE check if we're close to trigger_concussive_dust fields
	range[0] = radius / 1.41f; // not exactly right, since we're doing a box trap for a radius, but wtf, range[1] = radius / 1.41f; // this is all eye candy anyway
...
void Concussive_think(gentity_t *ent) {
	...
	if (g_gametype.integer == GT_SINGLE_PLAYER) { // in multiplayer this should be handled by ground_shaker
...
// JPW NERVE
	...
	if (etype == ET_MISSILE) {
	 ...
	 (ent->s.weapon == WP_DYNAMITE || ent->s.weapon == WP_DYNAMITE2) {
...
// big nasty dynamite scoring section
		else {
...
// check if dynamite is in trigger_objective_info field
			...
			if (ent->s.weapon == WP_DYNAMITE || ent->s.weapon == WP_PANZERFAUST || ent->s.weapon == WP_GRENADE_LAUNCHER || ent->s.weapon == WP_GRENADE_PINEAPPLE || ent->s.weapon == WP_ROCKET_LAUNCHER || ent->s.weapon == WP_MORTAR || ent->s.weapon == WP_ARTY) {
...
// JPW NERVE don't catch fire if under water or invulnerable
	 ...
	 (self->r.currentOrigin[i] > body->r.absmax[i]) {
