...
#define SCRIPT_REACHGOAL_DIST   8
	...
	if (cs->castScriptStatus.scriptGotoEnt >= 0 && cs->castScriptStatus.scriptGotoId == cs->thinkFuncChangeTime) {
	 ...
	 (!cs->followSlowApproach && VectorLength(vec) < 64 /*&& DotProduct(cs->bs->cur_ps.velocity, vec) < 0*/) {
				AIFunc_IdleStart(cs);  // resume normal AI
				return qtrue;
			} else {
...
qboolean AICast_ScriptAction_Wait(cast_state_t *cs, char *params) {
	...
	if (moverange != 0) {     // default to 200 if no range given
		if (moverange > 0) {
			...
			if ((cs->castScriptStatus.scriptWaitMovetime < level.time) && (cs->bs->enemy >= 0)) {
				// if we can attack them, or they can't attack us, stay here
				// TTimo gcc: suggest parentheses around && within || if (AICast_CheckAttack(cs, cs->bs->enemy, qfalse)
						|| (!AICast_EntityVisible(AICast_GetCastState(cs->bs->enemy), cs->entityNum, qfalse) && !AICast_CheckAttack(AICast_GetCastState(cs->bs->enemy), cs->entityNum, qfalse))) {
...
AICast_ScriptAction_Accum
  ...
  syntax: accum < buffer_index > < command > < parameter>
