...
void AICast_InputToUserCommand(cast_state_t *cs, bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3]) {
	...
	if (aiDefaults[cs->aiCharacter].attributes[ATTACK_CROUCH]) { // only crouch if this character is physically able to
		// disabled this bit so truck guy in forest_6 doesn't get stuck and then gib
	...
	if (bi->actionflags & ACTION_ATTACK) {
		...
		if ((cs->triggerReleaseTime < level.time) && ((cs->lockViewAnglesTime >= level.time) || (fabs(AngleDifference(cs->bs->ideal_viewangles[YAW], cs->bs->viewangles[YAW])) < 20)) && // check for radid luger firing by skilled users(release fire between shots)
...
// ---- (SA)	modified slightly for DM / DK
	...
	if (bi->speed) {
		if ((!(cs->aiFlags & AIFL_WALKFORWARD) && cs->bs->enemy >= 0) || (fabs(AngleDifference(cs->bs->ideal_viewangles[YAW], cs->bs->viewangles[YAW])) < 60)) {
			...
			// changed this to fix stim soldier flying attack
...
*/
	...
	// crouching?
	if ((cs->bs->attackcrouch_time > trap_AAS_Time()) && ((cs->lastAttackCrouch > level.time - 500) || (cs->thinkFuncChangeTime < level.time - 1000))) {
...
// 	}
	...
	for (frame = 0; frame < numframes; frame++) {
	 ...
	 (pm.touchents[i] < MAX_CLIENTS || (pm.touchents[i] != ENTITYNUM_WORLD && (g_entities[pm.touchents[i]].s.eType != ET_MOVER || g_entities[pm.touchents[i]].moverState != MOVER_POS1))) {
					// we have hit another entity, so abort
					move->stopevent = PREDICTSTOP_HITCLIENT;
					goto done;
		
		} else {
	 (!Q_stricmp(g_entities[pm.touchents[i]].classname, "script_mover")) {
...
qboolean AICast_GetAvoid (cast_state_t *cs, bot_goal_t *goal, vec3_t outpos, qboolean reverse, int blockEnt) {
	...
	enemyVisible = (cs->bs->enemy >= 0) && (AICast_CheckAttack(cs, cs->bs->enemy, qfalse));
	...
	for (yaw = -angleDiff * invert; yaw * invert <= maxYaw; yaw += inc * invert) {
	 ...
	 (goal) {
		...
		if ((distmoved > bestmoved)
				// &&	((cs->bs->origin[2] - castmove.endpos[2]) < 64)	// allow up, but not down(falling) && (castmove.groundEntityNum != ENTITYNUM_NONE)) {
...
void AICast_EvaluatePmove(int clientnum, pmove_t *pm) {
	...
	for (i = 0; i < pm->numtouch; i++) {
		...
		if (pm->touchents[i] < aicast_maxclients) {
			...
			if ((ocs->bs) && (!(ocs->aiFlags & AIFL_NOAVOID)) && ((ocs->leaderNum == cs->entityNum) || (VectorLength(ocs->bs->velocity) < 5)) && (ocs->obstructingTime < (level.time + 100))) {
				// if they are moving away from us already, let them go
				if (VectorLength(ocs->bs->cur_ps.velocity) > 10) {
					vec3_t v1, v2;
