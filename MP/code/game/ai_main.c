...
int BotAI(int client, float thinktime) {
	...
	while (trap_BotGetServerCommand(client, buf, sizeof(buf))) {
	 ...
	 (!Q_stricmp(buf, "cs")) {/*ConfigStringModified*/
	 ...
	 (!Q_stricmp(buf, "scores")) {/*FIXME: parse scores?*/

		} else {
	 (!Q_stricmp(buf, "clientLevelShot")) {/*ignore*/
		}
	}
	// add the delta angles to the bot's current view angles
	for (j = 0; j < 3; j++) {
...
int BotAIStartFrame(int time) {
	...
	if (botlib_residual >= thinktime) {
		...
		for (i = 0; i < MAX_GENTITIES; i++) {
			// in single player, we only need client entity information
...
// didn't want to comment in as I wasn't sure of any implications of changing the aas_entityinfo_t and bot_entitystate_t structures.
	...
	// in single player, don't need bot's thinking
	if (g_gametype.integer == GT_SINGLE_PLAYER) {
		return BLERR_NOERROR;
	}
