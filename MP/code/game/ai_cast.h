...
// predict events
typedef enum {
	PREDICTSTOP_NONE, PREDICTSTOP_HITENT, PREDICTSTOP_HITCLIENT
} predictStop_t;
typedef enum {
	AITEAM_NAZI, AITEAM_ALLIES, AITEAM_MONSTER, AITEAM_SPARE1, AITEAM_SPARE2, AITEAM_SPARE3, AITEAM_SPARE4, AITEAM_NEUTRAL
} AITeam_t;
typedef enum {
	BBOX_SMALL, BBOX_LARGE
} BBoxType_t;
// attributes
...
typedef struct {
	...
	char *quietDeathSoundScript;   // added for silent deaths(sniper / knife)
...
typedef enum {
	MS_DEFAULT, MS_WALK, MS_RUN, MS_CROUCH
} movestate_t;
typedef enum {
	MSTYPE_NONE, MSTYPE_TEMPORARY, MSTYPE_PERMANENT
} movestateType_t;
