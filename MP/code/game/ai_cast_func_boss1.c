...
char *AIFunc_FZombie_LightningAttack(cast_state_t *cs) {
	...
	if (ent->s.effect2Time) {
	 ...
	 (cs->followEntity == -2) { // at the base of wall, rotate around then dismount
	...
	if (cs->followEntity >= 0) { // move towards our current goal
	 ...
	 (AICast_VisibleFromPos(g_entities[cs->bs->enemy].client->ps.origin, cs->bs->enemy, cs->bs->origin, cs->entityNum, qfalse)) {
			move = qtrue;
		}
...
char *AIFunc_FZombie_HandLightningAttackStart(cast_state_t *cs) {
	gentity_t *ent = &g_entities[cs->entityNum];
	ent->client->ps.torsoAnim = ((ent->client->ps.torsoAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT)|BOTH_ATTACK4;
