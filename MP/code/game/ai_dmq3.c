...
void BotCTFSeekGoals(bot_state_t *bs) {
	...
	if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL) {
		return;
	}
		...
		if (wi.speed) {
			...
			if (!(dist > 100 && VectorLength(dir) < 32)) {
				...
				if (aim_skill > 0.8 && // if the weapon is ready to fire
					...
					trap_AAS_PredictClientMovement(&move, bs->enemy, origin, PRESENCE_CROUCH, qfalse, dir, cmdmove, 0, dist * 10 / wi.speed, 0.1, 0, 0, qfalse);
...
void BotCheckEvents(bot_state_t *bs, entityState_t *state) {
	...
	case EV_OBITUARY:
	{
	 ...
	 (attacker == bs->enemy && target == attacker) {
	...
	case EV_GLOBAL_SOUND:
	{
	 ...
	 (!strcmp(buf, "sound/teamplay/flagret_blu.wav")) {
			// blue flag is returned
			bs->blueflagstatus = 0;
			bs->flagstatuschanged = qtrue;

		} else {
	 (!strcmp(buf, "sound/items/poweruprespawn.wav")) {
