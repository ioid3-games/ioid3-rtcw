...
void Props_Activated(gentity_t *self) {
	...
	if (owner->active == qfalse) {
	 ...
	 (!Q_stricmp(self->classname, "props_chair")) {
			self->s.frame = 28;
			self->s.density = 1;
...
void SP_props_decoration(gentity_t *ent) {
	...
	if (ent->health) {
	 ...
	 (!Q_stricmp(type, "glass")) {
			ent->key = 0;

		} else {
	 (!Q_stricmp(type, "metal"))                                                     {
			ent->key = 2;

		} else {
	 (!Q_stricmp(type, "ceramic"))                                                                                                             {
			ent->key = 3;

		} else {
	 (!Q_stricmp(type, "rubble"))                                                                                                                                                                       {
			ent->key = 4;
		}
