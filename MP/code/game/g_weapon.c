...
// JPW NERVE
	 ...
	 ((ent->client->sess.sessionTeam == TEAM_RED) && (hit->spawnflags & ALLIED_OBJECTIVE))       { // redundant but added for code clarity
	 ...
	 (hit->spawnflags & ALLIED_OBJECTIVE) {
							...
							if (((hit->spawnflags & AXIS_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_BLUE)) || ((hit->spawnflags & ALLIED_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_RED))) {
								if (hit->track) {
							...
							if ((!(hit->spawnflags & OBJECTIVE_DESTROYED)) && te->s.teamNum && (te->s.teamNum != ent->client->sess.sessionTeam)) {
...
// jpw pulled						hit->spawnflags |= OBJECTIVE_DESTROYED; // this is pretty kludgy but we can't test it in explode fn
				...
				if (traceEnt->health >= 248) {
					...
					// more swipeage -- check if player is in trigger objective field
...
void weapon_callAirStrike(gentity_t *ent) {
	...
	pos[0] = 0;
	pos[1] = 0;
	pos[2] = crandom(); // generate either up or down vector, VectorNormalize(pos); // which adds randomness to pass direction below
...
float G_GetWeaponSpread(int weapon) {
	...
	if (g_gametype.integer == GT_SINGLE_PLAYER) { // don't affect SP game
...
// EARLY OUT: if I'm in multiplayer and I have binocs, try to use artillery and then early return b4 switch statement
	if (g_gametype.integer != GT_SINGLE_PLAYER) {
		if ((ent->client->ps.eFlags & EF_ZOOMING) && (ent->client->ps.stats[STAT_KEYS] & (1 << INV_BINOCS)) && (ent->s.weapon != WP_SNIPERRIFLE)) {
			if (!(ent->client->ps.leanf)) {
