...
// / // / // / // / // / // / // / // / 
typedef enum {
	truck_nosound = 0, truck_idle, truck_gear1, truck_gear2, truck_gear3, truck_reverse, truck_moving, truck_breaking, truck_bouncy1, truck_bouncy2, truck_bouncy3
} truck_states1;
...
// / // / // / // / // / // / // / // / 
typedef enum {
	plane_nosound = 0, plane_idle, plane_flyby1, plane_flyby2, plane_loop, plane_choke, plane_startup
} truck_states2;
...
void Calc_Roll(gentity_t *ent) {
	...
	if (dot2 > 0) {
	 ...
	 (ent->s.apos.trBase[ROLL] > - (ent->angle * 2)) {
			ent->s.apos.trBase[ROLL] -= 2;
		}
...
void Reached_Tramcar(gentity_t *ent) {
	...
	if (!Q_stricmp(ent->classname, "props_me109")) {
	 ...
	 ((next->spawnflags & 1) && !(next->count) && ent->health > 0)       { // SCRIPT flag
			GetNextTrack(ent);
			Think_SetupAirplaneWaypoints(ent);

		} else {
	 ((next->spawnflags & 2) && (ent->spawnflags & 8) && ent->health <= 0 && ent->takedamage)       { // death path
	 ...
	 ((next->spawnflags & 4)) { // explode the plane
