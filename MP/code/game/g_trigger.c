...
// 		G_Printf("trigger_aidoor at loc %s does not have a target door\n", vtos(ent->s.origin));
		...
		// door isn't ready, find a free ai_marker, and wait there until it's open
		if (other->r.svFlags & SVF_CASTAI) {
			...
			// had to change this, since it was causing AI to wait at door when the door is open, and won't close because they are sitting on the aidoor brush
			...
			if (
				!(
					(door->activator == other) && (door->moverState != MOVER_POS1) && (door->moverState != MOVER_POS1ROTATE)
...
// field which is acted upon(cgame side) by screenshakes to drop dust particles
void trigger_concussive_touch(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	G_Printf("hit concussive ent %p mins=%f, %f, %f maxs=%f, %f, %f\n", ent, ent->r.mins[0], ent->r.mins[1], ent->r.mins[2], ent->r.maxs[0], ent->r.maxs[1], ent->r.maxs[2]);
#endif
}
