...
void SV_GetChallenge(netadr_t from) {
	...
	NET_OutOfBandPrint(NS_SERVER, challenge->adr, "challengeResponse %d %d %d %d", challenge->challenge, sv_onlyVisibleClients->integer, clientChallenge, com_protocol->integer);
}
...
void SV_AuthorizeIpPacket(netadr_t from) {
	...
	if (!Q_stricmp(s, "accept")) {
		NET_OutOfBandPrint(NS_SERVER, challengeptr->adr, "challengeResponse %d %d %d %d", challengeptr->challenge, sv_onlyVisibleClients->integer, challengeptr->clientChallenge, com_protocol->integer);
		return;
	}
...
char *sv_pureVMPaths[] = {
	// QVM names
	"vm / %s.mp.qvm", // Files in original mp_bin.pk3
	"%s_mp_x86.dll", "%s.mp.i386.so", // Other VM names used by the game which could be put in a pk3.
	// They would be loaded by clients so it makes no sense to kick for using them.
	"%s_mp_x86_64.dll", "%s.mp.x86_64.so", "%s.mp.i386.dylib", "%s.mp.x86_64.dylib", "%s.mp.ppc.dylib"
};
...
// 		MSG_ReadDeltaUsercmd(msg, oldcmd, cmd);
	...
	for (i = 0; i < cmdCount; i++) {
		...
		if (sv_gametype->integer != GT_SINGLE_PLAYER) { // we need to allow this in single player, where loadgame's can cause the player to freeze after reloading if we do this check
