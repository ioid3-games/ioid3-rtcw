...
// 	Cvar_Set("nextmap", va("map %s", server));
	...
	SV_ClearWorld();	// media configstring setting should be done during the loading stage, so connected clients don't have to load during actual gameplay
	sv.state = SS_LOADING;
	...
	if (sv_pure->integer) {
		...
		// separate pk3 file and the client will need to load the latest cgame
		if (com_dedicated->integer) {
			SV_TouchCGameDLL();
		}
	} else {
...
void SV_Init(void) {
	...
	sv_maxclients = Cvar_Get("sv_maxclients", "20", CVAR_SERVERINFO|CVAR_LATCH); // changed to 20 from 8
	...
	sv_tourney = Cvar_Get("g_noTeamSwitching", "0", CVAR_ARCHIVE);
	// systeminfo
