...
static void SV_Map_f(void) {
	...
	if (Q_stricmpn(cmd, "sp", 2) == 0) {
		...
		Cvar_SetLatched("sv_maxclients", "32"); // modified this
		cmd += 2;
		killBots = qtrue;
...
static void SV_MapRestart_f(void) {
	...
	if (
		(nextgt == GT_WOLF && (worldspawnflags & 1)) || (nextgt == GT_WOLF_STOPWATCH && (worldspawnflags & 2)) || ((nextgt == GT_WOLF_CP || nextgt == GT_WOLF_CPH) && (worldspawnflags & 4))
	) {
