...
// 			SV_AddEntitiesVisibleFromPoint(ent->s.origin2, frame, eNums, qtrue, oldframe, localClient);
		...
		if (e < sv_maxclients->integer) 	// client
		{
			...
			if (awh_active->integer && !portal && !(client->r.svFlags & SVF_BOT) && (frame->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR)) {
...
notVisible:
		...
		// if this entity has changed events, then send it regardless of whether we can see it or not
		// not in multiplayer please
