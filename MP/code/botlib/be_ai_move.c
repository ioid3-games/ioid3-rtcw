...
// done.
		...
		if (!reachnum) {
			...
			reachnum = BotGetReachabilityToGoal(ms->origin, ms->areanum, ms->entitynum, ms->lastgoalareanum, ms->lastareanum, ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal, travelflags, travelflags);
		...
		for (i = numareas - 1; i >= 0; i--) {
			if (AAS_AreaJumpPad(areas[i])) {
				...
				foundjumppad = qtrue;
				lastreachnum = BotGetReachabilityToGoal(end, areas[i], ms->entitynum,  ms->lastgoalareanum, ms->lastareanum,  ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries,  goal, travelflags, TFL_JUMPPAD);
