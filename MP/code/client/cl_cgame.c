...
// this is only used when running a local server
...
// 		Com_Printf("%s%f\n", (const char *)VMA(1), VMF(2));
	...
	case CG_INGAME_POPUP:
	 ...
	 (VMA(1) && !Q_stricmp(VMA(1), "UIMENU_WM_PICKPLAYER")) {
				VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_WM_PICKPLAYER);
	
		} else {
	 (VMA(1) && !Q_stricmp(VMA(1), "UIMENU_WM_QUICKMESSAGE")) {
				VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_WM_QUICKMESSAGE);
	
		} else {
	 (VMA(1) && !Q_stricmp(VMA(1), "UIMENU_WM_QUICKMESSAGEALT")) {
				VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_WM_QUICKMESSAGEALT);
	
		} else {
	 (VMA(1) && !Q_stricmp(VMA(1), "UIMENU_WM_LIMBO")) {
				VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_WM_LIMBO);
	
		} else {
	 (VMA(1) && !Q_stricmp(VMA(1), "UIMENU_WM_AUTOUPDATE")) {
	 ...
	 (VMA(1) && !Q_stricmp(VMA(1), "hbook2")) { // ---- (SA)
				VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_BOOK2);
	
		} else {
	 (VMA(1) && !Q_stricmp(VMA(1), "hbook3")) { // ---- (SA)
