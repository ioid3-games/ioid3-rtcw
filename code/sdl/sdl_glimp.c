...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder, qboolean fixedFunction) {
	...
	for (i = 0; i < 16; i++) {
		...
		if (r_centerWindow->integer && !fullscreen) {
			if ((SDL_window = SDL_CreateWindow(CLIENT_WINDOW_TITLE, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, glConfig.vidWidth, glConfig.vidHeight, flags)) == NULL) {
				ri.Printf(PRINT_DEVELOPER, "SDL_CreateWindow failed: %s\n", SDL_GetError());
				continue;
			}
		} else if ((r_windowPosx->integer || r_windowPosy->integer) && !fullscreen) {
			if ((SDL_window = SDL_CreateWindow(CLIENT_WINDOW_TITLE, r_windowPosx->integer, r_windowPosy->integer, glConfig.vidWidth, glConfig.vidHeight, flags)) == NULL) {
				ri.Printf(PRINT_DEVELOPER, "SDL_CreateWindow failed: %s\n", SDL_GetError());
				continue;
			}
		} else {
