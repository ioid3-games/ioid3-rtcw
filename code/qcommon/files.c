...
When building a pak file, make sure a wolfconfig.cfg isn't present in it, or configs will never get loaded from disk!
...
qboolean FS_PakIsPure(pack_t *pack) {
	...
	if (fs_numServerPaks) {
		// NOTE: we are matching checksums without checking the pak names this means you can have the same pk3 as the server under
		// a different name, you will still get through sv_pure validation (what happens when two pk3's have the same checkums? is it a likely situation?)
		...
		if (search->pack->hashTable[hash]) {
			...
			do {
				...
				if (!FS_FilenameCompare(pakFile->name, filename)) {
					...
					if (Q_stricmp(filename + len - 4, ".bsp") == 0 && !Q_stricmpn(filename + 5, "mp_", 3)) {
						*file = 0;
						return -1;
					}
	...
	while (search) {
		if (search->dir && (unpure || !Q_stricmp(name, "qagame"))) {
...
#ifndef DEDICATED
			// if the server is pure, extract the dlls from the mp_bin.pk3 so that they can be referenced
			if (Q_stricmp(name, "qagame")) {
				...
				if (FS_FOpenFileReadDir(dllName, search, NULL, qfalse, unpure) > 0 && whitelisted) {
#else
				if (FS_FOpenFileReadDir(dllName, search, NULL, qfalse, unpure) > 0 && FS_CL_ExtractFromPakFile(search, netpath, dllName, NULL)) {
#endif
...
The return value doesn't tell wether file was extracted or not, it just says wether it's ok to continue (i.e. either the right file was
extracted successfully, or it was already present). cvar_lastVersion is the optional name of a CVAR_ARCHIVE used to store the wolf
...
This was not in the 1.30 filesystem code. Using fs_homepath for the file to remove.
=======================================================================================================================================
*/
...
Returns a list of mod directory names. A mod directory is a peer to baseq3 with a pk3 in it.
...
void FS_AddGameDirectory(const char *path, const char *dir, qboolean allowUnzippedDLLs) {
	...
	// sort them so that later alphabetic matches override earlier ones. This makes pak1.pk3 override pak0.pk3
	...
	// the order is: first others, sp_ paks, then pak0, then mp_ paks, so we have to do this in reverse overhere
...
qboolean FS_idPak(char *pak, char *base, int numPaks) {
	...
	for (i = 0; i < NUM_ID_PAKS; i++) {
		...
		// this fn prevents external sources from downloading/overwriting official files, so exclude both SP and MP files from this list as well
...
NOTE: If you're building a game that doesn't depend on the RTCW media mp_pak0.pk3, you'll want to remove this by defining STANDALONE in
q_shared.h.
=======================================================================================================================================
*/
static void FS_CheckMPPaks(void) {
	...
	for (path = fs_searchpaths; path; path = path->next) {
		...
		if (!Q_stricmpn(curpack->pakGamename, BASEGAME, MAX_OSPATH) && strlen(pakBasename) == 7 && !Q_stricmpn(pakBasename, "mp_pak", 6) && pakBasename[6] >= '0' && pakBasename[6] <= '0' + NUM_MP_PAKS - 1) {
...
NOTE: if you're building a game that doesn't depend on the RTCW media sp_pak0.pk3, you'll want to remove this by defining
...
static void FS_CheckSPPaks(void) {
	...
	for (path = fs_searchpaths; path; path = path->next) {
		...
		if (!Q_stricmpn(curpack->pakGamename, BASEGAME, MAX_OSPATH) && strlen(pakBasename) == 7 && !Q_stricmpn(pakBasename, "sp_pak", 6) && pakBasename[6] >= '1' && pakBasename[6] <= '1' + NUM_SP_PAKS - 1) {
			if (curpack->checksum != en_sppak_checksums[pakBasename[6] - '1'] && curpack->checksum != fr_sppak_checksums[pakBasename[6] - '1'] && curpack->checksum != gm_sppak_checksums[pakBasename[6] - '1'] && curpack->checksum != it_sppak_checksums[pakBasename[6] - '1'] && curpack->checksum != sp_sppak_checksums[pakBasename[6] - '1']) {
				if (pakBasename[6] == '1') {
					Com_Printf("\n\n"
						"**************************************************\n"
			...
			for (index = 0; index < ARRAY_LEN(en_sppak_checksums); index++) {
				if (curpack->checksum == en_sppak_checksums[index] || curpack->checksum == fr_sppak_checksums[index] || curpack->checksum == gm_sppak_checksums[index] || curpack->checksum == it_sppak_checksums[index] || curpack->checksum == sp_sppak_checksums[index]) {
					Com_Printf("\n\n"
						"**************************************************\n"
...
NOTE: If you're building a game that doesn't depend on the RTCW media pak0.pk3, you'll want to remove this by defining
STANDALONE in q_shared.h.
=======================================================================================================================================
*/
static void FS_CheckPak0(void) {
	...
	for (path = fs_searchpaths; path; path = path->next) {
		...
		if (!Q_stricmpn(curpack->pakGamename, "demomain", MAX_OSPATH) && !Q_stricmpn(pakBasename, "pak0", MAX_OSPATH)) {
...
NOTE: this code is taken from Wolf MP source. Pure checksums code is not relevant to SP binary anyway.
=======================================================================================================================================
*/
