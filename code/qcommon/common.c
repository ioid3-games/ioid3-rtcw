...
void QDECL Com_Printf(const char *fmt, ...) {
	...
	if (com_logfile && com_logfile->integer) {
		...
		if (!logfile && FS_Initialized() && !opening_qconsole) {
			...
			logfile = FS_FOpenFileWrite("rtcwconsole.log"); // changed name for Wolf
...
void Hunk_Clear(void) {
	...
	VM_Clear(); // FIXME: TODO: was commented out in wolf
#ifdef HUNK_DEBUG
	hunkblocks = NULL;
#endif
}
	...
	if (!Com_AddStartupCommands()) {
		...
		if (!com_dedicated->integer) {
			...
			if (!com_introPlayed->integer) {
				//Cvar_Set(com_introPlayed->name, "1"); // force this to get played every time (but leave cvar for override)
		...
		// starting a non dedicated server from the command line on OSX fails because the intro starts playing in the background
#ifdef CINEMATICS_INTRO
		playIntro = qfalse;
#endif
	}
