...
// added to make it easier to raise/lower our statsubar by only changing one thing
...
void CG_Text_Paint(float x, float y, int font, float scale, vec4_t color, const char *text, float adjust, int limit, int style) {
	...
	useScale = scale * fnt->glyphScale;
	color[3] *= cg_hudAlpha.value; // adjust for cg_hudalpha
...
IT_KEY (this makes this routine easier to find in files...).
=======================================================================================================================================
*/
static void CG_DrawStatusBarKeys() {
	...
	// just don't draw this stuff for now. It's got fog issues I need to clean up
...
//		{0.2, 1.0, 0.2, 1.0}, {1.0, 0.2, 0.2, 1.0}, {0.5, 0.5, 0.5, 1}};
	...
	// further change... we don't need to draw the ammo 3d model do we?
/*
	if (cent->currentState.weapon && cg_weapons[cent->currentState.weapon].ammoModel) {
		...
		// Wolf statusbar change
		//CG_Draw3DModel(CHAR_WIDTH * 3 + TEXT_ICON_SPACE, STATUSBARHEIGHT -20, ICON_SIZE, ICON_SIZE, cg_weapons[cent->currentState.weapon].ammoModel, 0, origin, angles);
	}
*/
	...
	// further change... we don't need to draw the armor do we?
	// ammo
	if (cent->currentState.weapon) {
		...
		if (value > -1) {
			if (cg.predictedPlayerState.weapon == WP_KNIFE) {
				color = 3;
			} else if ((cg.predictedPlayerState.weaponstate == WEAPON_FIRING || cg.predictedPlayerState.weaponstate == WEAPON_FIRINGALT) && cg.predictedPlayerState.weaponTime > 100) {
...
static float CG_DrawCoopOverlay(float y) {
	...
	w = (pwidth + lwidth + 7 + 7) * TINYCHAR_WIDTH; // was + 4 + 7
	x = 640 - w - 1; // was -32
	h = plyrs * TINYCHAR_HEIGHT;
	// set the max characters that can be printed before the left edge
	...
	for (i = 0; i < numSortedTeamPlayers; i++) {
		...
		if (ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
			...
			CG_DrawStringExt(xx, y, st, hcolor, qtrue, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 1);
			...
			CG_DrawStringExt(xx, y, ci->name, pcolor, qtrue, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, TEAM_OVERLAY_MAXNAME_WIDTH);
			...
			if (lwidth) {
				...
				xx = x + TINYCHAR_WIDTH * 5 + TINYCHAR_WIDTH * pwidth + ((lwidth / 2 - len / 2) * TINYCHAR_WIDTH);
				CG_DrawStringExt(xx, y, p, hcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, TEAM_OVERLAY_MAXLOCATION_WIDTH);
			}

			Com_sprintf(st, sizeof(st), "%3i/%i", ci->health, ci->armor); // pulled class stuff since it's at top now

			xx = x + TINYCHAR_WIDTH * 6 + TINYCHAR_WIDTH * pwidth + TINYCHAR_WIDTH * lwidth;

			CG_DrawStringExt(xx, y, st, pcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 7);
...
static void CG_DrawUpperRight(stereoFrame_t stereoFrame) {
	...
	// disabling drawattacker for the time being
...
#define CHATLOC_X 0
	...
	if (cgs.teamLastChatPos != cgs.teamChatPos) {
		...
		// rewritten to support first pass at fading chat messages
...
static void CG_DrawNotify(void) {
	...
	if (cgs.notifyLastPos != cgs.notifyPos) {
		...
		for (i = cgs.notifyPos - 1; i >= cgs.notifyLastPos; i--) {
			...
			trap_R_SetColor(hcolor);
			CG_DrawStringExt(NOTIFYLOC_X + TINYCHAR_WIDTH, NOTIFYLOC_Y - (cgs.notifyPos - i) * TINYCHAR_HEIGHT, cgs.notifyMsgs[i % chatHeight], hcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, maxCharsBeforeOverlay);
		}
	}
...
static void CG_DrawCrosshair(void) {
	...
	hcolor[3] = cg_crosshairAlpha.value;
	// on mg42
...
//		CG_FillRect(319, 242, 2, 16, hcolor);	// vertical
	...
	friendInSights = (qboolean)(cg.snap->ps.serverCursorHint == HINT_PLYR_FRIEND);
	...
	// modified, fixes crosshair offset in shifted/scaled 3d views, also breaks scaled view...
	if (cg_fixedAspect.integer) {
...
void CG_CheckForCursorHints(void) {
	...
	// removed all the client-side stuff. don't think it's really necessary anymore
}
...
//	CG_DrawBigString(320 - w / 2, 170, name, color[3] * 0.5);
	...
	if (strlen(name)) {
		if ((cgs.clientinfo[cg.crosshairClientNum].team == TEAM_RED) && (cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_RED)) { // only show same team info so people can't pan-search
			CG_DrawBigStringColor(320 - w / 2, 170, va("Axis: %s", name), teamColor);
		} else if ((cgs.clientinfo[cg.crosshairClientNum].team == TEAM_BLUE) && (cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_BLUE)) { // so's we can't find snipers for free
...
#ifdef LOCALISATION
			Com_sprintf(deploytime, sizeof(deploytime), CG_TranslateString("Deploying in %d seconds"), (int)(1 + (float)(cg_limbotime.integer - (cg.time % cg_limbotime.integer)) * 0.001f));
#else
			Com_sprintf(deploytime, sizeof(deploytime), "Deploying in %d seconds", (int)(1 + (float)(cg_limbotime.integer - (cg.time % cg_limbotime.integer)) * 0.001f));
#endif
		}
...
static void CG_DrawWarmup(void) {
	...
	if (cgs.gametype == GT_SINGLE_PLAYER) {
		return; // don't bother with this stuff in sp
	}
...
*/
	...
	w = CG_DrawStrlen(s);
	CG_DrawStringExt(320 - w * cw / 2, 120, s, colorWhite, qfalse, qtrue, cw, (int)(cw * 1.5), 0);
}
	...
	if (text) {
		...
		while (s && *s && count < len) {
			...
			if (Q_IsColorString(s)) {
				...
				CG_Text_PaintChar_Ext(x + (scalex), y - yadj, glyph->imageWidth, glyph->imageHeight, scalex, scaley, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
	...
	while (worldtext) {
		...
		if (CG_WorldToScreen(worldtext->origin, &x, &y) && PointVisible(worldtext->origin) && DistanceSquared(cg.refdef.vieworg, worldtext->origin) < MAX_RENDERDIST * MAX_RENDERDIST) {
			...
			while (tok) {
				...
				CG_Text_Paint_Ext(x - (width * 0.5), y + heightOffset, fTxtScale, fTxtScale, worldtext->color, tok, 0, 0, ITEM_TEXTSTYLE_NORMAL, font);
