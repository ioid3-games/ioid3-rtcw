...
static void CG_ResetEntity(centity_t *cent) {
	...
	// if an event is set, assume it is new enough to use, if the event had timed out, it would have been cleared
	cent->previousEvent = 0;
	// disabled this, since we clear events out now in g_main.c, this is redundant, and actually causes "double-door-sound" syndrome if a door is triggered and heard, and then comes into view before
	// it has timed out in g_main.c, therefore playing the sound again, since it thinks it hasn't processed this event yet.
