...
static int maxWeapBanks = MAX_WEAP_BANKS, maxWeapsInBank = MAX_WEAPS_IN_BANK;
...
void CG_PyroSmokeTrail(centity_t *ent, const weaponInfo_t *wi) {
	...
	for (; t <= ent->trailTime; t += step) {
		...
		if (!ent->currentState.otherEntityNum2) { // axis team, generate red smoke
			CG_SmokePuff(origin, dir, 25 + rnd * 110, rnd * 0.5 + 0.5, rnd * 0.5 + 0.5, 1, 0.5, 4800 + (rand()% 2800), t, 0, 0, cgs.media.smokePuffShader);
		} else {
			CG_SmokePuff(origin, dir, 25 + rnd * 110, 1.0, rnd * 0.5 + 0.5, rnd * 0.5 + 0.5, 0.5, 4800 + (rand()% 2800), t, 0, 0, cgs.media.smokePuffShader);
		}
	}
}
...
/*
	...
	if ((cg.time - ent->lastTrailTime)>= 50 + rand()%50) {
		ent->headJuncIndex = CG_AddSmokeJunc(ent->headJuncIndex, cgs.media.smokeTrailShader, origin, 4500, 0.4, 20, 80);
...
void CG_RegisterWeapon(int weaponNum) {
	...
	switch (weaponNum) {
		...
		case WP_SNIPER:
		case WP_MORTAR:
		// don't know about these, but we don't have models for 'em
	...
	if (!weaponInfo->weaponModel[W_TP_MODEL]) {
		...
		//I need to be able to run the game, I dont have the silencer weapon(19)
#ifndef _DEBUG
...
void CG_RegisterItemVisuals(int itemNum) {
	...
	// had to put this somewhere, this seems OK
...
static float CG_VenomSpinAngle(centity_t *cent) {
	...
	if (cg.snap->ps.weaponstate != WEAPON_FIRING) { // this seems better
		firing = qfalse;
	}
...
qboolean CG_DrawRealWeapons(centity_t *cent) {
	...
	switch (cent->currentState.aiChar) {
		...
		case AICHAR_ZOMBIE:
		case AICHAR_HELGA: // boss1 is now helga-blob
...
TODO: this needs to be fixed for multiplay. entities being hurt need to be sent by server to all clients, so they draw the correct
effects.
=======================================================================================================================================
*/
void CG_PlayerTeslaCoilFire(centity_t *cent, vec3_t flashorigin) {
	...
	if (!cent->pe.teslaDamageApplyTime || cent->pe.teslaDamageApplyTime < cg.time - 200) {
		...
		if ((cent != &cg_entities[cg.snap->ps.clientNum]) && (cg.snap->ps.teamNum != playerTeam) && (Distance(tagPos, cg.snap->ps.origin)< TESLA_LIGHTNING_MAX_DIST) && (DotProduct(viewDir, vec) > 0.8)) {
...
*/
				if (ctrav->currentState.aiChar && (ctrav != cent) && (ctrav->currentState.teamNum != playerTeam) && !(ctrav->currentState.eFlags & EF_DEAD) && ctrav->currentValid && (Distance(tagPos, ctrav->lerpOrigin) < TESLA_LIGHTNING_MAX_DIST)) { // is in the visible frame
	...
	for (i = 0; i < numPoints; i++) {
		...
		} else if ((!cent->pe.teslaEndPointTimes[i]) || (cent->pe.teslaEndPointTimes[i] > cg.time) || (cent->pe.teslaEndPointTimes[i] < cg.time - TESLA_LIGHTNING_POINT_TIMEOUT) || (VectorDistance(tagPos, cent->pe.teslaEndPoints[i]) > maxDist) || (DotProduct(viewDir, vec) < 0.7)) {
		...
		// we have a valid lightning point, so draw it sanity check though to make sure it's valid
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent) {
	...
	// might as well have this check consistant throughout the routine
	...
	if (weaponNum == WP_GAUNTLET) { // this is the 'knife'. no model yet, so we can give it to the zombie and have him visually 'unarmed'
		if (debuggingweapon) {
	...
	if (!ps && cg.snap->ps.pm_flags & PMF_LADDER && isPlayer) { // on ladder
		if (debuggingweapon) {
	...
	if (!ps && playerScaled) { // don't "un-scale" weap up in 1st person
		for (i = 0; i < 3; i++) { // scale weapon back up so it doesn't pick up the adjusted scale of the character models, this will affect any parts attached to the gun as well(barrel/bolt/flash/brass/etc.)
	...
	if (isPlayer) {
		...
		// opposite tag in akimbo, since at this point the weapon has fired and the fire seq has switched over
	...
	if (ps) {
		...
		for (i = W_PART_1; i < W_MAX_PARTS; i++) {
			...
			if (weaponNum == WP_VENOM) {
				...
				// not right now. at the moment, just spin the belt when firing, no swapout
				} else if (i == W_PART_3) {
...
#define BARREL_SMOKE_TIME 1000
	...
	if (flash.hModel) {
		if (weaponNum != WP_FLAMETHROWER && weaponNum != WP_TESLA) { // hide the flash also for now
	...
	if (ps || cg.renderingThirdPerson || !isPlayer) {
		if (firing) {
			...
			if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2]) {
				trap_R_AddLightToScene(flash.origin, 200 + (rand()& 31), weapon->flashDlightColor[0], weapon->flashDlightColor[1], weapon->flashDlightColor[2], 0);
			}
		} else {
...
static qboolean CG_WeaponSelectable(int i) {
	...
	// allow switch out of scope for weapons where you fired the last shot while scoped and we left you in that view to see the result of the shot
...
If the weap passed in is the lowest in a cycle(0), this will loop around to the top(maxWeapsInBank-1)and start down from there
...
Pass in a bank and cycle and this will return the next valid weapon in a higher bank. sameBankPosition: if there's a weapon in the next
bank at the same cycle, return that(colt returns thompson for example)rather than the lowest weapon.
=======================================================================================================================================
*/
...
Pass in a bank and cycle and this will return the next valid weapon in a lower bank. sameBankPosition: if there's a weapon in the prev
bank at the same cycle, return that(thompson returns colt for example)rather than the highest weapon.
=======================================================================================================================================
*/
...
Passing the weapnum of the mp40 returns the id of the thompson, and likewise passing the weapnum of the thompson returns the id of the
mp40. No equivalent available will return the weapnum passed in.
=======================================================================================================================================
*/
int getEquivWeapon(int weapnum) {
	int num = weapnum;
...
void CG_FinishWeaponChange(int lastweap, int newweap) {
	...
	// remember which weapon in this bank was last selected so when cycling back to this bank, that weap will be highlighted first
	...
	if (lastweap == cg.lastFiredWeapon) {
		...
		// if this ended up having the switchback be the same as the new weapon, set the switchback to the prev
...
void CG_AltWeapon_f(void) {
	...
	if (CG_WeaponSelectable(num)) { // new weapon is valid
		...
		// this way, if you switch away from the silenced luger, the silencer will still be attached when you switch back(until you remove it)
...
Weapon keys are not generally bound directly('bind 1 weapon 1'), rather the key is bound to a given bank ('bind 1 weaponbank 1').
=======================================================================================================================================
*/
...
void CG_Weapon_f(void) {
	...
	// the new weapon was not found in the reglar banks, assume the player want's to go directly to it if possible
	if (!banked) {
		if (CG_WeaponSelectable(num)) {
			CG_FinishWeaponChange(curweap, num);
			return;
...
void CG_OutOfAmmoChange(void) {
	...
	// still nothing available, just go to the next available weap using the regular selection scheme
	CG_NextWeap(qtrue);
}
...
void CG_FireWeapon(centity_t *cent) {
	...
	} else if (ent->weapon == WP_GRENADE_LAUNCHER || ent->weapon == WP_GRENADE_PINEAPPLE || ent->weapon == WP_DYNAMITE || ent->weapon == WP_GRENADE_SMOKE) {
...
void CG_AddDirtBulletParticles(vec3_t origin, vec3_t dir, int speed, int duration, int count, float randScale, float width, float height, float alpha, char *shadername) {
	...
	VectorCopy(origin, pos);
	CG_ParticleDirtBulletDebris_Core(pos, velocity, duration, width,height, alpha, shadername); // keep central one
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir, int surfFlags) { // modified to send missilehitwall surface parameters
	...
	switch (weapon) {
		...
		case WP_VENOM:
			...
			if (!clientNum) {
				...
				if (surfFlags & SURF_SNOW) {
					CG_AddDirtBulletParticles(origin, dir, 120, 900, 3, 0.6f, 20, 4, 0.3f, "water_splash"); // rand scale
				}
				// grass/gravel
				if (surfFlags &(SURF_GRASS|SURF_GRAVEL)) {
					CG_AddDirtBulletParticles(origin, dir, 190, 800, 3, 0.1, 60, 10, 0.5, "dirt_splash"); // rand scale
				} else {
				...
				CG_AddDirtBulletParticles(origin, dir, 190, 900, 5, 0.5, 50, 12, 0.125, "water_splash");
				break; // testing
#if 0
				// play a water splash
		...
		case WP_MORTAR:
			...
			// I like this new animation, feel free to revert
		...
		case WP_GRENADE_PINEAPPLE:
			...
			VectorMA(origin, 16, dir, sprOrg);
			VectorScale(dir, 100, sprVel);
			// testing new explosion animation
...
void CG_MissileHitPlayer(centity_t *cent, int weapon, vec3_t origin, vec3_t dir, int entityNum) {
	...
	switch (weapon) {
		...
		case WP_PANZERFAUST:
			...
			CG_MissileHitWall(weapon, 0, origin, dir, 0); // modified to send missilehitwall surface parameters
			break;
		default:
			break;
	}
...
Perform the same traces the server did to locate the hit splashes (FIXME: random seed isn't synced anymore). Right now this is random
like a shotgun. I want to make it more organized so that the pattern is more of a circle(with some degree of randomness).
=======================================================================================================================================
*/
...
void CG_VenomFire(entityState_t *es, qboolean fullmode) {
	...
	if (cgs.glconfig.hardwareType != GLHW_RAGEPRO) {
		...
		if (!(contents & CONTENTS_WATER)) {
			...
			if (fullmode) {
				CG_SmokePuff(v, up, 24, 1, 1, 1, 0.33, 1200, cg.time, 0, 0, cgs.media.shotgunSmokePuffShader);
			}
...
void CG_SpawnTracer(int sourceEnt, vec3_t pstart, vec3_t pend) {
	...
	if (sourceEnt < cgs.maxclients) {
		// for visual purposes, find the actual tag_weapon for this client and offset the start and end accordingly
		if (cg_entities[sourceEnt].currentState.eFlags & EF_MG42_ACTIVE) { // mounted
			start[2] -= 32; // hack to get the tracer down below the barrel FIXME: do properly
		} else {
...
void CG_Bullet(vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh, int fleshEntityNum, qboolean wolfkick, int otherEntNum2) {
	trace_t trace;
	int sourceContentType = 0, destContentType = 0;
	vec3_t dir;
	vec3_t start, trend, tmp;
	...
	if (flesh) {
		...
		if (fleshEntityNum == cg.snap->ps.clientNum) {
			// TODO: for metal guys, make metal a flag rather than an aitype check?
		...
		if (CG_CalcMuzzlePoint(sourceEntityNum, start) || cg.snap->ps.persistant[PERS_HWEAPON_USE]) {
			...
			if ((trace.surfaceFlags & SURF_METAL) || !(rand()% 10) || (otherEntNum2 != ENTITYNUM_NONE)) {
				// compute new spark direction from normal & dir(rotate -dir 180 degrees about normal)
				// NOTE: isn't this done by the server and sent along in the 'normal' that's passed into this routine?(1107 g_weapon.c)
				VectorScale(dir,-1.0f,tmp);
				RotatePointAroundVector(tmp,normal,tmp,180.0f);
				CG_MissileHitWall(fromweap, 0, end, tmp, trace.surfaceFlags); // sparks, modified to send missilehitwall surface parameters
			}
			...
			if (!(sourceContentType & CONTENTS_WATER) && (destContentType &(CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA))) { // only when shooting /into/ water
				...
				CG_MissileHitWall(fromweap, 1, end, normal, trace.surfaceFlags); // smoke puff, modified to send missilehitwall surface parameters
