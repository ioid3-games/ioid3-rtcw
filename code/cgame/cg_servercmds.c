...
*/
	...
	if (CG_GetVoiceChat(voiceChatList, cmd, &snd, &sprite, &chat)) {
		if (mode == SAY_TEAM || !cg_teamChatsOnly.integer) {
			...
			if (!loc || !*loc) {
				if (mode == SAY_TELL) {
					Com_sprintf(vchat.message, sizeof(vchat.message), "[%s]%c%c: %c%c%s", ci->name, Q_COLOR_ESCAPE, COLOR_YELLOW, Q_COLOR_ESCAPE, color, chat);
				...
				if (mode == SAY_TELL) {
					Com_sprintf(vchat.message, sizeof(vchat.message), "[%s]%c%c[%s]: %c%c%s", ci->name, Q_COLOR_ESCAPE, COLOR_YELLOW, loc, Q_COLOR_ESCAPE, color, chat);
				} else if (mode == SAY_TEAM) {
					Com_sprintf(vchat.message, sizeof(vchat.message), "(%s)%c%c(%s): %c%c%s", ci->name, Q_COLOR_ESCAPE, COLOR_YELLOW, loc, Q_COLOR_ESCAPE, color, chat);
				} else {
					Com_sprintf(vchat.message, sizeof(vchat.message), "%s %c%c(%s): %c%c%s", ci->name, Q_COLOR_ESCAPE, COLOR_YELLOW, loc, Q_COLOR_ESCAPE, color, chat);
...
static void CG_MapRestart(void) {
	...
	cg.v_noFireTime = 0;
	cg.v_fireTime = 0;
	// clear out animScriptData so we recalc everything and get new pointers from server
	...
	// check for server set weapons we might not know about(FIXME: this is a hack for the time being since a scripted "selectweapon" does not hit the first snap, the server weapon set in cg_playerstate.c line 219 doesn't do the trick)
	if (!cg.weaponSelect) {
...
static void CG_ServerCommand(void) {
...
#ifdef LOCALISATION
		CG_CenterPrint(va("%s %d %s", CG_TranslateString("Dynamite timer set at"), time, CG_TranslateString("seconds")), SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH);
#else
	...
	if (!strcmp(cmd, "cp")) {
		...
		if (args >= 3) {
			...
			CG_CenterPrint(CG_LocalizeServerCommand(CG_Argv(1)), SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH);
		}
	...
	if (!strcmp(cmd, "loaddeferred")) { // spelling fixed
