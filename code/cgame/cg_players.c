...
Modified this for head separation.
=======================================================================================================================================
*/
...
void CG_LoadClientInfo(int clientNum, clientInfo_t *ci) {
	...
	// load the head first (since if the head loads but there is a problem with something in the lower body, you will want to default the model back to a default and want the head to match)
	if (!CG_RegisterClientHeadname(ci, ci->modelName, ci->hSkinName)) {
		...
		if (!CG_RegisterClientHeadname(ci, ci->modelName, "default")) {
			...
			if (cg_buildScript.integer) {
				CG_Error("head model/skin(%s/default)failed to register", ci->modelName);
			}
		}
	}
...
static qboolean CG_ScanForExistingClientInfo(clientInfo_t *ci) {
	...
	for (i = 0; i < cgs.maxclients; i++) {
		...
		// added checks for same head. FIXME: soon this will be more efficient than just looking for a complete character(body/head)match.
...
void CG_NewClientInfo(int clientNum) {
	...
	newInfo.team = atoi(v);
	// modified this for head separation
	// the problem I see with having the model set for cg_forceModel in the game(g_forcemodel)
	...
	// modify \/ to differentiate for head models/skins as well
...
static void CG_IdleHeadMovement(centity_t *cent, const vec3_t torsoAngles, vec3_t headAngles) {
	...
	if ((cent->currentState.eFlags & EF_DEAD) || !(cent->currentState.eFlags & EF_HEADLOOK)) {
		...
		VectorSet(cent->pe.headLookIdeal, diffRandMax * (-0.25 + crandom()) * 0.25, diffRandMax * crandom(), 0);
...
static void CG_PlayerSprites(centity_t *cent) {
	...
	if (cgs.gametype <= GT_COOP && cent->voiceChatSpriteTime > cg.time && cg.snap->ps.persistant[PERS_TEAM] == team) {
	...
	if (cgs.gametype > GT_SINGLE_PLAYER && !(cent->currentState.eFlags & EF_DEAD) && cg.snap->ps.persistant[PERS_TEAM] == team) {
	...
	if (dist < SHADOW_MAX_DIST) { // show more detail
		...
		for (tagIndex = 0; shadowParts[tagIndex].tagname; tagIndex++) {
			...
			for (subIndex = 0;(subIndex = CG_GetOriginForTag(cent, &cent->pe.legsRefEnt, shadowParts[tagIndex].tagname, subIndex, origin, axis))>= 0; subIndex++) {
				...
				CG_ImpactMark(shadowParts[tagIndex].shader, origin, trace.plane.normal, angles[YAW], alpha, alpha, alpha, 1, qfalse, shadowParts[tagIndex].size * scale, qtrue, -1);
			}
		}
	}
...
void CG_AddZombieSpiritEffect(centity_t *cent) {
	...
	// sanity check the server time to make sure we don't start the effect to early, whilst reloading a savegame or something
	...
	refent.backlerp = 0;
	refent.renderfx = RF_NOSHADOW|RF_MINLIGHT;
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, int powerups, int team, entityState_t *es, const vec3_t fireRiseDir) {
	...
	// Zombie effects
		...
		if (es->time2) {
			...
			if (es->eFlags & EF_MONSTER_EFFECT2 && cent->currentState.effect2Time < cg.time) { // Ridah, Zombie death effect
...
void CG_AnimPlayerConditions(centity_t *cent) {
	...
	// changed this since we dont need to be careful about bandwidth anymore, and this method is much more accurate
...
void CG_Player(centity_t *cent) {
	...
	// the client number is stored in clientNum. It can't be derived from the entity number, because a single client may have multiple corpses on the level using the same clientinfo
	clientNum = cent->currentState.clientNum;
...
//	}
	...
	// setting the legs axis should pass the scale down through the other parts/tags/etc. and will need to be 'undone' for the weapon
...
//	}
	...
	// check for ladder and if you're on it, don't allow torso model rotation(so the body climbs aligned with the ladder)
...
//	}
	...
	if (!(cent->currentState.eFlags & EF_DEAD)) {
		...
		// add the current frame to the total, so when it comes to pick a new frame, we choose the average of those we missed over the last frame
...
void CG_ResetPlayerEntity(centity_t *cent) {
	...
	if (!(cent->currentState.eFlags & EF_DEAD)) {
		...
		cent->pe.legs.yawAngle = cent->nextState.apos.trBase[YAW];
		...
		cent->pe.torso.yawAngle = cent->nextState.apos.trBase[YAW];
		cent->pe.torso.yawing = qfalse;
		cent->pe.torso.pitchAngle = cent->nextState.apos.trBase[PITCH];
