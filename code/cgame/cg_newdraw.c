...
static void CG_DrawCursorhint(rectDef_t *rect) {
	...
	switch (cg.cursorHintIcon) {
		...
		case HINT_CHAIR:
			...
			if (cg.predictedPlayerState.weapon && !(WEAPS_ONE_HANDED & (1 << (cg.predictedPlayerState.weapon)))) { // this was backwards
...
static void CG_DrawSelectedPlayerWeapon(rectDef_t *rect) {
	...
	if (ci) {
		if (cg_weapons[ci->curWeapon].weaponIcon[1]) {
			CG_DrawPic(rect->x, rect->y, rect->w, rect->h, cg_weapons[ci->curWeapon].weaponIcon[1]); // using the 'selected' version of the icon
		} else {
