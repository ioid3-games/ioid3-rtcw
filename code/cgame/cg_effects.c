...
Big velocity vector with a negative acceleration for deceleration, etc. (breath could then come out of a guys mouth at the rate he's
...
#if 0 // don't play spawn in effect right now
	localEntity_t *le;
	refEntity_t *re;
...
//	vec3_t dir;
	...
	if (entityNum != cg.snap->ps.clientNum) {
		...
		for (i = 0; i < BLOOD_SPURT_COUNT; i++) {
			...
			CG_AddBloodTrails(bOrigin, ndir, 100, 250 + (int)(crandom() * 50), 3 + rand()% 2, 0.1);
		}
	}
}
...
//		le->angles.trDelta[2] = 0;
		le->angles.trDelta[2] = 400; // this is set with a very particular value to try to get it to flip exactly once before landing (based on player alive (standing) and on level ground)and will be unnecessary when I have things landing properly on their own
		le->angles.trTime = cg.time;
		le->bounceFactor = 0.2;
...
FIXME: accept the cent as parameter, and use it to grab the head position from the model TAG's
=======================================================================================================================================
*/
...
void CG_GibPlayer(centity_t *cent, vec3_t playerOrigin, vec3_t gdir) {
	...
	ci = &cgs.clientinfo[clientNum];
	// fetch the various positions of the tag_gib*'s and spawn the gibs from the correct places(especially the head)
