...
static void CG_Obituary(entityState_t *ent) {
	...
	if (attacker < 0 || attacker >= MAX_CLIENTS) {
		...
		// added mod check for machinegun (prolly mortar here too)
		switch (mod) {
			case MOD_MACHINEGUN:
	...
	if (target != attacker) {
		...
		if (attacker == cg.snap->ps.clientNum) {
			...
			switch (mod) {
				...
				case MOD_AIRSTRIKE:
					message = "was blasted by";
					message2 = "'s support fire"; // changed since it gets called for both air strikes and artillery
					break;
				case MOD_ROCKET:
					message = "ate";
					message2 = "'s rocket";
...
static void CG_UseItem(centity_t *cent) {
	...
	if (es->number == cg.snap->ps.clientNum) {
		if (!itemNum) {
#ifdef LOCALISATION
			CG_CenterPrint("No item to use", SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH);
#else
			CG_CenterPrint("noitem", SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH);
#endif
		} else {
		...
		if (cg_autoswitch.integer && cg.predictedPlayerState.weaponstate != WEAPON_RELOADING) {
			...
			if (weapon != WP_SNIPERRIFLE && weapon != WP_SNOOPERSCOPE && weapon != WP_FG42SCOPE) {
				...
				// 1 - "Always Switch" - always switch to new weap(Q3A default)
					...
					// 4 - either 2 or 3
					// 5 - both 2 and 3
					// FIXME: this works fine for predicted pickups(when you walk over the weapon), but not for manual pickups(activate item)
...
void CG_PainEvent(centity_t *cent, int health, qboolean crouching) {
	...
	if (cent->currentState.aiChar && cgs.gametype <= GT_SINGLE_PLAYER) {
		...
		// health is actually time to spend playing the animation
...
void CG_Explodef(vec3_t origin, vec3_t dir, int mass, int type, qhandle_t sound, int forceLowGrav, qhandle_t shader, int parent, qboolean damage) {
	...
	for (i = 0; i < POSSIBLE_PIECES; i++) {
		...
		for (howmany = 0; howmany < pieces[i]; howmany++) {
			...
			// debris rocks last longer in the boss map(is thee a better way of doing this at this late stage?)
			...
			if (type == 1) { // glass
				...
				// NOTE: need to look into this so that they fade out correctly
...
void CG_Effect(centity_t *cent, vec3_t origin, vec3_t dir) {
	...
	if (cent->currentState.eventParm & 1) { // fire
		...
		// sprVel is used without being set, so to be sure, I'm going to clear out the vector
		VectorClear(sprVel);
...
We should keep this separate since there will be considerable differences in the physical properties of shard vrs debris. not to
mention the fact there is no way we can quantify what type of effects the designers will potentially desire. If it is still possible to
merge the functionality of cg_shard into cg_explode at a latter time I would have no problem with that but for now I want to keep it
separate.
=======================================================================================================================================
*/
...
Since now we have a proper event for when a player dies, we could move other event logic such as obituaries for the player here.
=======================================================================================================================================
*/
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
	...
	switch (event) {
		...
		case EV_FOOTSTEP_ROOF:  // tile sound
			...
			if (cg_footsteps.integer) {
				trap_S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.footsteps[FOOTSTEP_ROOF][footstepcnt]);
			}
		...
		case EV_GRENADE_SUICIDE:
			...
			CG_MissileHitWall(WP_GRENADE_LAUNCHER, 0, position, dir, 0); // modified to send missilehitwall surface parameters
			break;
		...
		case EV_GRENADE_BOUNCE:
			...
			if (es->weapon == WP_DYNAMITE) {
				...
				} else if (flags &(SURF_METAL|SURF_ROOF|SURF_GLASS)) {
					if (rand()& 1) {
				...
				} else if (flags &(SURF_GRASS|SURF_GRAVEL|SURF_SNOW|SURF_CARPET)) {
					if (rand()& 1) {
		...
		case EV_GENERAL_SOUND:
			...
			if (!strstr(s, ".wav")) {
				...
				// try with .wav
				break; // all sounds should have extension
			}
...
//				break;
			case ET_ALARMBOX:
				// all this crap shouldn't be in here, but we don't have a generic entry_point into explosions that's reasonable. This will be an early thing to get fixed in future work
				{
					int i, j;
					vec3_t sprVel, sprOrg;
...
//		}
		...
		// as far as we could tell, for all non-TempEntities, the circular 'events' list contains the valid events. So we
		// skip processing the single 'event' field and go straight to the circular list
		goto skipEvent;
	}
