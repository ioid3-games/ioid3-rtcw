...
Leave expanding blood puffs behind gibs. Use BLOOD_PARTICLE_TRAIL to change between particle and trail code.
=======================================================================================================================================
*/
...
void CG_BloodTrail(localEntity_t *le) {
	...
	for (; t <= t2; t += step) {
		...
		if (cent && cent->currentState.aiChar == AICHAR_ZOMBIE) {
			...
			le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.bloodTrailShader, t, STYPE_STRETCH, newOrigin, 180, 1.0, 0.0, 12.0, 12.0, TJFL_NOCULL, col, col, 0, 0);
		}
#endif
	}
}
...
void CG_AddSparkElements(localEntity_t *le) {
	...
	while (1) {
		...
		// add a trail
		le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, le->refEntity.customShader, le->refEntity.origin, 200, 1.0 - lifeFrac, 0.0, lifeFrac * 2.0 * (((le->endTime - le->startTime)> 400) + 1) * 1.5, lifeFrac * 2.0 * (((le->endTime - le->startTime)> 400) + 1) * 1.5);
		// if it is in a nodrop zone, remove it
		// this keeps gibs from waiting at the bottom of pits of death and floating levels
...
void CG_AddBloodElements(localEntity_t *le) {
	...
	while (1) {
		...
		lifeFrac = (float)(cg.time - le->startTime) / (float)(le->endTime - le->startTime);
		// add a trail
		le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, cgs.media.bloodTrailShader, le->refEntity.origin, 200, 1.0 - lifeFrac, 1.0 - lifeFrac, 3.0, 5.0);
...
void CG_AddClientCritter(localEntity_t *le) {
	...
	while (time <= cg.time) {
		...
		if (le->leType == LE_ZOMBIE_SPIRIT) {
			le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.zombieSpiritTrailShader, time, STYPE_STRETCH, le->refEntity.origin, (int)le->effectWidth, 0.3 * alpha, 0.0, le->radius, 0, 0, colorWhite, colorWhite, 1.0, 1);
		}

		if (le->leType == LE_HELGA_SPIRIT && le->refEntity.hModel != cgs.media.ssSpiritSkullModel) {
			le->headJuncIndex = CG_AddTrailJunc(le->headJuncIndex, cgs.media.helgaSpiritTrailShader, time, STYPE_STRETCH, le->refEntity.origin, (int)le->effectWidth, 0.3 * alpha, 0.0, le->radius, 0, 0, colorWhite, colorWhite, 1.0, 1);
		}
		// tracking factor
...
#if 1 // flame
		if (le->effectWidth > 0) {
			le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, cgs.media.fireTrailShader, le->refEntity.origin, (int)(500.0 * (0.5 + 0.5 * (1.0 - lifeFrac))), 1.0, 0.5, 3, le->effectWidth);
#else // spark line
		if (le->effectWidth > 0) {
			le->headJuncIndex = CG_AddSparkJunc(le->headJuncIndex, cgs.media.sparkParticleShader, le->refEntity.origin, (int)(600.0 * (0.5 + 0.5 * (0.5 - lifeFrac))), 1.0 - lifeFrac * 2, 0.5 * (1.0 - lifeFrac), 5.0 * (1.0 - lifeFrac), 5.0 * (1.0 - lifeFrac));
#endif
		}
#endif
		// smoke
		if (le->effectFlags & 1) {
			le->headJuncIndex2 = CG_AddSmokeJunc(le->headJuncIndex2, cgs.media.smokeTrailShader, le->refEntity.origin, (int)(2000.0 * (0.5 + 0.5 * (1.0 - lifeFrac))), 1.0 * (trace.fraction == 1.0) * (0.5 + 0.5 * (1.0 - lifeFrac)), 1, (int)(60.0 * (0.5 + 0.5 * (1.0 - lifeFrac))));
		}
		// if it is in a nodrop zone, remove it
		// this keeps gibs from waiting at the bottom of pits of death and floating levels
