...
static void CG_DumpCastAi_f(void) {
	...
	CG_Printf("%s(%s)entity dumped to '%s'(%i %i %i).\n", aitype, ainame, aicastfilename, (int)cg.snap->ps.origin[0], (int)cg.snap->ps.origin[1], (int)cg.snap->ps.origin[2]);
}
...
static void CG_DumpCoopSpawnpoint_f(void) {
	...
	trap_FS_Write(buffptr, strlen(buffptr), f);
	trap_FS_FCloseFile(f);
	CG_Printf("coop_spawnpoint dumped to '%s'(%i %i %i).\n", entsfilename, (int)cg.snap->ps.origin[0], (int)cg.snap->ps.origin[1], (int)cg.snap->ps.origin[2]);
...
static void CG_DumpFlagPole_f(void) {
	...
	trap_FS_Write(buffptr, strlen(buffptr), f);
	trap_FS_FCloseFile(f);
	CG_Printf("coop_spawnpoint dumped to '%s'(%i %i %i).\n", entsfilename, (int)cg.snap->ps.origin[0], (int)cg.snap->ps.origin[1], (int)cg.snap->ps.origin[2] - 60);
	// draw the new spawnpoint
...
void CG_vstrDown_f(void) {
	...
	if (trap_Argc() == 5) {
		...
		CG_Printf("[cgnotify]^3Usage: ^7+vstr [down_vstr] [up_vstr]\n");
	}
}
...
static consoleCommand_t commands[] = {
	...
	{"loaddeferred", CG_LoadDeferredPlayers}, // spelling fixed
...
//	trap_AddCommand("teamtask");
	trap_AddCommand("loaddeferred"); // spelling fixed
