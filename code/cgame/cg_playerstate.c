...
void CG_Respawn(void) {
	...
	cg.holdableSelectTime = 0; // reset holdable timer
	...
	trap_Cvar_Set("cg_notebookpages", "3"); // clear notebook pages on spawn(except for pages 1 and 2)this is temporary
	trap_Cvar_Set("ui_notebookCurrentPage", "0"); // clear notebook pages on spawn(except for page 1)this is temporary
}
...
void CG_CheckPlayerstateEvents(playerState_t *ps, playerState_t *ops) {
	...
	// go through the predictable events buffer
	for (i = ps->eventSequence - MAX_EVENTS; i < ps->eventSequence; i++) {
		// if we have a new predictable event or the server told us to play another event instead of a predicted event we already issued or something the server told us changed our prediction causing a different event
