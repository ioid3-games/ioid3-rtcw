...
void CG_PositionEntityOnTag(refEntity_t *entity, const refEntity_t *parent, char *tagName, int startIndex, vec3_t *offset) {
	...
	// not sure why this was here.. causes jittery torso animation, since the torso might have different frame/oldFrame
...
#define FLY_JUMP 300
...

	{
		...
		// FIXME: origin of md3 is offset from center. need to center the origin when you toss it
...
void CG_NextItem_f(void) {
	...
	cg.weaponSelectTime = 0; // clear weapon selection drawing
	...
	for (i = 0; i < HI_NUM_HOLDABLE; i++) {
		...
		if (cg.predictedPlayerState.holdable[next]) {
			break;
		}
	}
...
qboolean CG_PlayerSeesItem(playerState_t *ps, entityState_t *item, int atTime, int itemType) {
	...
	vorigin[2] += ps->viewheight; // get the view loc up to the viewheight and subtract the item's offset(that is used to place it on the ground)
...
static void CG_Item(centity_t *cent) {
	...
	// The dropammo command can drop items that where not in the map, so the server doesn't tell the client to preload them to overcome this problem, lets load the model now
	CG_RegisterItemVisuals(item - bg_itemlist);
...
static void CG_Missile(centity_t *cent) {
	...
	VectorCopy(cent->lerpOrigin, ent.origin);
	VectorCopy(cent->lerpOrigin, ent.oldorigin);
	// removed plasma gun code as sp5 is taking that spot
...
A bat now is actually a spirit.
=======================================================================================================================================
*/
...
static void CG_Mover(centity_t *cent) {
	...
	// I'd like to do this as an entity flag(EF_SPARKING or something)and have it randomly
...
void CG_FlamethrowerProp(centity_t *cent) {
	...
	// this first one doesn't seem to do anything?
