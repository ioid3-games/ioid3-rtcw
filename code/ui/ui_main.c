...
void Text_Paint(float x, float y, int font, float scale, vec4_t color, const char *text, float adjust, int limit, int style) {
	...
	if (text) {
		...
		while (s && *s && count < len) {
			...
			//glyph = &fnt->glyphs[index]; // this needs to be an unsigned cast for localization
...
Generic routine for pretty much all status indicators that show a fractional value to the palyer by virtue of how full a drawn box is.
...
//		flags|=1; // BAR_LEFT (left, when vertical means grow 'up')
	...
	flags |= 16; // - draw the filled contrast box
...
static int QDECL UI_SavegamesQsortCompare(const void *arg1, const void *arg2) {
	...
	if (uiInfo.savegameStatus.sortKey == SORT_SAVENAME) {
		...
		// better way to do this?(i was adding up seconds, but that seems slower than a bunch of comparisons)
		i = sg->tm.tm_year;
		j = sg2->tm.tm_year;
...
NOTE: VOTEFLAGS_RESTART for map_restart vote is present and can be used, but doesn't have any UI binding.
=======================================================================================================================================
*/
...
#ifdef __APPLE__
			// currently its not possible to spawn a dedicated server on mac through the menus. You need to use Terminal.app to execute the dedicated binary
				...
				*/
				...
				if ((strlen(buf)) && (Q_stricmp(buf,";"))) {
					...
					if (UI_Cvar_VariableString("com_errorDiagnoseIP")[0]) {
						...
						if (missing_files[0]) {
							trap_Cvar_Set("com_errorMessage", va("%s\n\n%s\n%s", UI_Cvar_VariableString("com_errorMessage"), trap_TranslateString(MISSING_FILES_MSG), missing_files));
						}
					}
					*/
					Menus_ActivateByName("error_popmenu", qtrue);
