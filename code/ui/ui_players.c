...
static void UI_TorsoSequencing(playerInfo_t *pi) {
	...
	if (currentAnim == TORSO_RAISE || currentAnim == TORSO_RAISE2 || currentAnim == TORSO_RAISE3 || currentAnim == TORSO_RAISE4 || currentAnim == TORSO_RAISE5) {
		UI_SetTorsoAnim(pi, TORSO_STAND);
		return;
	}
}
...
//	Com_sprintf(filename, sizeof(filename), "models/players/%s/upper.md3", modelName);
	...
	// load the animations
	// changing name of config file to avoid backwards or alternate compatibility confustion
