...
// launch airstrike as line of bombs mostly-perpendicular to line of grenade travel (close air support should *always* drop parallel to friendly lines, tho accidents do happen)
...
void weapon_callAirStrike(gentity_t *ent) {
	...
	pos[0] = 0;
	pos[1] = 0;
	pos[2] = crandom(); // generate either up or down vector, which adds randomness to pass direction below
...
Modified so it doesn't have trouble with negative locations (quadrant problems). This was causing some problems with bullet marks
appearing since snapping too far off the target surface causes the the distance between the transmitted impact point and the actual hit
surface larger than the mark radius (so nothing shows).
=======================================================================================================================================
*/
...
Mechanism allows different weapon damage for single/multiplayer; we want "balanced" weapons in multiplayer but don't want to alter the
existing single-player damage items that have already been changed.
...
int G_GetWeaponDamage(int weapon) {
	switch (weapon) {
		...
		case WP_AKIMBO:
			return 8;
		case WP_VENOM:
			return 12; // slight modify for DM
		case WP_MP40:
			return 6;
		case WP_THOMPSON:
			return 8;
...
Wrote this so we can dynamically switch between old and new values while testing g_userAim.
=======================================================================================================================================
*/
...
#define SNIPER_DAMAGE G_GetWeaponDamage(WP_SNIPERRIFLE)
#define SNOOPER_SPREAD G_GetWeaponSpread(WP_SNOOPERSCOPE)
#define SNOOPER_DAMAGE G_GetWeaponDamage(WP_SNOOPERSCOPE)
...
void Cross_Fire(gentity_t *ent) {
	// temporarily use the zombie spit effect to check working state
...
TODO: find all targets in the client's view frame, and lock onto them all, applying damage and telling all clients to draw the
appropriate effects.
=======================================================================================================================================
*/
...
void Bullet_Endpos(gentity_t *ent, float spread, vec3_t *end) {
	...
	r = crandom() * spread;
	u = crandom() * spread;
	// if this is an AI shooting, apply their accuracy
...
void Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker, vec3_t start, vec3_t end, float spread, int damage, int recursion) {
	...
	// abort if too many recursions.. there must be a real solution for this, but for now this is the safest fix I can find
	if (recursion > 12) {
		return;
	}
	...
	// changed so player could shoot his own dynamite
	// whoops, but that broke bullets going through explosives...
...
/*
=======================================================================================================================================
weapon_grenadelauncher_fire_coop
=======================================================================================================================================
*/
...
gentity_t *weapon_grenadelauncher_fire(gentity_t *ent, int grenType) {
	...
	if ((ent->s.apos.trBase[0] > 0) && (grenType != WP_GRENADE_SMOKE)) { // smoke grenades always overhand
		underhand = qtrue;
	}
...
//			break;
	...
	// actually, this is sort of moot right now since you're not allowed to fire when leaning. Leave in in case we decide to enable
	// some lean-firing
...
void CalcMuzzlePointForActivate(gentity_t *ent, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint) {
	...
	// snap to integer coordinates for more efficient network bandwidth usage
	//SnapVector(muzzlePoint);
	// only used server-side, so leaving the accuracy in is fine (and means things that show a cursorhint will be hit when activated)
...
void CalcMuzzlePoints(gentity_t *ent, int weapon) {
	...
	// modified the muzzle stuff so that weapons that need to fire down a perfect trace straight out of the camera (SP5, Mauser right now)
	// can have that accuracy, but weapons that need an offset effect(bazooka/grenade/etc.)can still look like they came out of the weap
	CalcMuzzlePointForActivate(ent, forward, right, up, muzzleTrace);
...
void FireWeapon(gentity_t *ent) {
	...
	switch (ent->s.weapon) {
		...
		case WP_SNIPERRIFLE:
			...
			// added muzzle flip in multiplayer
			if (!ent->aiCharacter) {
//				if (g_gametype.integer != GT_SINGLE_PLAYER) {
				VectorCopy(ent->client->ps.viewangles,viewang);
