...
=======================================================================================================================================
*/
void G_MissileImpact(gentity_t *ent, trace_t *trace, int impactDamage, vec3_t dir) {
	gentity_t *other;
	...
	if (other->classname && Q_stricmp(other->classname, "func_explosive") == 0) {
		...
		if (other->health && impactDamage >= other->health) {
			// check for other->takedamage needs to be inside the health check since it is likely that, if successfully destroyed by the missile, in the next runmissile()
			// update takedamage would be set to '0' and the func_explosive would not be removed yet, causing a bounce
			if (other->takedamage) {
				BG_EvaluateTrajectoryDelta(&ent->s.pos, level.time, velocity);
				G_Damage(other, ent, &g_entities[ent->r.ownerNum], velocity, ent->s.origin, impactDamage, 0, ent->methodOfDeath);
			}
			// its possible of the func_explosive not to die from this and it should reflect the missile or explode it not vanish into oblivion
			if (other->health <= 0) {
...
Shake the player, caused by explosives (grenades/dynamite/etc.).
=======================================================================================================================================
*/
...
void Shaker_think(gentity_t *ent) {
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		Com_sprintf(cmd, sizeof(cmd), "shake %.4f", bounceamt);
...
void G_RunSpit(gentity_t *ent) {
	...
	if (rand()% 100 > 60) {
		...
		trap_Trace(&tr, ent->r.currentOrigin, NULL, NULL, end, ent->r.ownerNum, MASK_SHOT);
...
NOTE!!!! NOTE!!!!!
This accepts a /non-normalized/ direction vector to allow specification of how hard it's thrown. Please scale the vector before calling.
=======================================================================================================================================
*/
...
gentity_t *fire_rocket(gentity_t *self, vec3_t start, vec3_t dir) {
	...
	if (g_gametype.integer > GT_SINGLE_PLAYER) {
		...
		if (self->aiCharacter) {
			...
			// trying a bit more speed in SP for player rockets
...
gentity_t *fire_mortar(gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	bolt->damage = G_GetWeaponDamage(WP_MORTAR);
	bolt->splashDamage = G_GetWeaponDamage(WP_MORTAR);
	bolt->splashRadius = 120;
