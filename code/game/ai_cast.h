...
typedef enum {
	...
	ORDERSSOUNDSCRIPT,
	DEATHSOUNDSCRIPT,
	QUIETDEATHSOUNDSCRIPT, // ADDED FOR SILENT DEATHS (SNIPER/KNIFE)
...
typedef struct cast_state_s {
	...
	// Cast specific information follows. Add to this as needed, this way the bot_state_t structure remains untouched
	int aiCharacter;
	int aiFlags;
	...
	// !!! NOTE: make sure any entityNum type variables get initialized to -1 in AICast_CreateCharacter(), or they'll be defaulting to the player(index 0)
	// goal/AI stuff
	int followEntity;
	float followDist;
	...
	int lastGetHidePos;
	int startAttackCount;		// incremented each time we start a standing attack, used to make sure we only find a combat spot once per attack
