...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath) {
	...
	// don't print this in coop play, we just respawn mission should fail if we both die
	if (g_gametype.integer == GT_SINGLE_PLAYER) {
...
qboolean IsHeadShot(gentity_t *targ, gentity_t *attacker, vec3_t dir, vec3_t point, int mod) {
	...
	if (head_shot_weapon) {
		...
		VectorSet(head->r.mins, -6, -6, -6); // changed this z from -12 to -6 for crouching, also removed standing offset
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage, int dflags, int mod) {
	...
	// track pain for player. This is used by AI to determine how long it has been since their enemy was injured
	if (attacker) { // whoops, for falling damage there's no attacker
	...
	if (IsHeadShot(targ, attacker, dir, point, mod)) {
		...
		if (g_gametype.integer <= GT_SINGLE_PLAYER) {
			...
			// yes, headshotDamageScale of 0 gives no damage, thats because the bullet hit the head which is fully protected
	...
	if (take) {
		...
		if (mod == MOD_TELEFRAG || mod == MOD_WATER || mod == MOD_FALLING || mod == MOD_SLIME || mod == MOD_LAVA || mod == MOD_TRIGGER_HURT
			...
			if (attacker->client && targ->client) {
				...
				// from axis team to a an allies or a neutral or from allies team to a nazi or monster
				if ((attacker->client->sess.sessionTeam == TEAM_BLUE && (targ->aiTeam == AITEAM_NAZI || targ->aiTeam == AITEAM_MONSTER)) || (attacker->client->sess.sessionTeam == TEAM_RED && (targ->aiTeam == AITEAM_ALLIES || targ->aiTeam == AITEAM_NEUTRAL))) {
					targ->health = targ->health - take;
				}
			}
		} else {
		...
		if (attacker->health <= 0 && attackerpain) {
			...
			if (attacker->die) { // mg42 doesn't have die func(FIXME)
