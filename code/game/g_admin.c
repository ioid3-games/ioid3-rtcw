...
Remove case and control characters. Ripped from g_cmds.c.
=======================================================================================================================================
*/
...
void cmd_do_login(gentity_t *ent, qboolean silent) {
	...
	if ((Q_stricmp(str, a1_pass.string) == 0) || (Q_stricmp(str, a2_pass.string) == 0) || (Q_stricmp(str, a3_pass.string) == 0)) {
		...
		if (silent) {
			...
			// log it
			log = va("Player %s(IP:%i.%i.%i.%i)has silently logged in as %s.", ent->client->pers.netname, ent->client->sess.ip[0], ent->client->sess.ip[1], ent->client->sess.ip[2], ent->client->sess.ip[3], tag);
			logEntry(ADMLOG, log);
		} else {
			trap_SendServerCommand(-1, va("chat \"console: %s ^7has logged in as %s^7!\n\"", ent->client->pers.netname, tag));
			// log it
			log = va("Player %s(IP:%i.%i.%i.%i)has logged in as %s.", ent->client->pers.netname, ent->client->sess.ip[0], ent->client->sess.ip[1], ent->client->sess.ip[2], ent->client->sess.ip[3], tag);
			logEntry(ADMLOG, log);
		}
		...
		trap_SendServerCommand(ent - g_entities, va("print \"Incorrect password^1!\n\""));
		// log it
		log = va("Player %s(IP:%i.%i.%i.%i)has tried to login using password: %s", ent->client->pers.netname, ent->client->sess.ip[0], ent->client->sess.ip[1], ent->client->sess.ip[2], ent->client->sess.ip[3], str);
...
void logEntry(char *filename, char *info) {
	...
	varLine = va("Time: %02d:%02d:%02d/%02d %s %d : %s \n", ct.tm_hour, ct.tm_min, ct.tm_sec, ct.tm_mday, cMonths[ct.tm_mon], 1900 + ct.tm_year, info);
...
Basically copy-paste from g_cmds.c - with a tweak for bots as I'm to lazy to check if it would break anything there..
=======================================================================================================================================
*/
...
qboolean IPv4Valid(char *s) {
	...
	c = sscanf(s, "%3u.%3u.%3u.%3u%s", &d[0], &d[1], &d[2], &d[3], vrfy);
