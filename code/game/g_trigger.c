...
"life" time this brush will exist if value is zero will live for ever ei 0.5 sec 2.sec default is zero.
...
void trigger_aidoor_stayopen(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (other->client && other->health > 0) {
		...
		if (other->r.svFlags & SVF_CASTAI) {
			...
			if ( (door->grenadeFired > level.time) || ( !((door->activator == other) && (door->moverState != MOVER_POS1) && (door->moverState != MOVER_POS1ROTATE)) && (door->moverState != MOVER_POS2ROTATE) && (door->moverState != MOVER_POS2))) {
...
/*QUAKED SP_trigger_aidoor(.5 .5 .5)?
...
/*QUAKED SP_trigger_objective_info(.5 .5 .5) ? AXIS_OBJECTIVE ALLIED_OBJECTIVE
