...
void grabber_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker, int damage, int mod) {
	// FIXME: this is buggy. the trigger brush entity (ent->enemy )does not free. Need to fix.
...
ent calling this is the bounding box for the grabber, not the grabber ent itself. The grabber ent is 'ent->parent'.
=======================================================================================================================================
*/
...
Use func for the grabber entity. If not awake, allow waking by trigger. If awake, allow attacking by trigger.
=======================================================================================================================================
*/
...
void SP_shooter_tesla(gentity_t *ent) {
	...
	// it's a shooter_ since it will act like any other shooter, except the tesla is a client-side effect that reports damage back to the
	// game, so this will create an linked-entity on the client that acts as dictated and, if necessary, passes back damage info like the
	// weapon. this will keep the server->client messages to a minimum (start firing/stop firing) rather than sending an event for each bolt.
...
// view position is set on the client-side to keep it firm behind the gun with interpolation
...
void mg42_spawn(gentity_t *ent) {
	...
	//if(!(ent->spawnflags & 2)) // no tripod
	{
		...
		base->die = mg42_die;
		base->soundPos3 = ent->soundPos3;
		base->activateArc = ent->activateArc;
	...
	gun->s.eType = ET_MG42;
	// don't need to specify here, handled in G_CheckForCursorHints
