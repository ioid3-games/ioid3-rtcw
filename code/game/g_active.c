...
//		G_Printf("serverTime >>>>>\n");
	...
	// do some time-based muzzle flip -- this never gets touched in single player(see g_weapon.c)
...
*/
	...
	// drop button drops secondary weapon so new one can be picked up
...
//	}
	...
	if (pm.ps->pm_type == PM_DEAD) {
		...
		// EF_DEAD is checked for in Pmove functions, but wasn't being set until after Pmove
...
//		pm.noFootsteps = qtrue;
	...
	// allow AI Cast's to evaluate results of their pmove's (don't do this in multiplayer)
	if (g_gametype.integer <= GT_SINGLE_PLAYER) {
	...
	if (!(ent->r.svFlags & SVF_CASTAI)) {
		...
		if (client->ps.stats[STAT_HEALTH] <= 0) {
			...
			if (level.time > client->respawnTime) {
				// DHM - Nerve :: Single player game respawns immediately as before, but in multiplayer, require button press before respawn
				...
				} else if ((ucmd->buttons & (BUTTON_ATTACK|BUTTON_USE_HOLDABLE)) && (!(ent->client->ps.pm_flags & PMF_LIMBO))) {
					ClientRespawn(ent);
				}
			}
...
void SpectatorClientEndFrame(gentity_t *ent) {
	...
	if ((ent->client->sess.spectatorState == SPECTATOR_FOLLOW) || (ent->client->ps.pm_flags & PMF_LIMBO)) { // for limbo
		int clientNum;

		// check if it's time to respawn limbo players back into game
