...
// changed this for more realistic angles (at the cost of more network traffic?)
#if 1
	float speed;
	vec3_t moved;
	int moveyaw;
...
static qboolean PM_CheckJump(void) {
	...
	if (pm->cmd.serverTime - pm->ps->jumpTime < 500) { // trying shorter time. I find this effect annoying
		return qfalse;
	}
...
static void PM_CrashLand(void) {
	...
	// only play this if coming down hard
	if (!pm->ps->legsTimer) {
...
#define AI_STEPTEST_FALLDIST_PER_SEC (STEPSIZE + pm->ps->gravity * 0.35) // always allow them to fall down the minimum stepsize
	if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
		...
		pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, point, pm->ps->clientNum, pm->tracemask);
		// try and keep AI's on the ground if walking down steps
		if (pm->ps->aiChar) {
...
static void PM_CheckDuck(void) {
	...
	// disable crouching while using MG42
...
static void PM_BeginWeaponReload(int weapon) {
	...
	// okay to reload while overheating without tacking the reload time onto the end of the current weaponTime (the reload time is partially absorbed into the overheat time)
...
static void PM_BeginWeaponChange(int oldweapon, int newweapon, qboolean reload) { // modified to play 1st person alt-mode transition animations.
	...
	if (!pm->ps->aiChar && !(pm->ps->eFlags & EF_DEAD) && (newweapon == WP_NONE)) { // dont allow changing to null weapon
		return;
	}
...
void PM_CheckForReload(int weapon) {
	...
	// when the weapon is empty, it will switch away to the primary (so player can see results of last shot through scope without it auto-switching away for the reload)
	if (!pm->ps->aiChar) { // added this check since this confuses the AI
		switch (weapon) {
			case WP_SNOOPERSCOPE:
			case WP_SNIPERRIFLE:
			case WP_FG42SCOPE:
...
#define AIMSPREAD_VIEWRATE_RANGE 120.0f // degrees per second
	...
	switch (pm->ps->weapon) {
		...
		case WP_VENOM:
			...
			break;
		case WP_SNIPERRIFLE: // looong time to recover
			wpnScale = 10.0f;
			break;
		case WP_SNOOPERSCOPE: // looong time to recover
			wpnScale = 8.0f;
			break;
		case WP_MAUSER:
			wpnScale = 0.5f;
...
static void PM_Weapon(void) {
	...
	// remoed this, was preventing lava from hurting player
	//pm->watertype = 0;
	...
	if (pm->ps->weapon == WP_GRENADE_LAUNCHER || pm->ps->weapon == WP_GRENADE_PINEAPPLE || pm->ps->weapon == WP_DYNAMITE) {
		// AI's don't set grenadeTimeLeft on +attack, so I don't check for(pm->ps->aiChar)here
	...
	switch (pm->ps->weapon) {
		...
		case WP_COLT:
			...
			break;
		case WP_AKIMBO:
			// if you're firing an akimbo colt, and your other gun is dry, nextshot needs to take 2x time
			addTime = ammoTable[pm->ps->weapon].nextShotTime;
		...
		case WP_GARAND:
			...
			break;
		case WP_SNIPERRIFLE: // not so much added per shot. these weapons mostly uses player movement to get out of whack
			addTime = ammoTable[pm->ps->weapon].nextShotTime;
			aimSpreadScaleAdd = 20;
			break;
		case WP_SNOOPERSCOPE:
...
/*
	if (pm->cmd.buttons & BUTTON_GESTURE) {
		if (pm->ps->torsoTimer == 0) {
			...
			if (!pm->ps->aiChar) { // we'll play a custom sound upon calling the Taunt
...
void PM_UpdateLean(playerState_t *ps, usercmd_t *cmd, pmove_t *tpm) {
	...
	// not allowed when...
	if ((ps->eFlags &(EF_MELEE_ACTIVE|EF_MG42_ACTIVE|EF_FIRING))) {
		leaning = 0;
	}
...
void PM_LadderMove(void) {
	...
	VectorNormalize(pml.forward);
	VectorNormalize(pml.right);
	// move depending on the view, if view is straight forward, then go up if view is down more then X degrees, start going down
...
void PM_Sprint(void) {
	...
	if ((pm->cmd.buttons & BUTTON_SPRINT) && (pm->cmd.forwardmove || pm->cmd.rightmove) && !(pm->ps->pm_flags & PMF_DUCKED) && (!pm->waterlevel)) {
		if (pm->ps->powerups[PW_NOFATIGUE]) { // take time from powerup before taking it from sprintTime
			...
			pm->ps->sprintTime += 10; // go ahead and continue to recharge stamina at double rate with stamina powerup even when exerting
			...
			// dont drain sprintTime if not moving
			if (VectorLength(pm->ps->velocity) > 128) { // check for a bit more movement
				pm->ps->sprintTime -= 50;
			}
		}
...
void PmoveSingle(pmove_t *pmove) {
	...
	if (!isDummy) {
		if (!(pm->ps->pm_flags & PMF_LIMBO)) {
			PM_UpdateViewAngles(pm->ps, &pm->cmd, pm->trace);
		}
	}
	...
	if (pm->ps->pm_type >= PM_DEAD || pm->ps->pm_flags & PMF_LIMBO) {
		pm->cmd.forwardmove = 0;
		pm->cmd.rightmove = 0;
		pm->cmd.upmove = 0;
	}

	if ((pm->ps->pm_type == PM_SPECTATOR) || (pm->ps->pm_flags & PMF_LIMBO)) { // for limbo mode chase
		PM_CheckDuck();
		PM_FlyMove();
		PM_DropTimers();
		return;
	...
	if (pm->ps->powerups[PW_FLIGHT]) {
		...
		PM_FlyMove();
		// removed grapple flag since it's not used
#if 0
