...
void limbo(gentity_t *ent, qboolean makeCorpse) {
	...
	if (!(ent->client->ps.pm_flags & PMF_LIMBO)) {
		...
		for (i = 0; i < level.maxclients; i++) {
			if (level.clients[i].ps.pm_flags & PMF_LIMBO && level.clients[i].sess.spectatorClient == ent->s.number) {
...
void ClientRespawn(gentity_t *ent) {
	...
	// if single player, reload the last saved game for this player, no map_restart for coop games?
...
void SetWolfSpawnWeapons(gclient_t *client) {
	...
	int starthealth = 100,i,numMedics = 0;
...
// jpw
		switch (client->sess.playerWeapon) {
			...
			case 9: // WP_VENOM
				...
				// may not keep this, but put it in to test
...
Otherwise, set the modelInfo pointer to the first free slot. Returns qtrue if existing model found, qfalse otherwise.
=======================================================================================================================================
*/
...
void SaveIP(gclient_t *client, char *sip) {
	...
	sscanf(sip, "%3i.%3i.%3i.%3i", (int *)&client->sess.ip[0], (int *)&client->sess.ip[1], (int *)&client->sess.ip[2], (int *)&client->sess.ip[3]);
	return;
}
#endif
...
void ClientUserinfoChanged(int clientNum) {
	...
	// added this for head separation
	char head[MAX_QPATH];
	...
	// modified these for head separation
	...
	if (!(ent->r.svFlags & SVF_CASTAI)) {
		...
		team = (client->sess.sessionTeam == TEAM_RED) ? "Axis": ((client->sess.sessionTeam == TEAM_BLUE) ? "Allied" : "Spectator");
		// half of stuff is garbage for end user so only essentials..
		s = va("name\\%s\\team\\%s\\IP\\%i.%i.%i.%i", client->pers.netname, team, client->sess.ip[0],client->sess.ip[1], client->sess.ip[2], client->sess.ip[3]);
#endif
...
//	client->ps.sprintSpeedScale = 1.20;
	client->ps.sprintSpeedScale = 1.1; // trying new value
