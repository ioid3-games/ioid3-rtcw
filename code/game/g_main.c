...
vmCvar_t g_debugAudibleEvents;
vmCvar_t g_headshotMaxDist;
...
cvarTable_t gameCvarTable[] = {
	...
	// gameskill
	{&g_gameskill, "g_gameskill", "2", CVAR_SERVERINFO|CVAR_LATCH, 0, qfalse}, // new default '2'(was '1')
...
Non-AI's check for cursor hint contacts server-side because there's info we want to show that the client just doesn't know about (health
or other info of an explosive, invisible_users, items, etc.). traceEnt is the ent hit by the trace, checkEnt is the ent that is being
...
void G_CheckForCursorHints(gentity_t *ent) {
	...
	// OTHER ENTITIES
		...
		checkEnt = traceEnt;
		// check invisible_users first since you don't want to draw a hint based on that ent, but rather on what they are targeting
...
//					hintDist = CH_BREAKABLE_DIST;
				...
				if (!Q_stricmp(checkEnt->classname, "func_door_rotating")) {
					// stationary/closed, toggle door that's open
					if (checkEnt->moverState == MOVER_POS1ROTATE || (checkEnt->moverState == MOVER_POS2ROTATE && checkEnt->flags & FL_TOGGLE)) {
					...
					// stationary/closed, toggle door that's open
					if (checkEnt->moverState == MOVER_POS1 || (checkEnt->moverState == MOVER_POS2 && checkEnt->flags & FL_TOGGLE)) {
						hintDist = CH_DOOR_DIST;
						hintType = HINT_DOOR;
						// locked
						if (checkEnt->key >= KEY_LOCKED_TARGET) {
		...
		for (j = i + 1, e2 = e + 1; j < level.num_entities; j++, e2++) {
			...
			if (!strcmp(e->team, e2->team)) {
				...
				if (e2->targetname) {
					...
					// note to self: added this because of problems pertaining to keys and double doors
...
void G_InitGame(int levelTime, int randomSeed, int restart) {
	...
	G_RegisterCvars();
	// g_random is a value from 1 - 10 (1 and 10 included) is used in the scripting to randomize entity locations, and more
	r = (rand()% 10) + 1;
	...
	*/
	...
	if (g_gametype.integer <= GT_SINGLE_PLAYER) {
		...
		// moved this down so that it only happens in SinglePlayer games
		AICast_Init();
		AICast_ScriptLoad();
...
void MOTD(void) {
	...
	// it will render first message blank on server spawn but it's not a problem for map changes as then it falls in the loop along the process.. :)
...
void G_RunFrame(int levelTime) {
	...
	if (!level.latchGametype && g_gamestate.integer == GS_PLAYING && ((g_gametype.integer == GT_WOLF && (worldspawnflags & NO_GT_WOLF)) || (g_gametype.integer == GT_WOLF_STOPWATCH && (worldspawnflags & NO_STOPWATCH)) || ((g_gametype.integer == GT_WOLF_CP || g_gametype.integer == GT_WOLF_CPH) && (worldspawnflags & NO_CHECKPOINT)))) { // added CPH
	...
	for (i = 0; i < level.num_entities; i++, ent++) {
		...
		// let the server know about bbox or capsule collision
		...
		if (ent->s.eType == ET_MISSILE || ent->s.eType == ET_FLAMEBARREL || ent->s.eType == ET_FP_PARTS || ent->s.eType == ET_FIRE_COLUMN || ent->s.eType == ET_FIRE_COLUMN_SMOKE || ent->s.eType == ET_EXPLO_PART || ent->s.eType == ET_RAMJET) {
			G_RunMissile(ent);
			continue;
		}
		...
		if (i < MAX_CLIENTS) {
			if (newSpawns && !(ent->r.svFlags & SVF_CASTAI) && !(ent->client->ps.eFlags & EF_DEAD) && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
				if (!(ent->client->cameraPortal) && ent->client->sess.sessionTeam == TEAM_BLUE) {
					...
					// don't save a spawnpoint if the player is not on the ground or on a mover
