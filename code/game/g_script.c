...
/*
Scripting that allows the designers to control the behaviour of entities according to each different scenario.
*/
...
void G_Script_ScriptParse(gentity_t *ent) {
	...
	//char params[MAX_QPATH]; // some of our multiplayer script commands have longer parameters
...
void script_mover_blocked(gentity_t *ent, gentity_t *other) {
	...
	// FIXME: we could have certain entities stop us, thereby "pausing" movement until they move out the way
	// then we can just call the GotoMarker() again, telling it that we are just now calling it for the
	// first time, so it should start us on our way again (theoretically speaking)
