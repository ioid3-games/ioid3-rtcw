...
static void G_AdjustSingleClientPosition(gentity_t *ent, int time) {
	...
	if (i != ent->client->topMarker) {
		float frac = (float)(time - ent->client->clientMarkers[i].time) / (float)(ent->client->clientMarkers[j].time - ent->client->clientMarkers[i].time);
...
void G_AdjustClientPositions(gentity_t *ent, int time, qboolean forward) {
	...
	for (i = 0; i < level.numConnectedClients; i++, list++) {
		...
		if (list->client && list->inuse && (list->aiTeam >= 0) && (list != ent) && list->r.linked && (list->health > 0) && !(list->client->ps.pm_flags & PMF_LIMBO) && (list->client->ps.pm_type == PM_NORMAL)) {
			if (forward) {
...
void G_AttachBodyParts(gentity_t *ent) {
	...
	for (i = 0; i < level.numConnectedClients; i++, list++) {
		list = g_entities + level.sortedClients[i];
		// Gordon: ok lets test everything under the sun
		if (list->inuse && (list->aiTeam >= 0) && (list != ent) && list->r.linked && (list->health > 0) && !(list->client->ps.pm_flags & PMF_LIMBO) && (list->client->ps.pm_type == PM_NORMAL)) {
