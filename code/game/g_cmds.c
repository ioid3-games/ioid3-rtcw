...
#else
		Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i", level.sortedClients[i], g_entities[level.sortedClients[i]].client->ps.persistant[PERS_SCORE], ping, (level.time - cl->pers.enterTime) / 60000, scoreFlags, g_entities[level.sortedClients[i]].s.powerups);
#endif
		j = strlen(entry);
...
void Cmd_Give_f(gentity_t *ent) {
	...
	if (give_all || Q_stricmp(name, "weapons") == 0) {
		// we really don't want to give anything beyond WP_DYNAMITE
	...
	// "give allammo <n>" allows you to give a specific amount of ammo to /all/ weapons while allowing "give ammo <n>" to only give to the selected weap
...
=======================================================================================================================================
*/
void SetWolfData(gentity_t *ent, char *ptype, char *weap, char *pistol, char *grenade, char *skinnum) {
	gclient_t *client;
...
void StopFollowing(gentity_t *ent) {
	...
	// divert behaviour if TEAM_SPECTATOR, moved the code from SpectatorThink to put back into free fly correctly (I am not sure this can be called in non-TEAM_SPECTATOR situation, better be safe)
	if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) {
...
void Cmd_Vote_f(gentity_t *ent) {
	...
	// a majority will be determined in G_CheckVote, which will also account for players entering or leaving
}
...
void Cmd_Teleport_f(gentity_t *ent) {
	...
	if (foundPlayer) { // teleport to that player his personal spawnpoint
		...
		for (i = 0; i < level.numConnectedClients; i++) {
			...
			if (!(newent->r.svFlags & SVF_CASTAI) && count <= MAX_COOP_CLIENTS && newent != ent && newent->client && newent->client->hasCoopSpawn) {
...
void Cmd_Activate_f(gentity_t *ent) {
	...
	if (traceEnt->classname) {
		...
		if (((Q_stricmp(traceEnt->classname, "func_door") == 0) || (Q_stricmp(traceEnt->classname, "func_door_rotating") == 0))) {
			...
			if (traceEnt->touch) {
				if (ent->client->pers.autoActivate == PICKUP_ACTIVATE) {
					ent->client->pers.autoActivate = PICKUP_FORCE; // force the pickup of a normally autoactivate only item
				}
			...
			if (!ent->active && traceEnt->takedamage) { // not a dead gun
				// dont allow activating MG42 if crouching
		...
		} else if ((Q_stricmp(traceEnt->classname, "misc_flak") == 0) && traceEnt->active == qfalse) {
...
int Cmd_WolfKick_f(gentity_t *ent) {
	...
	if (!ent->melee) { // because we dont want you to open a door with a prop
		if ((Q_stricmp(traceEnt->classname, "func_door_rotating") == 0) && (traceEnt->s.apos.trType == TR_STATIONARY && traceEnt->s.pos.trType == TR_STATIONARY) && traceEnt->active == qfalse) {
			...
			if (traceEnt->key > KEY_NONE && traceEnt->key < KEY_NUM_KEYS) {// door requires key
	...
	if (traceEnt->takedamage && traceEnt->client) {
		...
		if (traceEnt->r.contents >= 0 && (traceEnt->r.contents & CONTENTS_TRIGGER) && !solidKick) {
			tent = G_TempEntity(tr.endpos, EV_WOLFKICK_MISS); // don't play the "hit" sound if you kick most triggers
		} else {
			tent = G_TempEntity(tr.endpos, EV_WOLFKICK_HIT_WALL);
		}
