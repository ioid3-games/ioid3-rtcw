...
/*
	...
	(*aasworld).areaupdate = (aas_routingupdate_t *)AAS_RoutingGetMemory(maxreachabilityareas * sizeof(aas_routingupdate_t));
*/
		...
		for (i = 0; i < numreach; i++, reach++) {
			...
			if ((*aasworld).areavisibility[nextareanum]) {
				...
				if (AAS_AreaVisible(enemyarea, nextareanum)) { // now last of all, check that this area is a good attacking spot
					if (((*aasworld).visCache[nextareanum] == 2) || (!(*aasworld).visCache[nextareanum] && (count += 10) && botimport.AICast_CheckAttackAtPos(srcnum, enemynum, (*aasworld).areawaypoints[nextareanum], qfalse, qfalse))) { // we are about to use lots of CPU time
						(*aasworld).visCache[nextareanum] = 2;
