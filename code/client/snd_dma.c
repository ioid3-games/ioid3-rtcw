...
cvar_t *s_mixahead;
cvar_t *s_mixPreStep;
cvar_t *s_mute; // for DM so he can 'toggle' sound on/off without disturbing volume levels
cvar_t *s_wavonly;
...
channel_t *S_ChannelMalloc(void) {
	...
	// be careful not to lose our freelist
...
Validates the parms and queues the sound up. If pos is NULL, the sound will be dynamically sourced from the entity.
...
void S_Base_StartSoundEx(vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle, int flags) {
	...
	// we have lots of NULL sounds using up valuable channels, so just ignore them
	...
	// make the call now, or else we could override following streaming sounds in the same frame, due to the delay
...
static void S_OpenBackgroundStream(const char *filename) {
	...
	Cvar_Set("s_currentMusic", s_backgroundLoop); // so the savegame will have the right music
}
