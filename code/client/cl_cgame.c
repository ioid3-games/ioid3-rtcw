...
static uiMenuCommand_t CL_GetUIMenuCommand(const char *type) {
	...
	if (!Q_stricmp(type, "briefing")) {
		return UIMENU_BRIEFING;
	} else if (!Q_stricmp(type, "UIMENU_WM_PICKTEAM")) {
...
#endif
	} else if (!Q_stricmp(type, "UIMENU_WM_LIMBO")) {
		return UIMENU_WM_LIMBO;
	} else if (!Q_stricmp(type, "hbook1")) {
		return UIMENU_BOOK1;
	} else if (!Q_stricmp(type, "hbook2")) {
		return UIMENU_BOOK2;
	} else if (!Q_stricmp(type, "hbook3")) {
		return UIMENU_BOOK3;
	} else if (!Q_stricmp(type, "pregame")) {
...
intptr_t CL_CgameSystemCalls(intptr_t *args) {
	...
	switch (args[0]) {
		...
		case CG_S_CLEARLOOPINGSOUNDS:
			S_ClearLoopingSounds(args[1]); // modified so no_pvs sounds can function
			return 0;
...
This updates the "hunkusage.dat" file with the current map and it's hunk usage count. This is used for level loading, so we can show a
percentage bar dependant on the amount of hunk memory allocated so far. This will be slightly inaccurate if some settings like sound
quality are changed, but these things should only account for a small variation (hopefully).
=======================================================================================================================================
*/
...
qboolean CL_GetTag(int clientNum, char *tagname, orientation_t *or) {
	...
	if (VM_IsNative(cgvm)) {
		...
		// copy result back to game memory and free temp in reverse order tagname isn't copied back because it might be a static string
