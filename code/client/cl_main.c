...
afterwards.
[err_prot]: HACK. This is a protocol error. The client uses a custom protocol error message(client sided)in the diagnostic window.
...
void CL_ConnectionlessPacket(netadr_t from, msg_t *msg) {
	...
	if (!Q_stricmp(c, "challengeResponse")) {
		...
		if (Cmd_Argc() > 4) {
			clc.onlyVisibleClients = atoi(Cmd_Argv(4));
		} else {
...
void CL_Frame(int msec) {
	...
	// when multiple clients set their timescale to a high value and then simultaneously connect to a server it will crash
#ifndef _DEBUG
	if (!clc.demoplaying) {
...
Format: shellExecute "open" <url> <doExit>
